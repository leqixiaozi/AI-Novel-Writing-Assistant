...
导航与术语配置位于 `src/client/navigation.ts`，会随 Git 同步。数据库结构、内置数据和迁移 SQL 位于 `migrations/001_card_kernel.sql` 至 `migrations/039_chapter_writing_workspace.sql`，数据说明见 `docs/data-model.md`。
