...
本文是新设计 PostgreSQL 结构的数据字典。权威迁移位于 `../migrations/`，普通启动注册清单以 `../src/server/database/migrations.ts` 为准：当前从 `001_card_kernel.sql` 到 `083_character_dialogue.sql`，共 81 个文件，编号并不连续。`084_world_packages.sql` 至 `110_comic_episodes.sql` 中的 26 个文件属于独立安装的手动迁移，清单见 `../src/server/runtime/manifest.ts`；它们不进入普通启动。运行时执行迁移目录中的 SQL，不在代码中维护第二份 SQL 副本。文件存在、代码已接线、实际数据库已安装、能力已启用和页面已验收是不同状态；本文描述结构与业务约束，不以文件存在推定作者数据库的当前状态。

`001`—`045` 建立卡片、书籍、研究、章节生产、AI 运行、资料管理和业务表单基础；`046`—`048` 补齐统一字典树、多维标签树、历史路径快照及开书前统一审阅。`049`—`083` 扩展表单 AI、模型路由、章节结算、导演、知识索引、世界一致性、图像与人物对话等来源和回执。`084`—`110` 的手动合同另有安装、能力开关和数据保护边界。详细规则见 `unified-tree-resources.md`、`unified-book-creation-form.md` 及下文“后续迁移与页面数据边界”。
...
`038_book_overview_planning_center.sql` 为 `planning_versions` 增加 `execution_mode`，只记录作者希望后续以人工、AI 辅助或自动方式执行；该字段本身不触发正文生成。`planning_version_references` 为每个不可变计划版本保存人物视角、地点、参与者、事件、伏笔、道具和组织／势力的稳定资料 ID 与精确 `card_version_id`。数据库触发器校验资料属于同一本书、引用的是当前活动版本，且用途与内容类型匹配；伏笔引用还必须保存埋设、推进、回收、误导或揭示动作。

`planning_operation_events` 为建立、修订、驳回、归档和恢复保存请求哈希、幂等键、预期修订、结果修订和操作者。同一本书内的幂等键唯一；相同请求可安全读取既有结果，不同请求复用同一键返回 `409`。规划引用同时登记到统一依赖账本，资料版本变化可以把下游规划纳入待复核范围。

书籍概览不建立第二份汇总正本。读取模型直接从内容类型与资料、规划对象、章节正文、事实提案、状态提案、质量问题、依赖状态、AI 任务和上下文规则聚合，每项返回来源、更新时间和可进入的页面；未知值保持 `null` 或显示“暂无”。`GET /books/:id/chapters/:chapterCardId/adopted-plan` 是后续正文阶段读取当前采用章计划的稳定合同，返回规划对象修订、章节资料版本、计划版本、内容哈希、执行意图、精确资料引用和父级卷计划版本。

> 🏠 **白话比喻**：书籍概览像汽车仪表盘，只读取油量、里程和故障灯，不在仪表盘里再造一只油箱；规划候选像几条备选导航路线，只有作者明确选定的路线才交给后续驾驶。对应到系统：概览是正本投影，规划采用指针是唯一执行依据，候选和历史不会偷偷影响正文。

> 🧠 **速记方法**：**看、规、选、交**。看真实进展；规划多个候选；明确采用一个版本；把采用版本及精确引用交给正文阶段。
...
问题状态使用 `open / acknowledged / deferred / dismissed / fix_proposed / fixed / verified / stale / superseded`。其中 `deferred` 单独表达“暂后处理”，不会与已确认或忽略混在一起。版本、证据和 `quality_issue_events` 全部只追加；人工接受、延期、驳回、确认或废弃都保存操作者与理由。`quality_fix_candidates` 在 AI 补丁生成成功后自动以 `proposed` 保存不可变初版，作者采用前可以继续新增修订版本并依靠 revision 阻止并发覆盖。候选只引用被审计正文和选区，不写正文；接受后仍须由 016 的章节正文版本采用事务产生新正文，`quality_fix_adoptions` 只登记那条正式采用回执。
...
`033_business_form_provenance.sql` 在既有 `card_versions` 上增加可空 `form_version_id` 和必填 `form_resolution_kind`。前者引用本次填写实际采用的不可变创作表单版本；后者区分本书表单、内容规格布局、系统默认布局、通用布局和迁移前历史。内容类型版本继续由原有 `type_version_id` 保存。

这里只保存“用哪版表格填写”，不复制浏览器解析出的字段布局。旧修订统一标记为 `legacy`，归档、恢复或未传新来源的旧入口沿用上一修订来源。

> 🏠 **白话比喻**：档案上盖“使用 2026 版申请表”的章就能追溯填写依据，不需要再把整张空白申请表复印进每份档案。对应到系统：版本 ID 是印章，字段和值仍只保存在既有类型版本与资料修订中。
...
`040_chapter_adoption_settlement.sql` 新增书级版本化结算策略、章节采用会话、统一确认清单及版本、AI 变化提取请求、稳定检查点和只追加审计事件。它们只负责编排 016—020 的正文、事实、知识与状态正本，不复制领域数据。

首次采用可以在会话事务内切换正文；存在稳定检查点或切换到另一正文时只进入 `impact_review_required`，由 F5 读取影响合同。稳定结算只有在没有待确认／稍后处理项时执行，同一事务写正式变化、当前投影、里程碑、依赖资源、图／语义派生请求和稳定检查点。下一章只读取最近的稳定检查点。

> 🏠 **白话比喻**：040 像章节交稿时的验收总单，本身不替代正文、人物档案或事实档案，只把每份原件的签字状态和编号串起来。对应到数据库：领域表仍是原件，采用会话和稳定检查点是跨表验收凭证。
...
`043_multiview_quality_workspace.sql` 扩展 `book_view_configs` 与 `smart_views.base_view_key` 的视图键，为人物关系、道具流转、状态变化、世界规则、整章对照和质量检查补默认查看偏好，并为质量问题增加独立 `deferred` 暂后处理状态。既有书由迁移补齐，新开书由模板安装器按键合并旧模板配置与全量默认配置。它不新增业务正本表；章节、关系、时间、事实、状态和质量结果继续分别以 016—026、040—042 的正式账本为准。
...
`045_planning_ai_candidates.sql` 新增 `planning_ai_candidate_runs`，为每次 AI 规划保存目标对象、基础规划版本、资料版本清单、已采用规划版本、提示词与模型快照、Token 用量、结果版本或失败原因。真正的规划内容仍只写入 038 的 `planning_versions`，运行表不复制第二份规划正文。

> 🏠 **白话比喻**：这张表像策划提案的收件登记簿，登记“参考了哪些档案、谁交的、交到了哪个候选夹”，但提案正文仍归档在规划版本柜。对应到数据库：运行负责追溯，规划版本负责内容，采用指针负责生效。

> 🧠 **速记方法**：**运行记来路，版本放内容，采用才生效。**
...
| 故事／卷／章／场景规划 | `planning_objects`、已采用的 `planning_versions` | 未采用候选、操作事件、精确资料引用 | 计划切版不自动改正文、事实或已结算状态。 |
...
| `110` | `comic_episodes`、`comic_episode_versions`、`comic_episode_adoptions` | 分集大纲按序号保存人工候选并由作者明确采用。候选与采用回执不可改写；新候选不覆盖已采用版本。当前尚未接 AI 大纲生成、分格或图片任务，迁移仍须手动安装。 |
