...
官方 MiniMax 两种兼容接口现在使用其支持的工具参数通道承载任务结果：将原输出合同作为唯一 `submit_creative_result` 的参数规格，明确选择该工具，只接收一次同名结果。OpenAI 的 arguments 字符串与 Anthropic 的 input 对象在适配层转成统一 JSON 内容，再走原任务校验。它是结果信封，不是执行工具的代理：不运行工具、不追加工具回合、不切换协议、不降低白名单约束。未知／多个工具、缺少匹配参数、模型拒绝或达到输出上限均失败，不能退回正文冒充有效结果。其他供应商保持原输出通道。

开书方向、资料和表单提示词升级至 v2：线上的 JSON Schema 明确以当前阶段限制 directions、plans 和 relations 数量，与本地校验一致。项目／世界／人物阶段不得提前生成骨架；历史冻结规格不静默覆盖，原失败批次仍是失败，新请求使用新规格。
