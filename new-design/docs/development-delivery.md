...
2026-09-20 后续源码另增加 `106_world_usage_scope.sql` 和 `107_payoff_ledger_windows.sql` 两份默认关闭的手动结构。二者已纳入运行包手动文件清单，清单检查 2/2 通过，但不进入默认启动迁移；仅在全新空白隔离 PostgreSQL 库安装并运行正式世界候选／采用、伏笔窗口写入／原键恢复。未安装作者库，也未改变上文作者库已安装迁移数。用户要求的入 Git 数据库结构快照须在本轮全部功能完成后另行生成；结构快照不包含小说正文、凭据或其他数据行，不能代替本页所述私有完整备份。
