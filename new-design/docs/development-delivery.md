...
2026-09-20 后续源码另增加 `106_world_usage_scope.sql` 和 `107_payoff_ledger_windows.sql` 两份默认关闭的手动结构。二者已纳入运行包手动文件清单，清单检查 2/2 通过，但不进入默认启动迁移；仅在全新空白隔离 PostgreSQL 库安装并运行正式世界候选／采用、伏笔窗口写入／原键恢复。未安装作者库，也未改变上文作者库已安装迁移数。[入 Git 的新版数据库结构快照](../database/schema/README.md)从当前作者库导出，并在另一空库还原核对；它不含小说正文、凭据行或其他表数据，也不包含尚未安装的 106／107，不能代替本页所述私有完整备份。
