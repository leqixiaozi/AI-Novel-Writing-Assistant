...
本轮纯表基线和模型初始化代码仍在准备，尚未操作作者数据库；独立空库重建仅是本次明确允许的例外。以下日期段是当时操作记录，不代表后续每次更新都必须重复备份／恢复演练，也不证明本轮代码已经测试或安装。

### 2026-09-22 纯表改造与依赖清理

按用户要求提交当前代码和文档，不作为全部完工或可运行交付。`132` 空库基线已装配 79 张应用表、AGE 图定义、领域保护及基础种子；13 份源片段与装配内容逐份一致。该数量目前是源码目标，尚未在 PostgreSQL 执行核验。模型初始化仅准备受控复制代码：读取原模型路线，按目标数据库密钥重新加密凭据，不包含作者作品，不调用模型。
...
本批修正卡片记录展开后的业务字段类型、变量遮蔽、空值和状态检查，并补回上下文影响查询；清理 42 个失效导入，不新增或升级软件包。普通运行 SQL 的字面量表引用检查未发现已删除专表依赖；该检查不等同于 PostgreSQL 执行验证。挂载局部字段继续按表单实例主卡类型校验，同一修正已同步至领域 SQL 与 `132` 装配基线，未执行到数据库。
...
尚未执行：纯表隔离数据库初始化、完整非模型业务回归、实际模型配置复制、作者库变更、服务重启、浏览器验收及 Obsidian 同步。旧版业务目录未修改；未调用真实模型、未备份或还原数据库。下一步按既定节点验证纯表数据库与真实业务流程，不能把编译和定向替身检查当成实际数据库切换或全部功能验收。
