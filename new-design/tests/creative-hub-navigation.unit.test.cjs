...
test('creative hub is a primary route with strict local deep links',()=>{
  ...
  assert.deepEqual(location.readCreativeHubLocation(`?threadId=${threadId}`),{threadId,explicit:true,error:null});
...
test('client api maps sessions and original diagnostic turns without legacy runtime controls',async()=>{
  ...
  await api.list();await api.state(threadId);await api.history(threadId);await api.start(threadId,{requestKey,question:'为什么还不能继续？',expectedThreadRevision:2});await api.resume(threadId,turnId);
  assert.deepEqual(calls.map(call=>call[0]),['/creative-hub/threads',`/creative-hub/threads/${threadId}/state`,`/creative-hub/threads/${threadId}/history`,`/creative-hub/threads/${threadId}/turns`,`/creative-hub/threads/${threadId}/turns/${turnId}/resume`]);
