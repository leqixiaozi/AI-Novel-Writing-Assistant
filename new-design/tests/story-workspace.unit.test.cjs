...
function planningEditorFixture(context,failure=false){
 const previous=Object.fromEntries(['location','history','localStorage','sessionStorage'].map(key=>[key,global[key]])),store=new Map(),sessionStore=new Map();
 global.location={search:'?plan=chapter',hash:''};global.history={replaceState(){}};global.localStorage={getItem:key=>store.get(key)??null,setItem:(key,value)=>store.set(key,value),removeItem:key=>store.delete(key)};global.sessionStorage={getItem:key=>sessionStore.get(key)??null,setItem:(key,value)=>sessionStore.set(key,value),removeItem:key=>sessionStore.delete(key)};
 ...
 const module=load('src/client/planningCenter/PlanningCenterPage.tsx',{'react':h.react,'react/jsx-runtime':jsx,'../api':{newDesignApi:new Proxy(api,{get(target,key){if(!(key in target)&&key!==Symbol.toStringTag)throw Error(`Forbidden side effect ${String(key)}`);return target[key];}}),ApiError:class ApiError extends Error{}},'../BookShell':{default:()=>null},'./AiPlanningPanel':{default:()=>null},'./StageOverview':{default:()=>null},'./PlanningStageIntro':{default:()=>null},'./rhythm':{default:()=>null},'./StoryFormatFields':{default:()=>null},'./PlanningMaterialCreator':{default:()=>null},'../payoffLedger/PayoffLedgerCard':{default:()=>null},'./legacyPlanningFields':{LEGACY_PLANNING_FIELDS:{story_macro:[],outline:[],structured:[]}},'../../common/storyWorkspace':load('src/common/storyWorkspace/receipts.ts'),'../storyWorkspace/Help':{default:()=>null}});
