...
exports.seedCreationTemplate=async function(db){
 ...
 const originalRuleSql=fs.readFileSync(path.join(__dirname,'../../migrations/002_builtin_novel_cards.sql'),'utf8'),originalFields=JSON.parse(originalRuleSql.match(/'10000000-0000-4000-8000-000000000001', '11000000-0000-4000-8000-000000000001', 'project_rule',[\s\S]*?\$json\$([\s\S]*?)\$json\$::jsonb/)[1]);
 await db.query("INSERT INTO new_design.card_types(id,space_id,type_key,name,status,draft_fields) VALUES('10000000-0000-4000-8000-000000000001',$1,'project_rule','\u4f5c\u54c1\u7ea6\u5b9a','archived',$2::jsonb)",[systemSpace,JSON.stringify(originalFields)]);
 await db.query("INSERT INTO new_design.card_type_versions(id,card_type_id,version,fields) VALUES('11000000-0000-4000-8000-000000000001','10000000-0000-4000-8000-000000000001',1,$1::jsonb)",[JSON.stringify(originalFields)]);
 await db.query("UPDATE new_design.card_types SET current_version_id='11000000-0000-4000-8000-000000000001' WHERE id='10000000-0000-4000-8000-000000000001'");
