...
test('package migration set includes real registered gaps and default-off manual artifacts',()=>{
 ...
 assert.deepEqual(spec.defaultMigrationFiles,migrations.map(item=>item.fileName));assert.equal(spec.manualMigrationFiles.includes('101_character_author_trials.sql'),true);assert.equal(spec.manualMigrationFiles.includes('102_character_author_influences.sql'),true);assert.equal(spec.manualMigrationFiles.includes('103_public_title_factory.sql'),true);assert.equal(spec.manualMigrationFiles.includes('104_book_content_history.sql'),true);assert.equal(spec.manualMigrationFiles.includes('105_database_model_credentials.sql'),true);for(const name of ['106_world_usage_scope.sql','107_payoff_ledger_windows.sql','114_comic_visual_assets.sql','115_creative_hub.sql']){assert.equal(spec.manualMigrationFiles.includes(name),true);assert.equal(spec.defaultMigrationFiles.includes(name),false);}validateRuntimeMigrationFiles(manifest());
