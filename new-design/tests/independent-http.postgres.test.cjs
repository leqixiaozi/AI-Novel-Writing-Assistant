...
test("real standalone Express and PostgreSQL read existing facts and expose actionable model configuration failure",async t=>{
 ...
 const status=await(await fetch(`${base}/models/status`)).json();assert.equal(status.success,true);assert.ok(status.data.tasks.length>=8);assert.ok(status.data.tasks.some(task=>task.taskType==='chapter_generation'));
 ...
 const catalog=await(await fetch(`${base}/models/catalog`)).json();assert.equal(catalog.success,true);assert.ok(catalog.data.tasks.length>=8);assert.ok(catalog.data.tasks.some(task=>task.key==='chapter_generation'));
