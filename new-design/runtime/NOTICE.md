...
默认迁移逐项匹配 `src/server/database/migrations.ts` 的81项正式注册（最后083，编号并非连续）。17项未注册手动迁移作为文件交付到 `app/migrations/manual/`，不计入启动清单，不自动执行；source manifest与运行包校验均拒绝缺项、错名、重复或把手动迁移放入默认目录。新增手动迁移时须同步两处锁定清单。该清单校验不代表完整Windows运行包已装配或启用。
