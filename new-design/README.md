...
- 全书编排、故事时间、世界与人物维护、多维资料和专业资源沿同一本书的资料编辑；正文选区修改保留原草稿，可确认替换和撤销。
- 开书与章节 AI 提供候选，人工编辑保留，正式采用与结算需明确确认；失败定位步骤，已保存结果从原页面核对和恢复。
- 知识与参考可选择精确版本、配置专属语义模型并准备索引；本批增补精确段落上下文、专属图像生成、写法／标题专项、专业图形及人物对话接线，仍处于编码与静态收口，未完成本批统一验证。
- 中文表单保护草稿及原请求凭证；“运行记录”只读展示进度、已有用量和来源页入口，不直接重试或采用。
- 修复知识文件上传及归档影响预览异常，保留路径安全检查和历史参考。
- 创作首页增加导演总控台入口，只读跟进原来源；开发启动、Dockerfile镜像重建和数据快照／校验规程已编码，未执行本次备份或恢复。

完整历史见[阶段记录](docs/releases.md)；实际检查与未执行项目见[当前原批统一验证](docs/current-batch-unified-validation.md)，范围与下一批隔离见[执行队列](docs/p1-p2-execution-queue.md)。真实模型、用户开发库升级及界面验收不能由构建或测试桩替代。

本次增量见[中央接线静态记录](docs/central-feature-wiring-static-review.md)、[段落消费者](docs/semantic-paragraph-consumer.md)和[开发／数据交付](docs/development-delivery.md)。旧批通过记录不覆盖本次尚未执行的验证。
