...
- 章节结果确认使用中文对象与类型化字段，人工／AI 共用核对清单并明确结算；失败可核对原请求、导入已保存结果，保留草稿，不重复生成。关系状态所需正式规格在本书配置中明确发布。验证边界见 [章节确认验证](docs/chapter-settlement-editing-review.md)。
- 提示词组合可选择与排列组件、填写中文参数、查看冻结预览并明确试运行；失败定位步骤，保留输入与记录，提供核对和恢复入口。实测与未验收项见 [组合验证](docs/prompt-composition-review.md)，长期规则见 [组合执行与恢复](docs/wiki/prompt-composition-execution.md)。
...
- 内容类型保存／发布失败可从原页核对结果、查看服务器差异并继续；规格读取失败提供重读入口，打开运行维护不离开编辑页。
- 开书提供一键／分步 AI 资料准备，统一审阅后确认；失败按阶段标识并从原页面重试或人工接管。资料与正式关系／规划的范围区分见 [开书准备与恢复](docs/creation-director-review.md)。
- 空白资料也能让 AI 准备名称和必填内容，可按中文名称选择本书参考资料；默认只填空白，不替换人工内容，勾选后正常保存。实际人物、世界设定、事件链路见 [空白表单验证](docs/blank-form-ai-review.md)。
- 资料表单与多维编辑器提供 AI 填空、补充、检查、方案比较、按要求调整和分类推荐；勾选建议填入草稿，检查后随资料一起保存。
...
- 研究提案正式采用会复核选项范围并保留历史名称，重复请求不重复创建资料。
- 上下文规则支持中文资料／版本／范围选择和分组条件编辑，保留未编辑的资料入口；运行前输入表单仍在后续清理范围内。
