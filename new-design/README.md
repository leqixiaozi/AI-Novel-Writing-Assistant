...
- 单个人物作者多轮谈话与场景分析已接智能层、原任务和实际用量；统一恢复增加此原回复来源。双端编译及四项隔离检查通过，未修改正式状态、认知或正文，手动101默认关闭。谈话创作倾向、书级历史、公共标题及新书来源接续仍待完成。

- 补接完整图片资料草稿、中英优化和原文／优化发送选择；封面一步设主图、首成功补空缺及归档替补。
- 书架按正式总纲形式和作者保存的发布／原创续写／平台分类筛选，缺来源明确未设置；简易创作接通原审校状态和体验偏好。
- 独立恢复页核对并保存十类原来源已有回复；运行记录保持只读。生成前可核对完整资料并明确取舍，来源变化拒绝提交。
- 原三组验证阻塞已修复，双端编译、完整图片与分类／封面替补、跨来源原回复恢复和世界一致性隔离验证通过；新建空库备份恢复的 312 张表及图片附件一致。概览接真实质量／来源复核和类型定位，已有资料优先显示。
- 知识页默认参考资料，索引与设置分区；质量和整章对照以本身工作区为主，资料筛选编辑收进辅助区，原填写与回执保留。
- 全部批次仍在收口：补单人人物谈话及场景分析、书级历史、未开书标题工坊和知识页布局。作者启用、真实模型、其余页面验收及完整私有运行包／升级分别待验。

完整历史见[阶段记录](docs/releases.md)，全部批次实际通过、失败及后续范围见[统一施工记录](docs/legacy-page-replication-progress.md#2026-09-18-全部批次统一收口结果)。手动098默认关闭且未安装作者库，不重启作者服务、不推送发布。真实模型、作者环境升级及界面验收不能由构建或测试桩替代。

早期增量记录见[中央接线静态记录](docs/central-feature-wiring-static-review.md)、[段落消费者](docs/semantic-paragraph-consumer.md)和[开发／数据交付](docs/development-delivery.md)。旧批通过记录不覆盖本次尚未通过的检查；当前全部批次状态以统一施工记录为准。
...
- `ChapterWritingRequest` / `ChapterBodyOperation` / `ChapterAdoptionPreparation`：让人工稿和 AI 结果统一进入不可变正文候选，冻结计划与运行来源，并把正式采用交给独立确认流程。
- `ChapterAdoptionSession` / `ChapterSettlementItem` / `ChapterStableCheckpoint`：把正文采用、多领域变化审核和稳定检查点收进同一结算边界；下一章只读取稳定结果，旧章换稿先进入影响预览。
- `ChapterRevisionPreview` / `ChapterRevisionPlan` / `ChapterRevisionExecution` / `ChapterRevisionReviewFlag`：冻结旧章换稿影响与来源 generation，保存逐项处置和二次确认，从前一稳定检查点选择性重算，并把后续正文交给作者复核。
...
Outbox 是投递记录，不是另一份小说数据。业务表与小型事件引用在同一 PostgreSQL 事务提交，通用 job 只引用 024、026—029 的专业请求；后台采用至少一次投递，重复副作用仍由 inbox、结果幂等键和专业账本共同阻止。

> 🏠 **白话比喻**：业务记录是盖章原件，Outbox 是桌边待寄箱，job 是快递派送单。盖章和放信一次完成；快递可能重派，但收件方按单号只入一次账。对应到系统：数据库保证可靠交接，业务幂等合同保证重复投递不重复生效。

> 🧠 **速记方法**：**原件和信一起交，投递至少一次；新租约发新票，旧 worker 不能签**。

## 内置创作资料规格

首次初始化会得到 29 种已发布的通用小说资料规格。原有 19 种保持不变，新增题材策略、推进模式、写法配置、质量规则、世界总览、能力／科技／修炼体系、种族、文化、宗教和参考资料。“高级设置／内容类型”把它们归入创作策略、人物与组织、世界设定、剧情结构、篇章结构、参考资料六个目录；目录可折叠和搜索，但不会让子类型继承字段。

“新建书籍”提供空白、模板、一句话灵感、没有想法、市场方向、拆书参考和续写已有作品七种入口。入口只影响第一次如何准备内容；创建完成后全部进入同一套书籍创作表单、本书资料和设置空间。AI 结果先预览确认，再以来源批次写入 PostgreSQL，不会静默覆盖作者已经修改的内容。
...
“创作资源”按三类树展示：创作策略、AI 创作资源和各书资料入口。AI 创作资源包含唯一的“提示词组件”类型和四条中性组件；研究分析另有“市场信号”类型。系统定义共 31 种，但开书模板仍只安装 29 种小说资料，不会把提示词组件或市场信号混进作品事实。
...
AI 执行合同把“要做什么、用哪套提示词、实际看过什么、最终走哪个模型”分开冻结。提示词配方可组合精确版本的提示词组件；任务合同锁定 Schema、上下文策略与预算；上下文清单记录入选和排除来源；模型路由按系统、任务组、具体任务、兼容节点、书籍和单次覆盖逐层解析，并保存不可变快照。密钥只以独立安全定位引用保存，任何公开合同、清单、路由结果和快照都只返回是否已配置。
...
“我的书籍”内置原创仙侠项目《照骨山河》的 55 条生产样例，覆盖全部 19 类资料，并完成第一卷前八章和第一章五场景的规划。事件规划表单可关联人物、地点、道具与剧情线。样例的来源分析、原创转化边界和资料清单见 `docs/xianxia-production-demo.md`。
