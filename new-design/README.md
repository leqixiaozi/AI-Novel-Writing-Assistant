...
- 新建环境改用 79 张应用表、4 张 AGE 投影表的纯表代码基线；候选、采用和归档继续保留历史，不通过兼容视图写入。代码尚在集中收尾，不能将这一结构说明视为作者库已安装。
- 显式空库初始化补齐模板、字典、表单和任务合同；模型路线及凭据通过独立、受控的初始化保留，普通启动不自动复制或调用模型。
- 后续遵守[数据库增量门禁](AGENTS.md#数据库增量门禁)：先说明数据库变化，重大变更先确认，日常调试不重复全库备份／还原。
