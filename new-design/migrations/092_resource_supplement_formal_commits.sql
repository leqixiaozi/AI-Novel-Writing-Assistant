-- Manual ordinary-supplement closure only. NOT registered by startup.
-- No capability flag change; corrective candidates/settlement/resolution stay shut.
SET search_path TO new_design,public;
DO $$ BEGIN
  IF NOT EXISTS(SELECT 1 FROM schema_migrations WHERE id='091_resource_supplement_correction_origins') THEN
    RAISE EXCEPTION 'retained integrity and corrective originals must precede formal closure';
  END IF;
END $$;
CREATE TABLE resource_supplement_formal_commits (
  book_id uuid NOT NULL REFERENCES books(id),
  session_id uuid NOT NULL UNIQUE REFERENCES chapter_adoption_sessions(id),
  settlement_id uuid PRIMARY KEY REFERENCES resource_supplement_integrity_journals(settlement_id),
...
CREATE TRIGGER resource_supplement_formal_commit_history BEFORE UPDATE OR DELETE ON resource_supplement_formal_commits
  FOR EACH ROW EXECUTE FUNCTION guard_resource_supplement_integrity_history();
CREATE FUNCTION assert_resource_supplement_formal_commit_source(saved resource_supplement_formal_commits) RETURNS void LANGUAGE plpgsql SET search_path TO new_design,public AS $$
...
END $$;
CREATE FUNCTION validate_resource_supplement_formal_commit() RETURNS trigger LANGUAGE plpgsql SET search_path TO new_design,public AS $$
BEGIN
  -- resource_supplement_formal_commit_v1: full actual source proof, never a flag.
  PERFORM assert_resource_supplement_formal_commit_source(NEW);
  RETURN NEW;
END $$;
CREATE TRIGGER resource_supplement_formal_commit_guard BEFORE INSERT ON resource_supplement_formal_commits
  FOR EACH ROW EXECUTE FUNCTION validate_resource_supplement_formal_commit();
CREATE FUNCTION assert_resource_supplement_formal_closure(settlement uuid) RETURNS void LANGUAGE plpgsql SET search_path TO new_design,public AS $$
DECLARE saved resource_supplement_formal_commits%ROWTYPE;
BEGIN
  SELECT * INTO saved FROM resource_supplement_formal_commits WHERE settlement_id=settlement;
  ...
  PERFORM assert_resource_supplement_formal_commit_source(saved);
END $$;
CREATE OR REPLACE FUNCTION require_resource_supplement_closure() RETURNS trigger LANGUAGE plpgsql SET search_path TO new_design,public AS $$
