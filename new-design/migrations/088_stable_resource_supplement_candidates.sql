...
-- CANDIDATES ONLY. Requires manual 085 and 087. Formal settlement closure stays disabled.
SET search_path TO new_design,public;

DO $$ BEGIN
  IF to_regclass('new_design.chapter_resource_supplements') IS NULL
    OR NOT EXISTS(SELECT 1 FROM schema_migrations WHERE id='085_character_resource_backfill') THEN
    RAISE EXCEPTION 'candidate contract requires manual 085 and 087' USING ERRCODE='23514';
  END IF;
END $$;
...
BEGIN
  ...
  IF NEW.frozen_plan->>'assetId' IN ('new_design.character.resource_backfill','new_design.character.stable_resource_supplement') AND (
  ...
  IF TG_OP='INSERT' AND NEW.frozen_plan->>'assetId' IN ('new_design.character.resource_backfill','new_design.character.stable_resource_supplement') THEN
    IF NOT EXISTS(
