...
export interface KnowledgeSemanticReceipt {bookId:string;requestKey:string;run:SemanticRetrievalRun;modelRequestState:"not_sent"|"sent_unknown"|"completed";usage:KnowledgeEmbeddingUsage;replySaved:boolean;replyStorage:"none"|"database"|"local_evidence";canCompleteSavedResult:boolean;canEndExpiredUnknown:boolean;failure:string|null;items:Array<{assetId:string;sourceVersionId:string;parsedVersionId:string;checksum:string;segment:import("./segments").KnowledgeSegment;title:string;excerpt:string;score:number}>}
