...
export default function KnowledgeReferencePage({bookId:initialBookId}:{bookId?:string}){
  ...
  const content=<main className="nd-knowledge-page">
    <header className="nd-page-header"><div><p className="nd-eyebrow">创作参考</p><h1>知识与参考</h1><p>上传参考正文，查看解析结果，再选择用于哪本书、哪一章或哪份资料。</p></div></header>
    ...
    {workspace&&<>
      <KnowledgeIndexPanel bookId={bookId} disabled={baseLocked} onLockChange={setIndexLocked} onDirtyChange={setIndexDirty} onInspect={(id,version)=>{const exact=workspace.items.find(row=>row.id===id&&row.parsedVersionId===version);if(!exact){setError(new Error('该原文版本不在当前参考目录内或已变化；请读取参考状态后核对，不会选择其他原文代替。'));return;}if(reason||ownerIndex){setError(new Error('当前绑定或归档输入保留；请先处理这份草稿，再查看另一参考。'));return;}setSelected(exact.id);setPreview(null);}}/>
      ...
      <div className="nd-knowledge-workspace"><aside aria-label="参考目录">{workspace.items.map(row=><button className={row.id===selected?'is-selected':''} key={row.id} disabled={locked} onClick={()=>{setSelected(row.id);setPreview(null);setOwnerIndex('');setReason('');}}><span>{row.title}</span><small>{labels[row.status]} · {row.filename}</small></button>)}{!workspace.items.length&&<p>上传第一份参考文件后，在这里选择并查看。</p>}{workspace.truncated&&<p>目录有截断，未显示的参考不会被修改。</p>}</aside>
        <section>{item?<><h2>{item.title}</h2><p>{labels[item.status]} · 修订 {item.revision} · {item.byteSize} 字节</p>{item.status==='failed'&&item.failure&&<p className="nd-message is-error">解析正文失败：{item.failure}。原文件保留，点击“重新解析”仅重做本参考解析。</p>}{item.recoveryReason&&<p className="nd-message">{item.recoveryReason} <a href="#knowledge-upload">选择原文件重新上传</a></p>}<button className="nd-button" disabled={locked||item.parseAvailable!==true} onClick={()=>void send({operation:'parse',bookId,assetId:item.id,input:{requestKey:crypto.randomUUID(),expectedRevision:item.revision}})}>{item.status==='failed'?'重新解析':'解析正文'}</button>
