...
export default function KnowledgeReferencePage({bookId:initialBookId}:{bookId?:string}){
  ...
  const [selected,setSelected]=useState<string|null>(null),[selectedItem,setSelectedItem]=useState<KnowledgeReferenceItem|null>(null),[olderItems,setOlderItems]=useState<KnowledgeReferenceItem[]>([]),[nextCursor,setNextCursor]=useState<string|null>(null),[loadingOlder,setLoadingOlder]=useState(false);
  const [title,setTitle]=useState(''),[file,setFile]=useState<File|null>(null),[ownerIndex,setOwnerIndex]=useState('');
  ...
  const sequence=useRef(0),inFlight=useRef(false);const item=selectedItem?.id===selected?selectedItem:workspace?.items.find(row=>row.id===selected)??olderItems.find(row=>row.id===selected)??null;
  ...
  useEffect(()=>{const token=++sequence.current;setWorkspace(null);setOlderItems([]);setNextCursor(null);setSelectedItem(null);setResults(null);setPreview(null);setSelected(null);setReceipt(null);setPointer(bookId?readKnowledgePointer(bookId):null);setStorageBlocked(Boolean(bookId)&&knowledgePointerStorageBlocked(bookId));
    if(bookId)void newDesignApi.getKnowledgeWorkspace(bookId).then(value=>{if(sequence.current===token){setWorkspace(value);setNextCursor(value.nextCursor);}}).catch(value=>{if(sequence.current===token)capture(value);});
  },[bookId]);
  ...
  async function refresh(){if(!bookId)return;const token=sequence.current;try{const value=await newDesignApi.getKnowledgeWorkspace(bookId);if(sequence.current===token){setWorkspace(value);setOlderItems([]);setNextCursor(value.nextCursor);setNotice("参考状态已读取。此前的保存回执仍保留。");if(selected)void newDesignApi.getKnowledgeReferenceItem(bookId,selected).then(current=>{if(sequence.current===token)setSelectedItem(current);}).catch(capture);}}catch(value){capture(value);}}
  async function loadOlder(){if(!bookId||!nextCursor||loadingOlder)return;const token=sequence.current;setLoadingOlder(true);try{const page=await newDesignApi.getKnowledgeWorkspace(bookId,nextCursor);if(sequence.current===token){setOlderItems(previous=>{const seen=new Set([...(workspace?.items??[]),...previous].map(row=>row.id));return [...previous,...page.items.filter(row=>!seen.has(row.id))];});setNextCursor(page.nextCursor);}}catch(value){if(sequence.current===token)capture(value);}finally{if(sequence.current===token)setLoadingOlder(false);}}
  async function selectReference(id:string,expectedParsedVersionId?:string){if(locked)return false;if((reason||ownerIndex)&&!window.confirm('切换参考将放弃未保存的绑定或归档填写。是否继续？'))return false;const token=sequence.current;try{const current=await newDesignApi.getKnowledgeReferenceItem(bookId,id);if(sequence.current!==token)return false;if(expectedParsedVersionId&&current.parsedVersionId!==expectedParsedVersionId){setError(new Error('原文版本已变化，请重新核对该参考，不会改选其他版本。'));return false;}setSelected(id);setSelectedItem(current);setPreview(null);setOwnerIndex('');setReason('');setError(null);return true;}catch(value){if(sequence.current===token)capture(value);return false;}}
  ...
  async function acceptReceipt(value:KnowledgeWriteReceipt,current:KnowledgePendingPointer){
    ...
    setReceipt(value);setSelected(value.assetId);setSelectedItem(value.item);setPending(null);setPointer(null);setSafeRetry(false);setError(null);setNotice(`${operationLabels[value.operation]}已保存。`);
  ...
  const content=<main className="nd-knowledge-page">
    ...
    {pointer&&<section className="nd-knowledge-recovery" aria-label="原请求恢复"><h2>{operationLabels[pointer.operation]}：结果待核对</h2><p>原请求 {pointer.requestKey}。核对仅查询保存回执，不重新上传、不调用模型。</p><button className="nd-button nd-button-secondary" disabled={busy} onClick={()=>void checkOriginal()}>核对原请求</button>{safeRetry&&pending&&<button className="nd-button nd-button-secondary" disabled={busy} onClick={()=>void send(pending,true)}>按原输入重试</button>}<p>{safeRetry?'服务器确认本次未写入；重试使用同一请求和输入。':'未确认是否保存前，其他写入保持锁定，避免重复资料和重复归档。'}</p></section>}
    {receipt&&<p className="nd-message">{operationLabels[receipt.operation]}已确认保存：{receipt.item.title}。刷新失败不会撤销此回执。</p>}
    {bookId&&<button className="nd-button nd-button-secondary" disabled={busy} onClick={()=>void refresh()}>读取参考状态</button>}
    {workspace&&<>
      <nav className="nd-view-tabs" aria-label="参考资料工作区">{([["materials","参考资料"],["index","索引与检索"],["settings","设置"]] as const).map(([key,label])=><button className="nd-button" type="button" aria-pressed={tab===key} key={key} onClick={()=>setTab(key)}>{label}</button>)}</nav>{indexLocked&&tab==="materials"&&<p role="status">索引原请求需要核对，请打开“索引与检索”查看原回执。</p>}<div hidden={tab==="materials"}><KnowledgeIndexPanel view={tab==="settings"?"settings":"index"} bookId={bookId} disabled={baseLocked} onLockChange={setIndexLocked} onDirtyChange={setIndexDirty} onInspect={(id,version)=>{void selectReference(id,version).then(opened=>{if(opened){setTab("materials");requestAnimationFrame(()=>document.getElementById("knowledge-selected-reference")?.scrollIntoView({block:"start"}));}});}}/></div><div hidden={tab!=="materials"}>
      ...
      <section><h2>正文检索</h2><label className="nd-control">查找原文<input value={search} maxLength={120} onChange={event=>setSearch(event.target.value)}/></label><button className="nd-button" disabled={busy||!search.trim()} onClick={()=>void findText()}>查找正文</button><p>此处检索已解析的正文，不是语义检索。</p>{results&&<div>{results.items.map(result=><button className="nd-knowledge-search-result" disabled={locked} key={`${result.assetId}:${result.parsedVersionId}`} onClick={()=>void selectReference(result.assetId,result.parsedVersionId)}>{result.title}<span>{result.excerpt}</span></button>)}{!results.items.length&&<p>没有匹配正文。</p>}{results.truncated&&<p>结果有截断，请缩小查找范围。</p>}</div>}</section>
      <p>{workspace.capabilities.vectorReason} <a href={workspace.capabilities.settingsRoute}>打开模型设置</a></p>
      <div className="nd-knowledge-workspace"><aside aria-label="参考目录">{[...workspace.items,...olderItems].map(row=><button className={row.id===selected?'is-selected':''} key={row.id} disabled={locked} onClick={()=>void selectReference(row.id)}><span>{row.title}</span><small>{labels[row.status]} · {row.filename}</small></button>)}{!workspace.items.length&&<p>上传第一份参考文件后，在这里选择并查看。</p>}{nextCursor&&<button className="nd-button" type="button" disabled={locked||loadingOlder} onClick={()=>void loadOlder()}>{loadingOlder?'正在读取…':'加载更早的参考'}</button>}{workspace.truncated&&!nextCursor&&<p>部分使用位置尚未列出，请按书籍或资料位置进一步核对。</p>}</aside>
