...
export const AssemblyCanvas=forwardRef<CanvasHandle,{initial:CanvasSnapshot;onSelect:(value:{type:'node'|'edge'|null;id:string|null})=>void;onResourceDrop?:(resource:{kind:'meta'|'template';id:string},point:{x:number;y:number})=>void;rootId?:string;uniqueModuleRefs?:boolean}>(({initial,onSelect,onResourceDrop,rootId,uniqueModuleRefs},ref)=>{
  ...
  return <div className="nd-assembly-canvas-wrap"><div className="nd-assembly-canvas-tools" role="toolbar" aria-label="画布工具"><button type="button" className={!hand?'active':''} onClick={()=>{setHand(false);graph().disablePanning()}}>选择</button><button type="button" className={hand?'active':''} onClick={()=>{setHand(true);graph().enablePanning()}}>手型</button><button type="button" onClick={autoLayout}>自动布局</button><button type="button" onClick={()=>copySelected(false)}>复制</button><button type="button" onClick={()=>copySelected(true)}>复制组</button><button type="button" onClick={collapseSelected}>收起 / 展开</button><button type="button" onClick={removeSelected}>删除</button><span className="spacer"/><button type="button" onClick={()=>zoom(graph().zoom()-.1)}>−</button><span>{zoomLabel}%</span><button type="button" onClick={()=>zoom(graph().zoom()+.1)}>＋</button><button type="button" onClick={()=>zoom(1)}>100%</button><button type="button" onClick={()=>{graph().zoomToFit({padding:55,minScale:.45,maxScale:2});setZoomLabel(Math.round(graph().zoom()*100))}}>适应画布</button></div><div ref={host} className="nd-assembly-canvas" aria-label="AntV X6 卡片编排画布" onDragOver={event=>{if(onResourceDrop&&event.dataTransfer.types.includes('application/x-card-assembly')){event.preventDefault();event.dataTransfer.dropEffect='copy'}}} onDrop={event=>{if(!onResourceDrop)return;const raw=event.dataTransfer.getData('application/x-card-assembly');if(!raw)return;event.preventDefault();try{const resource=JSON.parse(raw) as {kind:string;id:string};if((resource.kind==='meta'||resource.kind==='template')&&typeof resource.id==='string'){const point=graph().clientToLocal({x:event.clientX,y:event.clientY});onResourceDrop({kind:resource.kind,id:resource.id},point)}}catch{/* Ignore foreign drag payloads. */}}}/>{menu&&<div className="nd-assembly-context" style={{left:menu.x,top:menu.y}} role="menu"><button type="button" onClick={()=>{copySelected(false);setMenu(null)}}>复制节点</button><button type="button" onClick={()=>{copySelected(true);setMenu(null)}}>复制节点与子节点</button><button type="button" onClick={()=>{collapseSelected();setMenu(null)}}>收起 / 展开组</button><button type="button" onClick={()=>{removeSelected();setMenu(null)}}>删除选中项</button></div>}</div>;
});
