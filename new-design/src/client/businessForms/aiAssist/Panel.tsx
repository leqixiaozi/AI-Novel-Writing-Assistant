...
const ACTIONS:Array<{key:FormAssistAction;name:string;instruction:string}>=[
  ...
  {key:"supplement",name:"补充完善",instruction:"根据当前草稿和关联资料，补充所选项目，使其具体且可用于小说创作，保持已有设定。"},
...
export default function FormAiPanel({context,fields,values,disabled}:{context:FormAiContext;fields:FieldDefinition[];values:Record<string,unknown>;disabled?:boolean}){
  const [open,setOpen]=useState(false),[action,setAction]=useState<FormAssistAction>("fill_empty"),[instruction,setInstruction]=useState(""),[fieldKeys,setFieldKeys]=useState<string[]>([]);
  ...
  useEffect(()=>{generation.current++;setRun(null);setCandidateId("");setSelected(new Set());setMessage("");setBusy(false);requestKey.current=null;adoptKey.current=null;},[context.target.bookId,context.target.cardId,context.target.cardTypeId,context.target.cardRevision]);
  ...
  const generate=async()=>{
    ...
    const captured=latest.current,payload={target:captured.target,action,instruction:[ACTIONS.find(item=>item.key===action)!.instruction,instruction.trim()].filter(Boolean).join("\n"),values:captured.values,tagIds:captured.tagIds,fieldKeys},signature=JSON.stringify(payload),key=requestKey.current?.signature===signature?requestKey.current.key:crypto.randomUUID();requestKey.current={key,signature};
  ...
  const adopt=async()=>{
    if(!run||!candidate)return;const captured=latest.current,sequence=generation.current;setBusy(true);setMessage("");const payload={candidateId:candidate.id,fieldKeys:rows.filter(row=>selected.has(row.key)&&row.fieldKey).map(row=>row.fieldKey!),treeKeys:rows.filter(row=>selected.has(row.key)&&row.treeKey).map(row=>row.treeKey!),values:captured.values,tagIds:captured.tagIds},signature=JSON.stringify(payload),key=adoptKey.current?.signature===signature?adoptKey.current.key:crypto.randomUUID();adoptKey.current={key,signature};
  ...
  return <section className="nd-form-ai"><button className="nd-form-ai-toggle" type="button" aria-expanded={open} onClick={()=>setOpen(!open)}>AI 帮我完善 <span>{open?"收起":"展开"}</span></button>{open&&<div className="nd-form-ai-body">
    ...
    <div className="nd-form-ai-actions">{ACTIONS.map(item=><button className={`nd-button nd-button-secondary${action===item.key?" is-active":""}`} type="button" disabled={busy||disabled} aria-pressed={action===item.key} key={item.key} onClick={()=>{setAction(item.key);resetRequest();}}>{item.name}</button>)}</div>
    ...
    <button className="nd-button nd-button-primary" type="button" disabled={busy||disabled||!context.target.title.trim()||action==="adjust"&&!instruction.trim()} onClick={()=>void generate()}>{busy?"处理中…":"生成建议"}</button>
    {!context.target.title.trim()&&<p className="nd-help-text">先填写资料名称，再生成建议。</p>}
