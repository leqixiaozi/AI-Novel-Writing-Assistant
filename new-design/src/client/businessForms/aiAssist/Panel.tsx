...
export default function FormAiPanel({context,fields,values,disabled}:{context:FormAiContext;fields:FieldDefinition[];values:Record<string,unknown>;disabled?:boolean}){
  ...
  useEffect(()=>{generation.current++;setReferenceCardIds([]);setReferenceKnowledgeSources([]);setRun(null);setCandidateId("");setSelected(new Set());setMessage("");setBusy(false);const original=readFormAiRequest(context.target);setRequestUnknown(Boolean(original));requestKey.current=original?{key:original,signature:"unknown_original"}:null;adoptKey.current=null;if(original){setOpen(true);setMessage("此前资料提炼请求回执待核对；填写与原请求保留，请点击“核对原生成请求”，不再次调用模型。");}else if(new URLSearchParams(location.search).has("knowledgeAsset"))setOpen(true);},[context.target.bookId,context.target.cardId,context.target.cardTypeId,context.target.cardRevision]);
  ...
  const choose=(result:FormAssistRun)=>{setRun(result);setCandidateId(result.candidates[0]?.id??"");setSelected(new Set());setNewNames(Object.fromEntries(result.newNodes.map(item=>[item.id,item.name])));if(result.status!=="running"&&requestUnknown){setRequestUnknown(false);requestKey.current=null;forgetFormAiRequest(context.target);}};
  const generate=async()=>{
    if(requestUnknown)return;
    const sequence=++generation.current;setBusy(true);setMessage("");
    const captured=latest.current,payload={target:captured.target,action,instruction:[ACTIONS.find(item=>item.key===action)!.instruction,instruction.trim()].filter(Boolean).join("\n"),values:captured.values,tagIds:captured.tagIds,fieldKeys,referenceCardIds,referenceKnowledgeSources},signature=JSON.stringify(payload),key=requestKey.current?.signature===signature?requestKey.current.key:crypto.randomUUID();requestKey.current={key,signature};
    rememberFormAiRequest(captured.target,key);
    try {let result=await newDesignApi.generateBusinessFormAi({...payload,idempotencyKey:key});
      if(sequence!==generation.current)return;choose(result);if(result.status==="running")setRequestUnknown(true);else{requestKey.current=null;forgetFormAiRequest(captured.target);}
    ...
    }catch(error){if(sequence===generation.current){const notWritten=error instanceof ApiError&&error.recovery?.mutationOutcome==="not_written";setRequestUnknown(!notWritten);if(notWritten){requestKey.current=null;forgetFormAiRequest(captured.target);}setMessage(`${error instanceof Error?error.message:"生成回执尚未确认。"} 当前填写与知识引用保留；${notWritten?"本次新请求已确认未写入，核对标识项目后可明确重新准备。":"原请求保留，请点击“核对原生成请求”；不再次调用模型。"}`);}}finally{if(sequence===generation.current)setBusy(false);}
  };
  const adopt=async()=>{
    if(!run||!candidate||requestUnknown)return;const captured=latest.current,sequence=generation.current;setBusy(true);setMessage("");const payload={candidateId:candidate.id,fieldKeys:rows.filter(row=>selected.has(row.key)&&row.fieldKey).map(row=>row.fieldKey!),treeKeys:rows.filter(row=>selected.has(row.key)&&row.treeKey).map(row=>row.treeKey!),values:captured.values,tagIds:captured.tagIds,title:captured.target.title},signature=JSON.stringify(payload),key=adoptKey.current?.signature===signature?adoptKey.current.key:crypto.randomUUID();adoptKey.current={key,signature};
  ...
  return <section className="nd-form-ai"><button className="nd-form-ai-toggle" type="button" aria-expanded={open} onClick={()=>setOpen(!open)}>AI 帮我完善 <span>{open?"收起":"展开"}</span></button>{open&&<div className="nd-form-ai-body">
    ...
    <FormAiReferences bookId={context.target.bookId} selected={referenceCardIds} knowledgeSelected={referenceKnowledgeSources} disabled={busy||disabled||requestUnknown} onChange={ids=>{setReferenceCardIds(ids);resetRequest();}} onKnowledgeChange={sources=>{setReferenceKnowledgeSources(sources);resetRequest();}}/>
    ...
    <button className="nd-button nd-button-primary" type="button" disabled={busy||disabled||requestUnknown||action==="adjust"&&!instruction.trim()} onClick={()=>void generate()}>{busy?"处理中…":run?.status==="failed"?"重试生成建议":"生成建议"}</button>{requestUnknown&&<button className="nd-button" type="button" disabled={busy} onClick={()=>void checkOriginal()}>核对原生成请求</button>}
    ...
    {run&&<div className="nd-form-ai-review"><p className="nd-kicker">基于生成时的表单草稿 · 第 {run.snapshot.target.cardRevision??1} 次资料修订</p>
      ...
      <button className="nd-button nd-button-primary" type="button" disabled={busy||disabled||requestUnknown||!selected.size} onClick={()=>void adopt()}>采用勾选项到草稿</button></>}
      {run.newNodes.map(item=>{const tree=run.snapshot.trees.find(tree=>tree.key===item.treeKey);return <div className="nd-form-ai-new" key={item.id}><label className="nd-control"><span>{tree?.name} · 建议新增（仅本书）</span><input value={newNames[item.id]??item.name} disabled={busy||requestUnknown} onChange={event=>setNewNames(names=>({...names,[item.id]:event.target.value}))}/></label><button className="nd-button nd-button-secondary" type="button" disabled={busy||disabled||requestUnknown||!tree?.rule.allowInlineCreate||!(newNames[item.id]??item.name).trim()} onClick={()=>{if(requestUnknown)return;setBusy(true);void newDesignApi.confirmFormAiNode(context.target.bookId,run.id,{suggestionId:item.id,name:(newNames[item.id]??item.name).trim(),idempotencyKey:crypto.randomUUID()}).then(result=>setMessage(result.message)).catch(error=>setMessage(error.message)).finally(()=>setBusy(false));}}>确认新增到本书</button>{!tree?.rule.allowInlineCreate&&<small>该范围不允许原地新增，请到创作资源维护书内选项。</small>}</div>;})}
      <button className="nd-text-button" type="button" disabled={busy||requestUnknown||run.status==="running"} onClick={()=>void discard()}>丢弃这组建议</button>
