...
import type {ScopedFieldDefinition,AssociationWorkspace} from "../../../common/contracts";
...
export function useFieldWriteRecovery(bookId:string,cardTypeId:string,context:Record<string,string|null>,restore:(raw:Record<string,unknown>)=>void,refresh:()=>Promise<void>){
  ...
  async function finish(value:Pending,result?:ScopedFieldDefinition|AssociationWorkspace){if(value.command.operation!=='mount'&&(!result||!matches(result as ScopedFieldDefinition,value.command)))throw new Error("原保存返回值与填写范围不一致，保留原凭证核对。");if(value.command.operation==='mount'){const command=value.command;const item=(result as AssociationWorkspace|undefined)?.items.find(item=>item.id===command.mountId);if(!item||!item.localFields.some(field=>field.scope==='card_mount'&&field.cardMountId===command.mountId&&field.currentVersion.field.name===value.command.input.field.name))throw new Error("原关联字段回执范围不一致。");}await refresh();localStorage.removeItem(storage);current.current=null;setPending(null);setMessage("原保存结果已确认。");return true;}
  async function perform(command:FieldCommand,raw:Record<string,unknown>){if(current.current||running.current||storageBlocked.current)return false;
    ...
    let confirmed=false;
    try{let result:ScopedFieldDefinition|AssociationWorkspace|undefined;switch(command.operation){case 'book':result=await newDesignApi.createBookFieldExtension(bookId,command.input);break;case 'card':result=await newDesignApi.createCardLocalField(bookId,command.cardId,command.input);break;case 'revise':result=await newDesignApi.reviseCardLocalField(bookId,command.fieldId,command.input);break;case 'mount':result=await newDesignApi.addAssociationLocalField(bookId,command.mountId,command.input);break;}
      confirmed=true;
  ...
  async function verify(){const value=current.current;if(!value||running.current)return false;
    running.current=true;setWorking(true);try{if(value.command.operation==='mount'){const workspace=await newDesignApi.verifyAssociationWrite(bookId,{operation:'add_local_field',targetId:value.command.mountId,input:value.command.input});if(!workspace){setMessage('原关联回执尚未读取，保留原请求继续核对。');return false;}return await finish(value,workspace);}const receipt=await newDesignApi.verifyFieldWriteReceipt(bookId,value.command);if(!receipt){setMessage("尚未读取原回执，不能证明未保存；保留原请求继续核对。");return false;}const operations={book:'book_field_create',card:'card_field_create',revise:'card_field_revise'};
