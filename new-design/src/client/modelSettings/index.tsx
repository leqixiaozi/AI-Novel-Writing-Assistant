...
export default function ModelSettingsPage() {
  ...
  const useServer = (value: ModelRouteCenterCatalog, next: RouteSelection) => {
    setCatalog(value); setSelection(next); setBase(selectedRoute(value, next)); setDraft(initialSettings(value, next)); setReplaceUnsupported(false); setPreview(null); setCredentialId(""); setCredentialSecret("");
  };
  const read = async () => {
  ...
  const addCredential = async () => {
    if (!draft || locked || !credentialName.trim() || !credentialSecret) return;
    setBusy(true); setError("");
    try {
      await newDesignApi.saveManagedModelCredentialSecret({ name: credentialName.trim(), provider: draft.primary.provider, apiKey: credentialSecret, credentialId: credentialId || null });
      setNotice("密钥已加密保存到新版数据库；页面不会再次显示密钥。请从路线的凭据列表明确选择。" );
      setCredentialName(""); setCredentialSecret(""); setCredentialId("");
    ...
    } catch (reason) { report(reason, "保存数据库模型凭据"); if (!(reason instanceof ApiError) || reason.status === 409 || reason.status >= 500) { setCredentialOutcomeUnknown(true); setCredentialChecked(false); } }
    finally { setBusy(false); }
  };
  ...
  return <StructureShell title="模型设置" description="选择创作任务，配置模型、备用方案与用量上限。保存后仅影响后续生成，已有资料及运行快照保留。">
    ...
    {credentialOutcomeUnknown && <section className="nd-model-recovery"><h2>核对密钥保存结果</h2><p>不自动重复保存。请读取目录核对原凭据；目录只显示就绪状态，不回显密钥。</p>{credentialChecked && <ul>{catalog?.credentials.map(item => <li key={item.id}>{item.label} · {item.available ? "凭据可用" : "凭据未就绪"}</li>)}</ul>}<div className="nd-action-row"><button className="nd-button" disabled={busy} onClick={() => void read()}>核对服务器结果</button><button className="nd-button" disabled={busy || !credentialChecked} onClick={() => { setCredentialOutcomeUnknown(false); setError(""); setNotice("已核对凭据目录；当前模型路线编辑保留。若原密钥更新仍未知，可明确重新提交同一密钥。"); }}>已核对凭据目录，继续编辑</button></div></section>}
    {error && <section className="nd-message is-error" role="alert"><h2>{failedStep}未完成</h2><p>{error}</p>{Object.entries(issues).length > 0 && <ul>{Object.entries(issues).map(([path, detail]) => <li key={path}>{({provider:"模型服务",endpoint:"连接地址",model:"创作模型",credentialId:"凭据引用",apiKey:"服务密钥",failureCategories:"备用技术失败类别",maxOutputTokens:"每次输出上限",maxTotalTokens:"总用量上限",timeoutMs:"单次等待",maxRetries:"技术重试次数",retryDelayMs:"重试间隔",name:"凭据名称",expectedRevision:"服务器修订",expectedConfigId:"路线身份"} as Record<string,string>)[path.split(".").at(-1) ?? ""] ?? "设置内容"}：{detail}</li>)}</ul>}<p>{saved ? "已成功保存的版本保留，此处失败是读取或后续操作，不是原保存失败。" : "当前编辑和已保存创作内容保留。"}{pending ? "提交结果需先核对；写入及切换已暂停，避免重复提交。请点击“核对服务器结果”。" : "字段校验失败请核对列出的字段后点击“保存并启用”；读取失败点击“重新读取目录”，连接检查失败可在连接区域重试。"}</p></section>}
    ...
    <section className="nd-model-route-editor" aria-busy={busy}>{draft && catalog ? <><header><h2>{selection === "default" ? "默认创作模型" : MODEL_TASKS.find(task => task.key === selection)?.label}</h2><p>{base ? `当前修订 ${base.revision} · ${base.published ? `生效第 ${base.published.version} 版` : "尚未启用"}` : selection === "default" ? "填写连接信息后保存启用。" : "此任务继承默认设置。保存将建立独立任务路线。"}</p></header>{(base?.configurationIssue || base?.editable === false) && <section className="nd-message is-error"><h3>历史设置需要明确处理</h3><p>{base?.configurationIssue ?? "此路线包含此页不能安全编辑的历史设置，请明确确认处理方式。"}</p><label><input type="checkbox" disabled={locked} checked={replaceUnsupported} onChange={event => setReplaceUnsupported(event.target.checked)}/>我确认用此页支持的设置替换未知历史参数；历史版本仍保留</label></section>}
      ...
      <details className="nd-model-credential-details"><summary>数据库模型凭据</summary><p>密钥加密保存在新版数据库；目录与读取接口只显示名称和就绪状态，不回显密钥。更新原凭据会保留其引用及模型路线。不要将密钥放入资料、提示词或 Git。</p><fieldset disabled={locked} className="nd-model-form-grid"><label className="nd-control">更新原凭据或新建<select value={credentialId} onChange={event => { const id=event.target.value; setCredentialId(id); setCredentialName(catalog.credentials.find(item=>item.id===id)?.label??""); }}><option value="">新建数据库凭据</option>{catalog.credentials.filter(item=>item.provider===draft.primary.provider&&item.status==="active").map(item=><option key={item.id} value={item.id}>{item.label}{item.available?"":"（未就绪）"}</option>)}</select></label><label className="nd-control">凭据名称<input value={credentialName} onChange={event => setCredentialName(event.target.value)} readOnly={Boolean(credentialId)}/></label><label className="nd-control">服务密钥<input type="password" autoComplete="new-password" value={credentialSecret} maxLength={8192} onChange={event => setCredentialSecret(event.target.value)} placeholder="只在保存时发送；不会回显"/></label><button className="nd-button" disabled={!credentialName.trim() || !credentialSecret} onClick={() => void addCredential()}>{credentialId?"更新并加密保存密钥":"创建并加密保存凭据"}</button></fieldset></details>
