...
export default function BooksPage() {
  ...
  return <div className="nd-shell">
    ...
    {books.length?<div className="nd-book-grid">{books.map((book)=><article className="nd-book-card" key={book.id}><div><p className="nd-kicker">开书模板 · {book.templateName} v{book.templateVersion}</p><h2>{book.name}</h2><p>{book.description||"尚未填写书籍说明。"}</p></div><dl><div><dt>本书资料</dt><dd>{book.cardCount}</dd></div><div><dt>创作表单</dt><dd>{book.formCount}</dd></div></dl><div className="nd-row-actions"><a className="nd-button nd-button-primary" href={`/new-design/books/${book.id}/forms`}>进入创作</a><a className="nd-button nd-button-secondary" href={`/new-design/books/${book.id}/cards`}>本书资料</a></div></article>)}</div>:<div className="nd-empty nd-empty-page"><strong>还没有书籍</strong><span>选择适合你的起点，系统会把内容整理进统一创作表单。</span><a className="nd-button nd-button-primary" href="/new-design/books/new">新建书籍</a></div>}
  </div>;
}
