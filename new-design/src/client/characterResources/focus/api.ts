...
export function createResourceFocusApi(request:Request):ResourceFocusApi {
 ...
 return{
  previewResourceFocus:(book,actor,selection,includeHistory)=>request(`${base(book,actor)}/preview?${new URLSearchParams({...selection,...(includeHistory?{includeHistory:'true'}:{})})}`),
