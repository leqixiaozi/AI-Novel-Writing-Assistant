...
export function useResourceFocus(bookId:string,characterId:string,onRestore:(selection:ResourceLedgerSelection)=>void,initialId?:string|null){
 ...
 const prepare=async(selection:ResourceLedgerSelection,includeHistory=false)=>{if(locked)return;const seq=epoch.current;setBusy(true);setError('');try{const result=await newDesignApi.previewResourceFocus(bookId,characterId,selection,includeHistory);if(alive.current&&seq===epoch.current){setPreview(result);setVerifiedHash(null);setApplied(false);}}catch(reason){if(alive.current&&seq===epoch.current)setError(reason instanceof Error?reason.message:'原资源范围未完整读取。');}finally{if(alive.current&&seq===epoch.current)setBusy(false);}};
 ...
 const apply=async()=>{if(locked||!record?.output||record.status!=='review')return;await action(record.request,false,async seq=>{const current=await newDesignApi.previewResourceFocus(bookId,characterId,record.request.selection,record.request.includeHistory);if(!alive.current||seq!==epoch.current)return;if(current.sourceHash!==record.request.expectedSourceHash||!sameResourceFocusValue(current.snapshot,record.snapshot)){setApplied(false);setVerifiedHash(null);live.current(current.snapshot.selection);setError('原建议来源已变化，请保留原结果并查看全部资源。');return;}live.current(current.snapshot.selection);persist(record,true,false,true);setVerifiedHash(current.sourceHash);setApplied(true);});};
...
export function ResourceFocusPanel({controller,selection,disabled}:{controller:ResourceFocusController;selection:ResourceLedgerSelection|null;disabled:boolean}){
 ...
 return <section aria-label="人物资源显示建议">
  ...
  <button className="nd-button" disabled={disabled||controller.locked||!selection} onClick={()=>void controller.prepare(selection!,true)}>核对原确认的转交与淡出</button>
  {controller.preview?.snapshot.history&&<details><summary>本次原确认历史 · {controller.preview.snapshot.history.items.length} 条资源关联</summary>{controller.preview.snapshot.history.items.map(item=><section key={item.relationId}><h5>{item.name} · 当前持有：{item.currentHolding?.available?item.currentHolding.display:'未确认'}</h5>{item.changes.map(change=><p key={change.id}>{change.available?'原确认已核对':'原来源待核对'} · 第 {change.chapterOrder} 章 · {change.fieldLabel}：{change.beforeDisplay} → {change.afterDisplay}；{change.original.anchor?String(change.original.anchor.excerpt):'缺少正文证据'} <a href={change.sourceRoute}>核对原章节</a></p>)}</section>)}</details>}
  {controller.record?.output?.history&&<details><summary>AI 原确认显示建议与正文出处</summary>{controller.record.output.history.map(item=><p key={item.relationId}>{controller.record!.snapshot.history?.items.find(source=>source.relationId===item.relationId)?.name}：{({transferred:'已转交',stale:'已淡出',other:'其它原确认',unknown:'待核对'})[item.status]}；{item.explanation}{item.evidence.map((proof,index)=><span key={index}> · 原确认 {proof.changeId}：{proof.excerpt}</span>)}</p>)}</details>}
  {controller.preview&&<><p>人物：{controller.preview.snapshot.ledger.characterName}；{controller.preview.snapshot.ledger.items.length} 条当前档案关联，{controller.preview.snapshot.history?.items.length??0} 条历史资源关联。</p><details><summary>本次判断的原档案与策划</summary>{controller.preview.snapshot.evidenceSources.map(source=><p key={`${source.cardId}:${source.fieldKey}`}>{controller.preview!.snapshot.objects.find(object=>object.id===source.cardId)?.title} · {source.fieldKey}：{source.text}</p>)}</details>
