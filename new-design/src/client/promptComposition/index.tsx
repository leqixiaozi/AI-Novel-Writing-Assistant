...
export default function PromptCompositionPage(){
  ...
  useEffect(()=>{if(!catalog||base||preview||draft.context.bookId||draft.name)return;const query=new URLSearchParams(location.search),ids=query.getAll("bookId");if(ids.length===0)return;if(ids.length!==1||!catalog.books.some(book=>book.id===ids[0]&&book.status==="active")){setError("知识参考链接的书籍重复、无效或已归档；请选择正确书籍后重新核对来源，未加入参考或调用模型。");setFailedStep("核对知识参考链接的本书范围");return;}void changeBook(ids[0]);setNotice("已读取知识页所选书籍，请核对精确知识来源后明确加入组合，再保存和预览。");},[catalog]);
  ...
  return <ResourceShell active="composition"><main className="nd-composition-page"><p>试运行选择参考书籍，不写小说内容。组件分类用于查找，组合中的顺序决定指令排列。</p>
    ...
    <KnowledgePicker context={draft.context} sources={sources} disabled={locked||base?.editable===false} onChange={context=>change({...draft,context})}/>
    ...
    {preview&&Boolean(preview.knowledgeSources?.length)&&<section className="nd-composition-preview"><h2>此冻结预览的知识引用</h2><p>这里只展示原预览已经冻结的选择，不随当前草稿变化；过期来源仍会在执行前重新核对。</p><ul>{preview.knowledgeSources?.map(source=><li key={source.parsedVersionId}>{source.title} · {source.segment?`精确字位 ${source.segment.start}～${source.segment.end}（从零计，末位不包含）`:"解析全文"}<details><summary>核对精确版本</summary><code>{source.parsedVersionId} · {source.checksum}</code>{source.segment&&<p>原分块：{source.segment.chunkId} · 段落校验：{source.segment.checksum}</p>}</details></li>)}</ul></section>}
    {catalog&&<PreviewResult catalog={catalog} preview={preview} result={result} canRun={previewFresh&&!dirty&&!pending} busy={locked} onRun={()=>void run()} onRefresh={()=>void refreshResult()} onComponentChange={(id,enabled)=>{if(locked)return;change({...draft,components:draft.components.map(item=>item.cardId===id?{...item,enabled}:item)});setNotice('来源取舍已加入草稿，请重新保存并预览。');}} onExcludeSource={id=>{if(locked)return;change({...draft,context:{...draft.context,sources:draft.context.sources.filter(item=>item.cardId!==id)}});setNotice('来源排除已加入草稿，请重新保存并预览。');}}/>}<p><a href={COMPOSITION_ROUTE}>组合页面入口</a> · <a href="/new-design/resources/prompts" target="_blank" rel="noreferrer">维护指令组件</a></p>
  </main></ResourceShell>;
}
