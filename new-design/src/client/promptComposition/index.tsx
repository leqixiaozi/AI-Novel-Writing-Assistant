...
export default function PromptCompositionPage(){
  ...
  useEffect(()=>{if(!catalog||base||preview||draft.context.bookId||draft.name)return;const query=new URLSearchParams(location.search),ids=query.getAll("bookId");if(ids.length!==1||!catalog.books.some(book=>book.id===ids[0]&&book.status==="active"))return;void changeBook(ids[0]);setNotice("已读取知识页所选书籍，请核对精确知识来源后明确加入组合，再保存和预览。");},[catalog]);
