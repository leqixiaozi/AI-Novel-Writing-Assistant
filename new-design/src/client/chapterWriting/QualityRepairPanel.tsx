...
export default function QualityRepairPanel({bookId,document,issues,disabled,existingInstruction,onPrepare}:{bookId:string;document:ChapterDocumentDetail;issues:QualityIssue[];disabled:boolean;existingInstruction:string;onPrepare:(version:ChapterBodyVersion,instruction:string)=>void}){
 ...
 const [source,setSource]=useState<QualityIssue|null>(null),[busy,setBusy]=useState(false),[message,setMessage]=useState('');
 ...
 useEffect(()=>{
  const sequence=++generation.current;setSource(null);setMessage('');setBusy(false);
  if(sourceId)void newDesignApi.getQualityIssue(sourceId).then(issue=>{if(sequence!==generation.current)return;if(issue.bookId!==bookId)throw Error('指定质量问题不属于本书，未切换到其他问题。');setSource(issue);}).catch(error=>{if(sequence===generation.current)setMessage(error instanceof Error?error.message:'原质量问题未读取。');});
  return()=>{generation.current++;};
 },[bookId,document.id,sourceId]);
 const items=source&&!issues.some(issue=>issue.id===source.id)?[source,...issues]:issues;
 ...
 return <section className="nd-quality-repair"><header><h3>质量问题修复</h3><a href={`/new-design/books/${bookId}/views/quality`}>查看证据与处理决定</a></header>{message&&<p role="status">{message}</p>}{items.length?<ul>{items.map(issue=><li key={issue.id}><strong>{issue.currentVersion.title}</strong><small>{issue.currentVersion.suggestedAction||issue.currentVersion.description}</small><button className="nd-button" type="button" disabled={disabled||busy||!actionable.has(issue.currentStatus)} onClick={()=>void prepare(issue)}>准备此问题修复</button></li>)}</ul>:<p>本章暂无已登记问题。没有报告时不生成虚构诊断。</p>}</section>;
}
