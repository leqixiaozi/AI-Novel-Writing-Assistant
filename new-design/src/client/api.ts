...
export const newDesignApi = {
  ...
  saveStoryTime:(bookId:string,input:{cardId:string;startOrder:number|null;endOrder:number|null;startLabel:string;endLabel:string;uncertainty:string;revision?:number})=>request<StoryTimePosition>(`/books/${bookId}/story-time`,{method:"PUT",body:JSON.stringify(input)}),
  saveNarrativePlacement:(bookId:string,input:{subjectCardId:string;chapterCardId:string;sceneCardId?:string|null;role:NarrativePlacement["role"];note:string;revision?:number})=>request<NarrativePlacement>(`/books/${bookId}/narrative-placement`,{method:"PUT",body:JSON.stringify(input)}),
  saveCharacterRelation:(bookId:string,input:{sourceCardId:string;targetCardId:string;sourceLabel:string;inverseLabel:string;note:string;revision?:number})=>request<CharacterRelation>(`/books/${bookId}/character-relation`,{method:"PUT",body:JSON.stringify(input)}),
  saveClueLifecycle:(bookId:string,input:{clueCardId:string;plantChapterId:string;revealChapterId:string;plantAnchor:string;revealAnchor:string;plantPlacementRevision?:number;revealPlacementRevision?:number;plantAnchorRevision?:number;revealAnchorRevision?:number})=>request<BookViewWorkspace>(`/books/${bookId}/clue-lifecycle`,{method:"PUT",body:JSON.stringify(input)}),
