...
export default function NewDesignLanding() {
  ...
  return <div className="nd-shell">
    ...
    {message?<div className="nd-fatal"><h2>数据库未就绪</h2><p>{message}</p></div>:<div className="nd-entry-grid"><a className="nd-entry-card is-primary" href={recent?`/new-design/books/${recent.id}/forms`:"/new-design/books"}><p className="nd-kicker">推荐入口</p><h2>{recent?`继续《${recent.name}》`:"开始第一本书"}</h2><p>{recent?`${recent.cardCount} 条资料已经在独立书籍空间中就绪。`:"选择模板，建立一套可持续生产的书籍资料。"}</p><span>进入我的书籍 →</span></a><a className="nd-entry-card" href="/new-design/resources"><p className="nd-kicker">跨书复用</p><h2>资源中心</h2><p>选择题材、推进、写法和质量策略，安装为本书独立快照。</p><span>浏览创作资源 →</span></a><a className="nd-entry-card" href="/new-design/structure/card-types"><p className="nd-kicker">底座管理</p><h2>结构设计中心</h2><p>维护 29 种资料规格、稳定字典、关系、组合表单和模板版本。</p><span>管理结构 →</span></a></div>}
  </div>;
}
