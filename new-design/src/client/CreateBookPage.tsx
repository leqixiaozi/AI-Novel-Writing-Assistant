...
import type { BookCreationMethod, BookCreationSession, InspirationCandidate, StrategyResourceSummary, TemplateGroupSummary } from "../common/contracts";
...
export default function CreateBookPage() {
  ...
  useEffect(()=>{void Promise.all([newDesignApi.listTemplates(),newDesignApi.listInspirationCandidates(),newDesignApi.listStrategyResources()]).then(([allTemplates,allInspirations,allStrategies])=>{const published=allTemplates.filter((item)=>item.currentVersionId);setTemplates(published);setTemplateVersionId(published[0]?.currentVersionId??"");setInspirations(allInspirations);setInspirationId(allInspirations[0]?.id??"");setStrategyResources(allStrategies);}).catch((error)=>setMessage(error instanceof Error?error.message:"开书选项加载失败。"));},[]);
