...
import { homeBookAction, homeDraftAction, homeJourneyMetric } from "../../common/home/presentation";
import defaultCover from "./default-novel-cover.webp";
...
export function HomeFirstBook({ snapshot, models, book }: { snapshot: HomeSnapshot; models: HomeModelStatus | null; book: HomeBookFact | null }) {
  ...
  if (completed === stages.length) return null;
  const next = stages.find(stage => !stage.complete);
  return <section className="nd-home-first" aria-labelledby="home-first-title"><span className="nd-home-first-icon" aria-hidden="true">◉</span><div><strong id="home-first-title">第一本书向导 <small>{completed}/{stages.length} 步完成</small></strong><p>{next ? next.title === "AI 配置" && !models ? "正在核对 AI 配置；人工整理作品仍可继续。" : `继续完成：${next.title}` : "首书准备已完成，继续创作或查看本书进展。"}</p></div><div className="nd-home-milestones" aria-label={`已完成 ${completed} 个步骤`}>{stages.map(stage => <a href={stage.href} key={stage.title} className={stage.complete ? "has-evidence" : ""} aria-label={`${stage.title}：${stage.complete ? "已完成" : "待完成"}`} title={`${stage.title}：${stage.complete ? "已完成" : "待完成"}`} />)}</div><a className="nd-home-first-next" href={next?.href ?? (route ? `${route}/overview` : "/new-design/books")}>{next ? "继续准备 →" : "查看进展 →"}</a></section>;
}
function HomeStarter({ draft }: { draft: HomeCreationDraft | null }) {
  ...
  return <section className="nd-home-hero nd-home-starter">
    <div className="nd-home-hero-main"><p className="nd-home-overline">{draft ? "继续你的开书草稿" : "开始第一本小说"}</p><h1>{draft ? draft.name || "一个正在酝酿的故事" : "把一个模糊想法，写成完整故事"}</h1><p className="nd-home-description">{action?.reason ?? "选择灵感或模板，让 AI 陪你准备整本结构；也可以从空白表单开始，亲手填写故事设定。"}</p><div className="nd-home-next"><span>{action ? "下一步" : "从一个想法开始"}</span><h3>{action?.title ?? "边准备，边审阅，再继续"}</h3><p>在开书页选择自动准备、逐步审阅或人工填写，所有结果均可明确确认。</p></div></div>
...
export function HomeHero({ book, draft, coverUrl }: { book: HomeBookFact | null; draft: HomeCreationDraft | null; coverUrl: string | null }) {
  ...
  const journey = homeJourneyMetric(book);
  return <>
    ...
    <section className="nd-home-hero" aria-labelledby="home-book-title"><div className="nd-home-hero-main"><div className="nd-home-overline">继续你的故事 <span className={`nd-home-tag is-${action.tone}`}>{action.tone === "attention" ? "等你处理" : action.tone === "running" ? "创作进行中" : "你的作品"}</span></div><p className="nd-home-book-label">正在创作</p><h1 id="home-book-title">《{book.name}》</h1><p className="nd-home-description">{book.description || "从本书的方向、人物和世界出发，一章章写下你的故事。"}</p><div className="nd-home-next"><span>下一步</span><h3>{action.title}</h3><p>{action.reason}</p></div>
      <section className="nd-home-journey" aria-labelledby="home-journey-title"><div className="nd-home-section-heading"><div><h3 id="home-journey-title">整本创作旅程</h3><small>这里记录已开展的环节；完本状态请看「完本与导出」</small></div><div className="nd-home-journey-progress"><strong>{journey.value}</strong><small>{journey.caption}</small></div></div><div className="nd-home-journey-track"><span style={{ width: `${journey.trackPercent}%` }} /></div><ol>{journey.stages.map((stage, index) => <li key={stage.label}><a href={stage.href} className={stage.evidenced ? "has-evidence" : ""} title={`${stage.label}：${stage.detail}`}><span className="nd-home-stage-number" aria-hidden="true">{String(index + 1).padStart(2, "0")}</span><strong>{stage.label}</strong><small>{stage.detail}</small></a></li>)}</ol></section>
    </div><aside className="nd-home-book-side"><div className="nd-home-cover-row"><div className="nd-home-book-cover" aria-label={`《${book.name}》书名展示`}><img src={coverUrl ?? defaultCover} alt={coverUrl ? `《${book.name}》封面` : ""} />{!coverUrl && <><span>我的创作</span><strong>{book.name}</strong><small>一章一章，写成故事</small></>}</div><p>{book.description || "从本书的方向、人物和世界出发，一章章写下你的故事。"}</p></div><a className="nd-button nd-button-primary" href={action.href}>{action.label} →</a><a className="nd-home-secondary-link" href={`${route}/overview`}>打开本书工作台</a><a className="nd-home-secondary-link" href="/new-design/operations/director">查看所有作品进展</a></aside>
