import AssociationsPublication from "./referenceParity/AssociationsPublication";
...
export default function TemplateGroupsPage(){
  const [referenceLocked,setReferenceLocked]=useState(false),[associationLocked,setAssociationLocked]=useState(false),[visibleLocked,setVisibleLocked]=useState(false),[resourceLocked,setResourceLocked]=useState(false);
  ...
  const locked=resourceLocked||referenceLocked||visibleLocked||associationLocked||recovery.locked||syncRecovery.locked,dirty=!!draft&&structureDraftHash(draft)!==baseline.current;
  ...
  return <StructureShell title="开书模板" description="把内容类型、选项、资料关联、创作表单和菜单冻结成版本，再安全安装到每本书。">
    <div className="nd-structure-workspace"><aside className="nd-catalog-list"><div className="nd-list-heading"><div><p className="nd-kicker">模板目录</p><strong>{templates.length} 个</strong></div><button disabled={locked} type="button" onClick={()=>choose(blankTemplate())}>＋</button></div>{templates.map(template=><button disabled={locked} className={draft?.id===template.id?"is-selected":""} key={template.id} type="button" onClick={()=>choose(template)}><strong>{template.name}</strong><small>{template.currentVersion?'已发布 v'+template.currentVersion:"草稿"}</small></button>)}</aside>
      <section className="nd-structure-editor">{draft?<>
        ...
        {draft.id&&<ReferenceSpecification template={draft} disabled={resourceLocked||visibleLocked||associationLocked||recovery.locked||syncRecovery.locked||dirty} onLock={setReferenceLocked} onPublished={value=>{const selected=++selection.current;draftRef.current=structuredClone(value);baseline.current=structureDraftHash(value);setDraft(draftRef.current);setPreviews({});void readVersions(value.id,selected);void load().catch(()=>setMessage("目录读取失败，原发布结果保留。"));}}/>}
        {draft.id&&<ReferenceSpecification kind="visible" template={draft} disabled={resourceLocked||referenceLocked||associationLocked||recovery.locked||syncRecovery.locked||dirty} onLock={setVisibleLocked} onPublished={value=>{const selected=++selection.current;draftRef.current=structuredClone(value);baseline.current=structureDraftHash(value);setDraft(draftRef.current);setPreviews({});void readVersions(value.id,selected);void load().catch(()=>setMessage("目录读取失败，原发布结果保留。"));}}/>}
        {draft.id&&<ReferenceSpecification kind="resource" template={draft} disabled={visibleLocked||referenceLocked||associationLocked||recovery.locked||syncRecovery.locked||dirty} onLock={setResourceLocked} onPublished={value=>{const selected=++selection.current;draftRef.current=structuredClone(value);baseline.current=structureDraftHash(value);setDraft(draftRef.current);setPreviews({});void readVersions(value.id,selected);void load().catch(()=>setMessage("目录读取失败，原发布结果保留。"));}}/>}
        {draft.id&&<AssociationsPublication template={draft} disabled={resourceLocked||visibleLocked||referenceLocked||recovery.locked||syncRecovery.locked||dirty} onLock={setAssociationLocked} onPublished={value=>{const selected=++selection.current;draftRef.current=structuredClone(value);baseline.current=structureDraftHash(value);setDraft(draftRef.current);setPreviews({});void readVersions(value.id,selected);void load().catch(()=>setMessage("目录读取失败，原发布结果保留。"));}}/>}
