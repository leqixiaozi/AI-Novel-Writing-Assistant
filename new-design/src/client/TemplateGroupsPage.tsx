...
export default function TemplateGroupsPage(){
  const [referenceLocked,setReferenceLocked]=useState(false),[associationLocked,setAssociationLocked]=useState(false),[visibleLocked,setVisibleLocked]=useState(false);
  ...
  const locked=referenceLocked||visibleLocked||associationLocked||recovery.locked||syncRecovery.locked,dirty=!!draft&&structureDraftHash(draft)!==baseline.current;
