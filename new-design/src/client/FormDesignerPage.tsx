...
function starterDefinition(): CardGroupFormDefinition {
  return { primaryTypeKey:"event",groups:[{key:"main",name:"主卡",order:10,sections:[{key:"core",name:"核心信息",order:10,slots:[{key:"primary",name:"主卡",kind:"primary_card",allowedTypeKeys:["event"],min:1,max:1,localFields:[]}]}]}] };
}
...
export default function FormDesignerPage() {
  ...
  return <StructureShell title="卡片组表单" description="把多张卡片、关系和局部上下文编排成可发布、可复用的成熟创作表单。">
    <div className="nd-structure-workspace nd-form-designer-workspace">
      ...
      <section className="nd-structure-editor">{draft?<>
        ...
        <div className="nd-designer-groups">{draft.draftDefinition.groups.map((group,groupIndex)=><section key={`${group.key}-${groupIndex}`} className="nd-designer-group"><div className="nd-section-heading"><div><p className="nd-kicker">分组 {groupIndex+1}</p><input className="nd-inline-title-input" value={group.name} onChange={(event)=>updateDefinition((definition)=>{definition.groups[groupIndex].name=event.target.value;})}/></div></div>{group.sections.map((section,sectionIndex)=><div key={`${section.key}-${sectionIndex}`} className="nd-designer-section"><div className="nd-section-heading"><div><strong>{section.name}</strong><small>{section.slots.length} 个槽位</small></div><button className="nd-button nd-button-secondary" type="button" onClick={()=>addSlot(groupIndex,sectionIndex)}>＋ 添加槽位</button></div>{section.slots.map((slot,slotIndex)=><div className="nd-slot-editor" key={`${slot.key}-${slotIndex}`}><div className="nd-form-grid"><label className="nd-control"><span>槽位名称</span><input value={slot.name} onChange={(event)=>updateDefinition((definition)=>{definition.groups[groupIndex].sections[sectionIndex].slots[slotIndex].name=event.target.value;})}/></label><label className="nd-control"><span>槽位键</span><input value={slot.key} onChange={(event)=>updateDefinition((definition)=>{definition.groups[groupIndex].sections[sectionIndex].slots[slotIndex].key=event.target.value;})}/></label><label className="nd-control"><span>关系类型</span><select disabled={slot.kind==="primary_card"} value={slot.relationTypeKey??""} onChange={(event)=>updateDefinition((definition)=>{const target=definition.groups[groupIndex].sections[sectionIndex].slots[slotIndex];const relation=relations.find((item)=>item.key===event.target.value);target.relationTypeKey=event.target.value;target.allowedTypeKeys=relation?.targetTypeKeys??target.allowedTypeKeys;target.localFields=relation?.propertiesSchema??target.localFields;})}><option value="">主卡无需关系</option>{relations.map((relation)=><option key={relation.id} value={relation.key}>{relation.name}</option>)}</select></label><label className="nd-control"><span>允许类型</span><input value={slot.allowedTypeKeys.join(", ")} onChange={(event)=>updateDefinition((definition)=>{definition.groups[groupIndex].sections[sectionIndex].slots[slotIndex].allowedTypeKeys=event.target.value.split(",").map((value)=>value.trim()).filter(Boolean);})}/></label><label className="nd-control"><span>最少</span><input type="number" min="0" value={slot.min} onChange={(event)=>updateDefinition((definition)=>{definition.groups[groupIndex].sections[sectionIndex].slots[slotIndex].min=Number(event.target.value);})}/></label><label className="nd-control"><span>最多</span><input type="number" min="1" value={slot.max} onChange={(event)=>updateDefinition((definition)=>{definition.groups[groupIndex].sections[sectionIndex].slots[slotIndex].max=Number(event.target.value);})}/></label></div>{slot.localFields.length?<p className="nd-help-text">挂载局部字段：{slot.localFields.map((field)=>field.name).join("、")}</p>:null}</div>)}</div>)}</section>)}</div>
      ...
      </>:<div className="nd-empty nd-empty-page">选择或创建一个卡片组表单。</div>}</section>
    </div>
  </StructureShell>;
}
