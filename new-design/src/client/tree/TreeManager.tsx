...
export default function TreeManager({nodes,selectedId,onSelect,onAdd,onMove}:Props){
  ...
  const render=(node:TreeSelectorNode,depth:number):React.ReactNode=>{
    ...
    const branch=(children.get(node.id)??[]).filter(item=>visible.has(item.id)),siblings=children.get(node.parentId)??[],siblingIndex=siblings.findIndex(item=>item.id===node.id),open=Boolean(normalized)||expanded.has(node.id),selected=selectedId===node.id;
    return <div className="nd-tree-manager-node" key={node.id}>
      <div className={`nd-tree-manager-row${selected?" is-selected":""}`} style={{paddingLeft:`${.45+depth*.85}rem`}}>
        ...
        <div className="nd-tree-manager-actions">
