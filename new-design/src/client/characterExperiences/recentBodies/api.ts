import type {RecentBodySelection,RecentBodyExperienceRequest,RecentBodyExperiencePreview,RecentBodyExperienceWorkspace,RecentBodyExperienceRecord,RecentBodyExperienceDraft,RecentBodySeriesPreview} from '../../../common/characterExperiences/recentBodies';
...
export function createRecentBodyExperienceApi(request:Request){const e=encodeURIComponent,base=(book:string)=>`/books/${e(book)}/character-experiences/recent-bodies`,post=(input:unknown)=>({method:'POST',body:JSON.stringify(input)});return{
 ...
 seriesPreview:(book:string,input:RecentBodySelection)=>request<RecentBodySeriesPreview>(`${base(book)}/series-preview`,post(input)),
