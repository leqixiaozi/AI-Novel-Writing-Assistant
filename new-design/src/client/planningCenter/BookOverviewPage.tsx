  ...
  return <BookShell book={book} active="overview"><main className="nd-overview-page">
    ...
    <div className="nd-overview-columns">
      <section aria-labelledby="materials-title"><div className="nd-section-heading"><div><p className="nd-kicker">主要资料</p><h2 id="materials-title">按本书内容类型检查</h2></div><a href={`/new-design/books/${bookId}/cards`}>维护本书资料</a></div>
        {overview.materials.length?<div className="nd-readiness-list">{overview.materials.filter(item=>item.activeCount>0).sort((a,b)=>a.categoryName.localeCompare(b.categoryName)||a.typeName.localeCompare(b.typeName)).map(item=><a href={item.sourceRoute} key={item.typeKey}><span className={`nd-status-dot is-${item.state}`} aria-hidden="true"/><div><strong>{item.typeName}</strong><small>{item.categoryName} · {item.activeCount?`${item.activeCount} 条资料`:`暂无资料`}</small></div><span>{item.requiredFieldCount?`${item.filledRequiredFieldCount}/${item.requiredFieldCount} 个必填项`:(item.activeCount?"可用":"待补充")}</span></a>)}{overview.materials.some(item=>!item.activeCount)&&<details className="nd-materials-unbuilt"><summary>未建立的资料类型（{overview.materials.filter(item=>!item.activeCount).length}）</summary>{[...new Set(overview.materials.filter(item=>!item.activeCount).map(item=>item.categoryName))].map(category=><section key={category}><h3>{category}</h3>{overview.materials.filter(item=>!item.activeCount&&item.categoryName===category).map(item=><a href={item.sourceRoute} key={item.typeKey}><strong>{item.typeName}</strong><span>待建立</span></a>)}</section>)}</details>}</div>:<div className="nd-empty-state"><strong>暂无可检查的内容类型</strong><p>从本书设置安装内容类型后，这里会按真实字段和资料汇总。</p></div>}
      </section>
