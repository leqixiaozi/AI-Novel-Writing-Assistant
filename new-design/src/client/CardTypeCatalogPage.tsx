...
export default function CardTypeCatalogPage() {
  ...
  return <StructureShell title="内容类型" description="维护系统、开书模板和书籍可以安装的字段结构与组合能力。">
    <div className="nd-types-workspace"><aside className="nd-type-list-pane"><div className="nd-list-heading"><div><p className="nd-kicker">内容目录</p><strong>{categories.length} 个分类 · {cardTypes.length} 种类型</strong></div><div className="nd-tree-actions"><button title="新建分类" type="button" onClick={()=>setAddingCategory((value)=>!value)}>分</button><button title="新建内容类型" type="button" onClick={()=>{setCreating(true);setSelectedTypeId(null);}}>＋</button></div></div>{addingCategory&&<div className="nd-category-create"><input autoFocus value={categoryName} placeholder="分类名称" onChange={(event)=>setCategoryName(event.target.value)} onKeyDown={(event)=>{if(event.key==="Enter")void addCategory();}}/><button type="button" onClick={()=>void addCategory()}>保存</button></div>}<label className="nd-type-search"><span className="nd-visually-hidden">搜索内容类型</span><input value={query} placeholder="搜索类型或分类" onChange={(event)=>setQuery(event.target.value)}/></label><div className="nd-type-tree">{tree.map((node)=>renderNode(node))}</div>{message&&<p className="nd-message is-error">{message}</p>}</aside><TypeDesigner selected={selected} categories={categories} onSaved={saved}/></div>
  </StructureShell>;
}
