...
export default function CardTypeCatalogPage() {
  ...
  const retryLoad=async()=>{setLoading(true);setMessage("");try{await load();setLoadFailed(false);}catch(error){setLoadFailed(true);setMessage(`读取内容类型目录失败：${error instanceof Error?error.message:"未收到有效回执。"} 当前选择与编辑仍保留，点击“重试读取目录”。`);}finally{setLoading(false);}};
  ...
  const renderNode=(node:CardTypeTreeNode,depth=0):React.ReactNode=>{
    ...
    return <div className="nd-tree-node" key={node.category.id}>
      ...
      {open&&<div className="nd-tree-children">{node.children.map((child)=>renderNode(child,depth+1))}{node.cardTypes.map((item)=>{
        ...
        return <button key={item.id} disabled={editorBusy} type="button" className={`nd-tree-leaf${active?" is-selected":""}`} style={{paddingLeft:`${2+depth*.85}rem`}} onClick={()=>{setCreating(false);setSelectedTypeId(item.id);}} aria-current={active?"page":undefined} title={item.name}>
