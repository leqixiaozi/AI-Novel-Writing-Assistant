...
export default function PromptManagementPage(){
  ...
  const run=async(label:string,work:()=>Promise<void>)=>{
    ...
    try{if(retry)await read();await work();setRetry(null);setIssues({});setMessageKind("success");setMessage(`${label}完成。`);}
    catch(error){setMessageKind("error");setMessage(`${phase.current}失败或待核对：${error instanceof ApiError?error.message:"未收到有效回执。"} 当前页面编辑仍保留。`);if(error instanceof ApiError)setIssues(error.issues);setRetry(error instanceof ApiError&&error.status===422?null:{label:phase.current,work});}
  ...
  const saveComponent=()=>{
    const selectedGroup=groups.find(group=>group.id===categoryId);if(!selectedGroup){setMessageKind("info");setMessage("请选择中文主分类，再保存组件。");return;}
  ...
  const checkServer=async()=>{setBusy(true);try{const next=await api.getPromptCatalog();setCatalog(next);setServerChecked(true);setComparison(next.components.find(card=>card.id===editing?.id)??null);setMessageKind("info");setMessage(mode==="category"?`服务器分类：${next.organization.groups.find(group=>group.id===categoryId)?.name??"尚未确认创建"}。当前名称和上级编辑仍保留；请核对后点击“保留编辑继续”。`:"服务器内容已读取，请对比后选择保留当前编辑继续。");}catch{setMessageKind("error");setMessage("读取服务器结果失败。当前编辑仍保留，请检查运行维护后再次核对。");}finally{setBusy(false);}};
  const useRevision=()=>{if(!comparison)return;setEditing(comparison);setRetry(null);setComparison(null);setDirty(true);setMessageKind("info");setMessage("当前编辑仍保留。请检查差异后保存；保存将替换服务器正文，历史版本仍保留。");};
  ...
  return <ResourceShell active="prompts"><main className="nd-prompt-page">
    ...
    {message&&<section className={messageKind==="error"?"nd-type-recovery":"nd-message"} role={messageKind==="error"?"alert":"status"}><p>{message}</p><div className="nd-action-row">{retry&&<button className="nd-button nd-button-secondary" disabled={busy} onClick={()=>void run(retry.label,retry.work)}>核对并重试{retry.label}</button>}{retry&&<button className="nd-button nd-button-secondary" disabled={busy} onClick={()=>void checkServer()}>查看服务器内容</button>}{retry&&mode==="category"&&<button className="nd-button nd-button-secondary" disabled={busy||!serverChecked} onClick={()=>{setRetry(null);setDirty(true);setMessageKind("info");setMessage("当前编辑仍保留。请核对分类名称与上级，点击保存将以当前编辑更新分类。");}}>保留编辑继续</button>}{messageKind==="error"&&<a className="nd-button nd-button-secondary" href="/new-design/structure/maintenance" target="_blank" rel="noreferrer">打开运行维护</a>}</div></section>}
