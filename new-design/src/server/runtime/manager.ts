...
const RELEASE_GATE_DEBTS=["干净 Windows x64 离线安装与二进制/许可证装配","首次 initdb、随机凭据 ACL、本机监听与端口冲突","AGE/vector/pg_trgm 创建、LOAD 与 001—033 实库迁移","启动、停止、异常退出、重复启动与错误 PID 防误杀","030 runner 租约恢复与停机排空","031 备份恢复及数据库/附件一致性","同主版本与跨主版本升级、失败回滚","覆盖安装和卸载保留数据","长路径、中文用户目录、杀毒与权限限制","磁盘不足、性能与安全验证"];
...
export class PrivateRuntimeManager {
  ...
  async doctor():Promise<PrivateRuntimeDiagnostics>{
    ...
    checks.push(check("runtime.migrations",status.migrationsApplied===33?"passed":"unavailable",status.migrationsApplied===33?"001—033 已登记。":"无法确认 001—033 实库迁移。","数据库 ready 后核对 schema_migrations。"));
