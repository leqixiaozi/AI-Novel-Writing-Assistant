...
export async function verifyRuntimePackage(packageRoot:string):Promise<RuntimePackageManifest>{
  ...
  if(manifest.migrationRange.first!=="001_card_kernel.sql"||manifest.migrationRange.last!=="033_business_form_provenance.sql"||manifest.migrationRange.count!==33||migrations.length!==33||migrations.some((value,index)=>value!==index+1))throw new NewDesignError("私有运行包必须连续且完整包含 001—033 迁移。",503);
