...
export async function verifyRuntimePackage(packageRoot:string):Promise<RuntimePackageManifest>{
  ...
  if(manifest.migrationRange.first!=="001_card_kernel.sql"||manifest.migrationRange.last!=="037_context_binding_assembly_snapshots.sql"||manifest.migrationRange.count!==37||migrations.length!==37||migrations.some((value,index)=>value!==index+1))throw new NewDesignError("私有运行包必须连续且完整包含 001—037 迁移。",503);
