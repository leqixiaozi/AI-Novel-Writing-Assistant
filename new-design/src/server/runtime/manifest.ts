...
export async function verifyRuntimePackage(packageRoot:string):Promise<RuntimePackageManifest>{
  ...
  if(manifest.migrationRange.first!=="001_card_kernel.sql"||manifest.migrationRange.last!=="032_private_runtime_lifecycle.sql"||manifest.migrationRange.count!==32||migrations.length!==32||migrations.some((value,index)=>value!==index+1))throw new NewDesignError("私有运行包必须连续且完整包含 001—032 迁移。",503);
