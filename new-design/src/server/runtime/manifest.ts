...
export async function verifyRuntimePackage(packageRoot:string):Promise<RuntimePackageManifest>{
  ...
  if(manifest.migrationRange.first!=="001_card_kernel.sql"||manifest.migrationRange.last!=="043_multiview_quality_workspace.sql"||manifest.migrationRange.count!==43||migrations.length!==43||migrations.some((value,index)=>value!==index+1))throw new NewDesignError("私有运行包必须连续且完整包含 001—043 迁移。",503);
