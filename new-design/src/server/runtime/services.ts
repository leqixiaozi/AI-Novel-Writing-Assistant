...
import { isDevelopmentDatabaseRuntimeEnabled } from "../database/developmentRuntime";
...
export async function startNewDesignRuntimeServices(handlers:RegisteredBackgroundHandlers):Promise<PrivateRuntimeServices>{
  ...
  if(!development)await manager.setWorkerRuntime("starting");
  try{for(const runner of runners)runner.start();if(!development)await manager.setWorkerRuntime("ready");}
  catch(error){if(!development)await manager.setWorkerRuntime("failed").catch(()=>undefined);throw error;}
  let stopped=false;
  return{consumerKeys:loaded.map(consumerKey),stop:async()=>{if(stopped)return;stopped=true;if(!development)await manager.setWorkerRuntime("draining").catch(()=>undefined);await Promise.allSettled(runners.map(runner=>runner.stop()));await stopNewDesignDatabase();}};
}
