...
const targetSchema=z.object({bookId:z.string().uuid(),cardTypeId:z.string().uuid(),cardId:z.string().uuid().nullable(),typeVersionId:z.string().uuid(),cardRevision:z.number().int().positive().nullable(),formVersionId:z.string().uuid().nullable(),title:z.string().trim().max(160)}).superRefine((target,ctx)=>{if(Boolean(target.cardId)!==Boolean(target.cardRevision))ctx.addIssue({code:"custom",path:["cardRevision"],message:"资料身份与修订号不匹配。"});});
const knowledgeSourceSchema=z.object({assetId:z.string().uuid(),sourceVersionId:z.string().uuid(),parsedVersionId:z.string().uuid(),checksum:z.string().regex(/^[a-f0-9]{64}$/)}).strict();
export const formAiRequestSchema=z.object({target:targetSchema,action:z.enum(FORM_ASSIST_ACTIONS),instruction:z.string().trim().max(2000).default(""),referenceCardIds:z.array(z.string().uuid()).max(20).default([]),referenceKnowledgeSources:z.array(knowledgeSourceSchema).max(20).default([]),values:z.record(z.string(),z.unknown()),tagIds:z.array(z.string().uuid()).max(200).default([]),fieldKeys:z.array(z.string().min(1)).max(300).default([]),idempotencyKey:z.string().trim().min(8).max(160)}).strict();
export const formAiAdoptSchema=z.object({candidateId:z.string().uuid(),fieldKeys:z.array(z.string()).max(300),treeKeys:z.array(z.string()).max(100).default([]),values:z.record(z.string(),z.unknown()),tagIds:z.array(z.string().uuid()).max(200).default([]),title:z.string().trim().max(160).optional(),idempotencyKey:z.string().trim().min(8).max(160)});
...
export function blankFormValue(value:unknown):boolean{return value===null||value===undefined||(typeof value==="string"&&!value.trim())||Array.isArray(value)&&value.length===0;}
...
const fillsOnlyEmpty=(action:string)=>["fill_empty","fill_required","supplement","prepare_all"].includes(action);
export function validateFormCandidate(snapshot:FormAssistSnapshot,candidate:FormAssistCandidate,action:string):Record<string,string>{
  ...
  for(const [key,value] of Object.entries(candidate.values)){if(key==="__title"){if(typeof value!=="string"||!value.trim()||value.trim().length>160||!blankFormValue(snapshot.target.title))issues[key]="名称建议无效或已有人工名称。";continue;}const field=byKey.get(key);if(!field||!formAiFieldVisible(field,snapshot.values)||field.aiSuggestible===false){issues[key]="AI 返回了当前范围以外或不允许 AI 建议的字段。";continue;}const invalid=validateFieldValue({...field,required:false},value);if(invalid)issues[key]=invalid;if(fillsOnlyEmpty(action)&&!blankFormValue(snapshot.values[key]))issues[key]="填写空白项不能修改已有内容。";const tree=snapshot.trees.find(item=>item.kind==="dictionary"&&item.key===key);if(tree&&!tree.rule.aiSuggestible)issues[key]="这个字典字段不允许 AI 建议。";if(tree&&!blankFormValue(value)){const ids=Array.isArray(value)?value:typeof value==="string"?[value]:[],checked=validateTreeSelection(tree.nodes,tree.rule,ids as string[]);if(!checked.valid)issues[key]=`${field.name}：${checked.message}`;}}
...
export function aiFormFields(snapshot:FormAssistSnapshot,action:string,fieldKeys:string[]){
  const fields=action==="check"?[]:snapshot.fields.filter(field=>formAiFieldVisible(field,snapshot.values)&&field.aiSuggestible!==false&&(!fieldKeys.length||fieldKeys.includes(field.key))&&(field.optionSource?.kind!=="dictionary_tree"||field.optionSource.rule.aiSuggestible)&&(!fillsOnlyEmpty(action)||blankFormValue(snapshot.values[field.key]))&&(action!=="fill_required"||field.required)&&(action!=="recommend"||field.optionSource?.kind==="dictionary_tree")).map(field=>{const tree=snapshot.trees.find(item=>item.kind==="dictionary"&&item.key===field.key);const required=["fill_required","prepare_all"].includes(action)&&field.required;return tree?{...field,required,options:tree.nodes.filter(node=>selectableTreeNodeIds(tree.nodes,tree.rule).has(node.id)).map(node=>({value:node.id,label:node.path.join(" / ")||node.name}))}:{...field,required};});
  ...
  if(action!=="check"&&action!=="recommend"&&blankFormValue(snapshot.target.title)&&(!fieldKeys.length||fieldKeys.includes("__title")))fields.push({...synthetic("__title","资料名称：结合本次创作方向推荐简洁中文名称","short_text"),required:["fill_required","prepare_all"].includes(action)});
