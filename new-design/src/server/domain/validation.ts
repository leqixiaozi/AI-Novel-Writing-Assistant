...
export const marketAnalysisInputSchema=z.object({scanRecordId:z.string().uuid(),scanVersionId:z.string().uuid().optional(),requestKey:z.string().uuid().optional(),itemIds:z.array(z.string().uuid()).min(1).max(80),focus:z.string().trim().max(500).default(""),budgetTokens:z.number().int().min(1000).max(12000).default(5000)}).strict();
...
export const initialStateInputSchema=z.object({requestKey:z.string().uuid().optional(),subjectKind:stateSubjectKindSchema,subjectId:z.string().uuid(),stateKey:z.string().trim().min(1).max(100),value:z.unknown(),sourceFactId:z.string().uuid().nullable().optional(),expectedRevision:z.number().int().nonnegative().optional(),actor:z.string().trim().max(160).default("user"),note:z.string().trim().max(500).default("")}).refine((value)=>value.value!==undefined,{path:["value"],message:"请提供初始状态值。"});
...
export const planningAiCandidateSchema=z.object({targetObjectId:z.string().uuid().nullable().optional(),instruction:z.string().trim().max(2000).default(""),createdBy:z.string().trim().max(160).default("user")});
...
const contextSourceSchema=z.enum(["card_version","card_relation","body_version","text_anchor","planning_version","canonical_fact","state_change","story_time","research_version","prompt_component","asset_version"]);
...
const contextSourceTypeSchema=z.enum(["card_version","body_version","canonical_fact","knowledge_state_change","state_change","story_time","story_event_relation","research_document_version","planning_version","research_version","research_pack_version","prompt_component","retrieval_chunk","asset_version"]);
const explicitContextSourceTypeSchema=z.enum(["card_version","body_version","canonical_fact","knowledge_state_change","state_change","story_time","story_event_relation","research_document_version","planning_version","research_version","research_pack_version","asset_version"]);
...
export const qualityIssueStatusSchema=z.enum(["open","acknowledged","deferred","dismissed","fix_proposed","fixed","verified","stale","superseded"]);
...
export const qualityIssueTransitionSchema=z.object({toStatus:z.enum(["open","acknowledged","deferred","dismissed","fix_proposed","stale","superseded"]),expectedRevision:z.number().int().positive(),actorKind:z.enum(["user","system","policy"]),actor:z.string().trim().max(160).default(""),reason:z.string().trim().min(1).max(2000)});
...
const embeddingProfileInputSchema=z.object({profileKey:z.string().trim().regex(/^[a-z][a-z0-9_.-]{1,79}$/),name:z.string().trim().min(1).max(200),purpose:z.enum(["semantic_retrieval","similarity","clustering"]),providerKey:z.string().trim().regex(/^[a-z][a-z0-9_.-]{1,79}$/),modelKey:z.string().trim().min(1).max(160),dimensions:z.number().int().min(1).max(2000),connectionVersionId:z.string().uuid().nullable().optional(),distanceMetric:embeddingMetricSchema,normalize:z.boolean().default(false),chunkerKey:z.string().trim().regex(/^[a-z][a-z0-9_.-]{1,79}$/),chunkerVersion:z.string().trim().min(1).max(80),maxChunkChars:z.number().int().min(128).max(50000),overlapChars:z.number().int().min(0),allowedSourceKinds:z.array(embeddingSourceKindSchema).min(1),createdBy:z.string().trim().max(160).default("")});
const embeddingChunkOverlapRefinement={path:["overlapChars"] as string[],message:"重叠长度必须小于分块长度。"};
export const embeddingProfileCreateSchema=embeddingProfileInputSchema.refine(value=>value.overlapChars<value.maxChunkChars,embeddingChunkOverlapRefinement);
export const embeddingProfileVersionSchema=embeddingProfileInputSchema.omit({profileKey:true,name:true,purpose:true}).extend({expectedRevision:z.number().int().positive()}).refine(value=>value.overlapChars<value.maxChunkChars,embeddingChunkOverlapRefinement);
...
export const outboxTopicSchema=z.enum(["dependency.recompute.requested","asset.derivation.requested","graph.projection.requested","embedding.chunking.requested","embedding.generation.requested","embedding.index.requested","ai.task.requested","backup.requested","publication.export.requested"]);
export const backgroundHandlerKeySchema=z.enum(["dependency.recompute","asset.derive","graph.project","embedding.chunk","embedding.generate","embedding.index","ai.task","backup.run","publication.export"]);
...
export const completionSnapshotCreateSchema=z.object({createdBy:z.string().trim().min(1).max(160)});
export const bookCompletionRecordSchema=z.object({snapshotId:z.string().uuid(),expectedSourceHash:sha256Schema,actor:z.string().trim().min(1).max(160),reason:z.string().trim().min(1).max(2000)});
export const bookCompletionReopenSchema=z.object({actor:z.string().trim().min(1).max(160),reason:z.string().trim().min(1).max(2000)});
export const publicationExportManifestSchema=z.object({completionSnapshotId:z.string().uuid().nullable().optional(),exportMode:z.enum(["formal","review_draft"]),format:z.enum(["markdown","plain_text","docx"]),rangeKind:z.enum(["book","volume","chapters"]),volumeId:z.string().uuid().nullable().optional(),chapterDocumentIds:z.array(z.string().uuid()).max(10000).default([]),includeVersionManifest:z.boolean().default(true),createdBy:z.string().trim().min(1).max(160)}).superRefine((value,ctx)=>{if(value.rangeKind==="volume"&&!value.volumeId)ctx.addIssue({code:"custom",path:["volumeId"],message:"按卷导出必须选择一个卷。"});if(value.rangeKind==="chapters"&&!value.chapterDocumentIds.length)ctx.addIssue({code:"custom",path:["chapterDocumentIds"],message:"按章节导出必须选择至少一个章节。"});if(value.exportMode==="formal"&&!value.completionSnapshotId)ctx.addIssue({code:"custom",path:["completionSnapshotId"],message:"正式导出必须绑定完本检查快照。"});});
export const publicationExportSubmitSchema=z.object({expectedSourceHash:sha256Schema,requestedBy:z.string().trim().min(1).max(160),idempotencyKey:z.string().trim().min(8).max(200)});
export const releaseGateAssessmentSchema=z.object({outcome:z.enum(["unexecuted","passed","failed","blocked"]),evidence:z.string().trim().min(1).max(5000),evidenceRef:z.string().trim().max(1000).default(""),assessedBy:z.string().trim().min(1).max(160)});
