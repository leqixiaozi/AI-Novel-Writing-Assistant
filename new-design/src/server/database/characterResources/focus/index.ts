...
async function readRow(db:PoolClient,bookId:string,id:string){return await readBookGeneration(db,bookId,id,contract);}
...
export async function endUnknownResourceFocus(bookId:string,input:ResourceFocusRequest):Promise<ResourceFocusRecord>{
 ...
 try{
  ...
  const ended=await updateGenerationBatch(db,parsed.requestKey,{status:'discarded',stage:'ended_unknown',error_message:'原显示建议占用已明确结束；原模型是否完成及用量仍未知，完整原来源保留。',completed_at:new Date().toISOString()});await db.query('COMMIT');return record(ended);
...
export async function generateResourceFocus(bookId:string,input:ResourceFocusRequest,ai?:NewDesignAiGateway):Promise<ResourceFocusRecord>{
 ...
 try{
  ...
  await createBookGeneration(db,bookId,{id:parsed.requestKey,instruction:parsed.instruction,input_payload:{contract,request:parsed,requestHash:formHash(parsed),snapshot:source.snapshot},prompt_id:prompt.assetId,prompt_version:prompt.version});
 ...
 try{
  ...
  const row=await finishBookGeneration(execution,bookId,parsed.requestKey,contract,{status:'review',stage:'review',progress:100,output_payload:{...generated,result:output},completed_at:new Date().toISOString()});
  ...
  if(!modelCompleted&&attempts!==null&&attempts.every(attempt=>!attempt.requestSent||attempt.responseReceived)){
   const failed=await finishBookGeneration(execution,bookId,parsed.requestKey,contract,{status:'failed',stage:'failed',error_message:error instanceof AiExecutionError?error.message:'原回复未通过资源建议规格。',output_payload:{failureSnapshot:trace},completed_at:new Date().toISOString()}).catch(()=>null);if(failed)return record(failed);
  }
  await finishBookGeneration(execution,bookId,parsed.requestKey,contract,{stage:modelCompleted?'result_pending':'result_unknown',error_message:'原资源建议结果待核对；完整来源保留，不能重复生成。'}).catch(()=>undefined);
