...
async function freezeResourceBackfillSources(db:PoolClient,bookId:string,bodyVersionId:string,bodyContent:string,scope:ResourceBackfillScope):Promise<ResourceBackfillFrozenScope>{
 ...
 const rows=(await db.query("SELECT id,subject_card_id,start_offset,end_offset,excerpt FROM new_design.text_anchors WHERE book_id=$1 AND body_version_id=$2 AND status='active' AND subject_card_id=ANY($3::uuid[]) ORDER BY start_offset,id",[bookId,bodyVersionId,[scope.characterId,...scope.resourceIds]])).rows;
