...
export async function readCharacterResourceHistoryInTransaction(db:PoolClient,bookId:string,characterId:string,selection:ResourceLedgerSelection):Promise<CharacterResourceHistory>{
 ...
 for(const row of rows.slice(0,200)){
  ...
  const source:ResourceHistoryChange={id:String(change.id),subjectKind:change.subject_kind,subjectId:String(change.subject_id),stateKey:String(change.state_key),before:change.before_json,after:change.after_json,reason:String(change.reason),fieldLabel:field?.label??String(change.state_key),fieldAvailable:Boolean(field),beforeDisplay:field?displaySettlementValue(field.field,change.before_json,field.dictionaryNodes):rawDisplay(change.before_json),afterDisplay:field?displaySettlementValue(field.field,change.after_json,field.dictionaryNodes):rawDisplay(change.after_json),checkpointId,chapterDocumentId:String(change.chapter_document_id),bodyVersionId:String(change.body_version_id),chapterOrder:Number(row.logical_order),available:!reason,unavailableReason:reason,original:{change:json(change)!,relationVersion:json(relationVersion),resourceVersion:json(resourceVersion),characterVersion:json(row.character_version),anchor:json(anchor),editingContract:json(contract)},sourceRoute:`/new-design/books/${bookId}/writing?chapterDocument=${change.chapter_document_id}${row.session_id?`&session=${row.session_id}`:''}`};
