...
export async function captureManagedModelSnapshot(taskType: ModelTaskKey, route: ManagedTaskRoute, context?: ManagedDatabaseContext, scope?: {bookId:string;taskContractVersionId:string}): Promise<ManagedModelSnapshot> {
  ...
  return withClient(context, async client => {
    if(scope){
      ...
      const composition=taskType!=="chapter_settlement"&&taskType!=="chapter_generation"&&taskType!=="world_consistency"&&taskType!=="creative_extraction"&&taskType!=="character_dialogue"&&contract?.task_key===`prompt_composition_${contract.recipe_id}_${taskType}`;
      if(!book||!contract||!(settlement||chapter||world||creative||dialogue||composition))throw new NewDesignError("模型快照必须引用真实书籍与本任务精确受控合同版本。",422);
      if(dialogue){
