...
export async function freezeWorldPackage(db:PoolClient,input:WorldPackageInput,lock=false,sourceSpaceId:string=PUBLIC_WORLD_SPACE_ID){
 ...
 const cards:FrozenWorldCard[]=[],types:WorldPackageFrame['types']=[],forms:WorldPackageFrame['forms']=[],dictionaries:FrozenWorldDictionary[]=[],dictionaryIds=new Set<string>();
 for(const ref of [...input.cards].sort((a,b)=>a.cardId.localeCompare(b.cardId))){
  ...
  const locals=(await db.query('SELECT definition.id definition_id,definition.field_key,field_version.id version_id,field_version.field_schema,local.value FROM new_design.card_version_local_values local JOIN new_design.field_definitions definition ON definition.id=local.field_definition_id AND definition.card_id=$2 JOIN new_design.field_definition_versions field_version ON field_version.id=local.field_definition_version_id AND field_version.field_definition_id=definition.id WHERE local.card_version_id=$1 ORDER BY definition.field_key'+(lock?' FOR SHARE OF definition,field_version,local':''),[ref.versionId,ref.cardId])).rows;
  ...
  if(row.form_version_id&&!forms.some(form=>form.id===row.form_version_id)){
   const form=assertFound((await db.query('SELECT version.id,version.form_id,version.definition,form.space_id FROM new_design.card_group_form_versions version JOIN new_design.card_group_forms form ON form.id=version.form_id WHERE version.id=$1'+(lock?' FOR SHARE OF version,form':''),[row.form_version_id])).rows[0],'公共来源的原表单版本不存在。');
 ...
 for(const id of [...dictionaryIds].sort()){
  const dictionary=assertFound((await db.query("SELECT to_jsonb(dictionary) definition FROM new_design.dictionary_definitions dictionary WHERE id=$1 AND status='published' AND (owner_space_id IS NULL OR owner_space_id=$2)"+(lock?' FOR SHARE':''),[id,sourceSpaceId])).rows[0],'世界字典不可用或属于其他空间。');
  const nodes=(await db.query('SELECT item.id,version.id version_id,to_jsonb(version.*) snapshot FROM new_design.dictionary_items item JOIN new_design.dictionary_item_versions version ON version.id=item.current_version_id AND version.item_id=item.id WHERE item.dictionary_id=$1 ORDER BY item.id LIMIT 1001'+(lock?' FOR SHARE OF item,version':''),[id])).rows;
