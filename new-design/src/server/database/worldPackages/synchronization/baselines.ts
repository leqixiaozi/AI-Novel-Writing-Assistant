...
export async function saveSyncBaselines(db:PoolClient,commandId:string,installationId:string,input:WorldSyncInput,published:PublishedWorldPackage,workspace:WorldSyncWorkspace,added:WorldSyncReceipt['addedCards'],relations:WorldSyncReceipt['relationVersions']){
 ...
 const localRows=await listRecordCards(db,'card_version_local_value'),definitions=(await db.query('SELECT id,field_key FROM new_design.field_definitions')).rows,keys=new Map(definitions.map(row=>[String(row.id),String(row.field_key)])),fieldBaselines=[];
 for(const choice of fields){
  ...
  const local=(await db.query('SELECT current_version_id,values FROM new_design.cards WHERE id=$1',[mapping.targetCardId])).rows[0],extra=Object.fromEntries(localRows.filter(row=>row.card_version_id===String(local.current_version_id)).map(row=>[keys.get(String(row.field_definition_id))??String(row.field_definition_id),row.value]));
  const publicVersion=source?.versionId??fallback?.sourceVersionId;
  ...
  fieldBaselines.push({command_id:commandId,installation_id:installationId,source_card_id:choice.sourceCardId,source_key:choice.sourceKey,target_key:choice.targetKey,public_version_id:versionId,local_version_id:String(local.current_version_id),public_value:worldValue(source?{...source.values,...source.localValues}:{},choice.sourceKey),local_value:worldValue({...local.values,...extra},choice.targetKey)});
 }
 const relationBaselines=[];
 for(const choice of input.relationChoices??[]){
  ...
  relationBaselines.push({command_id:commandId,installation_id:installationId,source_relation_id:choice.sourceRelationId,target_relation_id:targetId,target_type_id:String(local.relation_type_id),public_version_id:source?.versionId??null,local_version_id:String(local.id),public_value:publicRelationValue(source),local_value:value});
 }
 return{fieldBaselines,relationBaselines};
}
