...
async function portraitContent(client:PoolClient,row:TrialRow,reply:ImageGenerationReply):Promise<PublicPortraitOutput>{
 ...
 if(content&&(content.storage_provider!=='local'||content.storage_locator!==file.locator||content.mime_type!==file.mimeType))throw new NewDesignError('相同图片内容已有不同受控引用，原回复保留，请核对内容存储。',409);
 if(!content)content=(await client.query(`INSERT INTO new_design.asset_content_objects(id,checksum,byte_size,mime_type,storage_kind,storage_provider,storage_locator,integrity_state,last_verified_at,created_by) VALUES($1,$2,$3,$4,'managed_file','local',$5,'verified',now(),'public_characters') ON CONFLICT(checksum_algorithm,checksum,byte_size) DO NOTHING RETURNING *`,[randomUUID(),file.checksum,file.byteSize,file.mimeType,file.locator])).rows[0]??(await client.query('SELECT * FROM new_design.asset_content_objects WHERE checksum=$1 AND byte_size=$2',[file.checksum,file.byteSize])).rows[0];
 if(!content||content.storage_provider!=='local'||content.storage_locator!==file.locator||content.mime_type!==file.mimeType)throw new NewDesignError('公共角色图片受控存储引用未确认，原回复保留。',409);
...
export async function completePublicCharacterTrial(id:string){
 const initial=await getPublicCharacterTrial(uuid.parse(id));return transaction(initial.requestKey,async client=>{
  ...
  await finish(client,row,'succeeded',output,'固定角色版本的试用结果已保存；档案、书籍及正式事实保持独立。');return receipt({...row,status:'succeeded',output,summary:'试用结果已保存，供作者审阅。'});
 });
}
...
export async function executePublicPortraitCommand(raw:unknown){const input=publicPortraitCommandSchema.parse(raw);return transaction(input.requestKey,async(client,markAbsent)=>{
 ...
 const id=randomUUID();await client.query('INSERT INTO new_design.public_character_portrait_events(id,request_key,request_hash,resource_id,trial_id,operation,input_payload) VALUES($1,$2,$3,$4,$5,$6,$7::jsonb)',[id,input.requestKey,stableHash(input),input.resourceId,input.trialId,input.operation,JSON.stringify(input)]);return{id,input,repeated:false};
 });}
