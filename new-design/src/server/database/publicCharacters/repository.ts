...
export async function finish(client:PoolClient,row:TrialRow,status:'succeeded'|'failed'|'ended_unknown',output:PublicCharacterTrial['output'],summary:string){
 ...
 const attemptChange=await client.query('UPDATE new_design.ai_task_attempts SET status=$2,ended_at=now(),error_summary=$3,error_category=CASE WHEN $2=\'failed\' THEN \'unknown\' ELSE NULL END,retry_eligibility=CASE WHEN $2=\'failed\' THEN \'none\' ELSE NULL END WHERE id=$1 AND status=\'running\'',[row.attempt_id,status==='ended_unknown'?'discarded':status,summary]);if(attemptChange.rowCount!==1)throw new NewDesignError('原试用尝试状态已变化，保留原结果核对。',409);
 ...
 await client.query('UPDATE new_design.public_character_trials SET status=$2,output=$3::jsonb,summary=$4 WHERE id=$1',[row.id,status,output===null?null:JSON.stringify(output),summary]);
}
