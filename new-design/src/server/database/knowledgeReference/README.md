...
原文件像收到的快递，解析正文像拆箱后可阅读的说明书。对应技术上，上传成功只登记 pending 解析请求；作者点击解析，校验原文件 SHA-256/字节数/UTF-8，才记录真实派生版本和 succeeded。速记：“收到不等于拆好，解析不等于向量索引”。当前仅关键词检索；无受控嵌入执行器，向量能力明确不可用，模型设置入口为 `/new-design/structure/models`。
