...
export async function freezeWorldConsistency(client:PoolClient,bookId:string,requestId:string,catalog:WorldConsistencyCatalog,recheck:WorldFrozenPlan["recheck"]):Promise<WorldFrozenPlan>{
 ...
 await client.query("INSERT INTO new_design.ai_contract_publications(id,entity_kind,entity_id,to_version_id,entity_revision,action,actor,idempotency_key) VALUES($1,'prompt_recipe',$2,$3,2,'publish','world_consistency',$4)",[randomUUID(),recipeId,recipeVersionId,`${key}_recipe`]);
 ...
 await client.query("INSERT INTO new_design.ai_contract_publications(id,entity_kind,entity_id,to_version_id,entity_revision,action,actor,idempotency_key) VALUES($1,'task_contract',$2,$3,2,'publish','world_consistency',$4)",[randomUUID(),contractId,contractVersionId,`${key}_contract`]);
 ...
 const slotId=randomUUID();await client.query("INSERT INTO new_design.context_manifest_slots(id,manifest_id,slot_key,sort_order,required,token_budget) VALUES($1,$2,'world_material_evidence',0,true,NULL)",[slotId,manifestId]);
