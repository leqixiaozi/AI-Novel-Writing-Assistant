...
function receipt(row:Record<string,any>|null|undefined,inputHash:string):VisibleBatchWriteReceipt|null{
 if(!row)return null;
...
export async function readVisibleBatchWrite(bookId:string,batchKey:string,input:Input):Promise<VisibleBatchWriteReceipt|null>{
 ...
 try{await db.query('BEGIN READ ONLY');await lock(db,bookId,batchKey,parsed.requestKey);
  const row=await readBookGeneration(db,bookId,batchKey,'story_workspace_ai_v1')?await readBatchDecision(db,batchKey,parsed.requestKey):null;
...
export async function writeVisibleBatch(bookId:string,batchKey:string,input:Input):Promise<VisibleBatchWriteReceipt>{
 ...
 try{
  ...
  const prior=await readBatchDecision(db,batchKey,parsed.requestKey),original=receipt(prior,hash(bookId,batchKey,parsed));
  ...
  for(const item of parsed.items){
   ...
   const existing=await readBatchDecision(db,batchKey,item.requestKey);
   ...
   if(!existing)await saveBatchDecision(db,batchKey,{id:decisionId,candidate_id:item.slotId,decision:'adopt',selected_field_keys:item.fieldKeys,draft_snapshot:selected,request_hash:formHash({bookId,batchKey,input:adoption}),source_hash:item.sourceHash,idempotency_key:item.requestKey});
  ...
  await saveBatchDecision(db,batchKey,{decision:'adopt',draft_snapshot:result,request_hash:result.inputHash,source_hash:formHash(parsed.items.map(item=>[item.slotId,item.sourceHash])),idempotency_key:parsed.requestKey});
