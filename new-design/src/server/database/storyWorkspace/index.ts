...
export async function validateStoryBatchSlot(db:PoolClient,record:StoryBatchRecord,slotId:string):Promise<StoryBatchDraft>{
 ...
 if(record.snapshot.worldUsage)await assertWorldUsageScopesCurrent(db,bookId,record.snapshot.worldUsage);
  ...
  }else if((await db.query("SELECT 1 FROM new_design.planning_objects WHERE book_id=$1 AND level='story' AND status='active'",[bookId])).rowCount)throw new NewDesignError('本书已有故事总览，请选择该规划后另行准备。',409);
...
export async function endUnknownStoryBatch(bookId:string,key:string):Promise<StoryBatchRecord>{
 ...
 try{
  ...
  const row=(await db.query("SELECT * FROM new_design.ai_generation_batches WHERE id=$1 AND book_id=$2 AND input_payload->>'contract'=$3 FOR UPDATE",[key,bookId,contract])).rows[0];
  ...
  const ended=(await db.query("UPDATE new_design.ai_generation_batches SET status='discarded',stage='ended_unknown',error_message='原请求已结束占用；模型结果与实际用量仍未确认。已有资料与原请求保留。',updated_at=now(),completed_at=now() WHERE id=$1 RETURNING *",[key])).rows[0];
...
export async function generateStoryBatch(bookId:string,request:StoryBatchRequest,ai?:NewDesignAiGateway):Promise<StoryBatchRecord>{
 ...
 try{
  ...
  const current=(await executionDb.query("SELECT * FROM new_design.ai_generation_batches WHERE id=$1 AND book_id=$2 AND input_payload->>'contract'=$3",[request.requestKey,bookId,contract])).rows[0];
  ...
  if(confirmedEnd){
   const row=(await executionDb.query("UPDATE new_design.ai_generation_batches SET status='failed',stage='failed',error_message=$2,output_payload=$3::jsonb,updated_at=now(),completed_at=now() WHERE id=$1 AND book_id=$4 AND status='running' RETURNING *",[request.requestKey,error instanceof AiExecutionError?error.message:'本次模型回复未通过候选规格，请检查来源后另行准备。',JSON.stringify({failureSnapshot:trace}),bookId]).catch(()=>({rows:[]}))).rows[0];
  ...
  await executionDb.query("UPDATE new_design.ai_generation_batches SET stage=$2,error_message=$3,updated_at=now() WHERE id=$1 AND book_id=$4 AND status='running'",[request.requestKey,modelCompleted?'result_pending':'result_unknown','候选结果待核对，请保留原请求，不重复准备。',bookId]).catch(()=>undefined);
