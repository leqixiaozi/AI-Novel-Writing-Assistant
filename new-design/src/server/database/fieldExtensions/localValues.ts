...
export async function readCardLocalSnapshot(db:RecordCardDb,cardVersionId:string):Promise<Array<{field_key:string;value:unknown}>>{
  ...
  return rows.flatMap(row=>{const definition=definitions.find(item=>item.id===row.field_definition_id);return definition?[{field_key:String(definition.field_key),value:row.value}]:[];});
}
