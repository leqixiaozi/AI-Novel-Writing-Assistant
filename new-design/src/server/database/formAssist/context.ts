...
export async function freezeFormContext(db:Queryable,target:FormAssistTarget,values:Record<string,unknown>,tagIds:string[],referenceCardIds:string[]=[],referenceKnowledgeSources:KnowledgeSourceSelection[]=[]):Promise<FormAssistSnapshot> {
  ...
  for(const field of fields){const source=field.optionSource;if(source?.kind!=="dictionary_tree")continue;
    ...
    const itemRows=(await listRecordCards(db,"dictionary_item",{includeArchived:true})).filter(item=>String(item.dictionary_id)===String(dictionary.id)),itemVersions=await listRecordCards(db,"dictionary_item_version",{includeArchived:true}),nodes=itemRows.map(item=>({...item,path_labels:itemVersions.find(version=>String(version.id)===String(item.current_version_id))?.path_labels})).sort((a,b)=>String(a.id).localeCompare(String(b.id)));
  ...
  const bindingRows=(await listRecordCards(db,"card_type_tag_binding",{includeArchived:true})).filter(binding=>String(binding.card_type_id)===target.cardTypeId&&binding.status==='active'),dimensions=await listRecordCards(db,"material_tag_dimension",{includeArchived:true}),bindings=bindingRows.map(binding=>({...binding,...dimensions.find(dimension=>String(dimension.id)===String(binding.dimension_id)),binding_revision:binding.revision,config:binding.config})).filter(binding=>binding.status==='active'&&(!binding.owner_space_id||String(binding.owner_space_id)===String(book.space_id))).sort((a,b)=>String(a.id).localeCompare(String(b.id)));
  for(const binding of bindings){const tags=await listRecordCards(db,"material_tag",{includeArchived:true,where:{dimension_id:binding.dimension_id}}),tagVersions=await listRecordCards(db,"material_tag_version",{includeArchived:true}),nodes=tags.flatMap(tag=>{const version=tagVersions.find(item=>item.id===tag.current_version_id);return version?[{...tag,name:version.name,path_names:version.path_names}]:[];}).sort((a,b)=>a.id.localeCompare(b.id));
