...
export async function freezeFormKnowledge(db:Pick<PoolClient,"query">,bookId:string,sources:KnowledgeSourceSelection[]):Promise<NonNullable<FormAssistSnapshot["knowledgeReferences"]>>{
 ...
 for(const source of sources){
  ...
  for(const id of [...new Set([source.assetId,String(parsed.asset_id)])].sort()){
   const asset=await requireRecordCard(db,id,'asset','知识参考资产不存在。',{lock:true});
   if(asset.book_id!==bookId)throw new NewDesignError('知识参考资产不属于本书。',404);
  }
