...
角色专用适配页复用本模块原资源空间、卡片版本和不可变回执；原五类策略目录仍保持。创建要求明确的当前发布规格版本，编辑／归档要求确切原版本和修订。原角色局部字段按原定义版本完整复制，包含归档定义与0值；原表单来源也保留。角色仅允许创建、编辑、收藏和归档，不允许策略安装、反馈或通用试验。入书仍使用原严格映射预览导入器。

手动096只扩展原保护函数，保留函数OID／触发器和原五类约束；能力默认关闭，不纳入普通启动迁移。只读能力检测核对实际原触发器及扩展合同，不能仅凭存在的能力行伪称可操作。关闭后原键回执和相同完整输入重放仍返回已提交结果。096未安装作者库，相关隔离PG与编辑刷新竞态用例未执行，不声称功能通过。

固定版本试聊与图片由 [publicCharacters](../publicCharacters/README.md) 单独编排原任务、回执和内容对象，不属于策略通用试验或角色编辑命令。手动097独立默认关闭，096管理启用不代表试用已启用，关闭后已保存原回执仍可核对。
