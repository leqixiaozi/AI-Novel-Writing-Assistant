...
export async function executeAssociationWrite(bookId:string,targetId:string,operation:string,input:{idempotencyKey:string},write:(client:PoolClient)=>Promise<AssociationWorkspace>):Promise<AssociationWorkspace>{
  const saved=await executeBookFormWrite<AssociationReceipt>(bookId,associationReceiptKey(input.idempotencyKey),{command:'association_write',bookId,targetId,operation,input},async client=>{
    ...
    if((await client.query("SELECT 1 FROM new_design.association_actions WHERE idempotency_key=$1",[input.idempotencyKey])).rows[0])throw new StructureWriteError('form','save','原关联请求缺少完整回执，请保留原凭证核对，不能用当前关联替代当次保存结果。',409,'unknown');
    const workspace=await write(client),row=assertFound((await client.query("SELECT form.*,version.version FROM new_design.card_group_forms form JOIN new_design.card_group_form_versions version ON version.id=form.current_version_id WHERE form.id=$1",[workspace.formId])).rows[0],'无法核对关联表单来源。');
    return{id:String(row.id),key:String(row.form_key),name:String(row.name),description:String(row.description??''),status:row.status,revision:Number(row.revision),currentVersion:Number(row.version),currentVersionId:String(row.current_version_id),draftDefinition:row.draft_definition,isSystem:Boolean(row.is_system),createdAt:new Date(row.created_at).toISOString(),updatedAt:new Date(row.updated_at).toISOString(),associationResult:{bookId,targetId,operation,workspace}};
  });
