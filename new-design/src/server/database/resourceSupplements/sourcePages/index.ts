...
export function getResourceSupplementSource(book:string,session:string){
 uuid.parse(book);uuid.parse(session);return read(async client=>{
  const row=(await client.query(`SELECT session.*,body.content_hash body_hash FROM new_design.chapter_adoption_sessions session
...
export function getResourceSupplementChapterBasis(book:string,document:string){
 uuid.parse(book);uuid.parse(document);return read(async client=>{
  const rows=(await client.query(`SELECT checkpoint.id FROM new_design.chapter_stable_checkpoints checkpoint
...
export function listResourceSupplementIssues(book:string,character:string):Promise<ResourceSupplementIssue[]>{
 uuid.parse(book);uuid.parse(character);return read(async client=>{
  ...
  if(!(await client.query("SELECT id FROM new_design.schema_migrations WHERE id='090_resource_supplement_integrity'")).rowCount)return [];
  const rows=(await client.query(`SELECT issue.issue_id,issue.chapter_document_id,issue.body_version_id,document.title,issue.state_key,issue.source_route
   ...
   JOIN new_design.resource_supplement_integrity_journals journal ON journal.settlement_id=issue.settlement_id AND journal.book_id=issue.book_id
   JOIN new_design.chapter_resource_supplements origin ON origin.session_id=(journal.merged_write->>'sessionId')::uuid AND origin.book_id=issue.book_id
   ...
   WHERE issue.book_id=$1 AND origin.full_input#>>'{resourceScope,characterId}'=$2
    AND NOT EXISTS(SELECT 1 FROM new_design.resource_supplement_integrity_resolutions WHERE issue_id=issue.issue_id)
...
export function getResourceSupplementIssueSource(book:string,issue:string):Promise<ResourceSupplementIssueSource>{
 uuid.parse(book);uuid.parse(issue);return read(async client=>{
  if(!(await client.query("SELECT id FROM new_design.schema_migrations WHERE id='090_resource_supplement_integrity'")).rowCount)throw new NewDesignError('资源冲突来源未能读取，请保留原来源页核对。',503);
  const row=(await client.query(`SELECT to_jsonb(issue) issue,origin.full_input->'resourceScope' resource_scope FROM new_design.resource_supplement_integrity_issues issue
   JOIN new_design.resource_supplement_integrity_journals journal ON journal.settlement_id=issue.settlement_id AND journal.book_id=issue.book_id
   JOIN new_design.chapter_resource_supplements origin ON origin.session_id=(journal.merged_write->>'sessionId')::uuid AND origin.book_id=issue.book_id
