import {runRows,readRunPreview,insertRunRecord,transitionRunPreview} from "./records";
import {listRecordCards} from "../recordCards";
import {readRecipeSlots,readRecipeComponents} from "../aiContracts/records";
...
async function mapPreview(row:Row,reader?:Pick<PoolClient,'query'>):Promise<AiRunPreview>{
 ...
 const sections=(await runRows(pool,"ai_run_prompt_section",{preview_id:row.id})).sort((a,b)=>Number(a.sort_order)-Number(b.sort_order)||a.id.localeCompare(b.id)).map(mapSection);
 const submission=(await runRows(pool,"ai_run_submission",{preview_id:row.id}))[0];
...
async function f5Blockers(bookId:string,safeCheckpoint:Record<string,unknown>):Promise<string[]>{
  const documentId=typeof safeCheckpoint.chapterDocumentId==="string"?safeCheckpoint.chapterDocumentId:null;if(!documentId)return[];
  const pool=await getNewDesignPool(),rows=(await listRecordCards(pool,"chapter_revision_review_flag",{where:{book_id:bookId,target_chapter_document_id:documentId}})).filter(row=>["pending_review","in_review"].includes(row.status)).sort((a,b)=>iso(b.updated_at).localeCompare(iso(a.updated_at))).slice(0,20),
    running=(await listRecordCards(pool,"chapter_revision_execution",{where:{book_id:bookId,chapter_document_id:documentId}})).filter(row=>!["stable","cancelled"].includes(row.status)).sort((a,b)=>iso(b.updated_at).localeCompare(iso(a.updated_at)));
  const blockers=rows.map(row=>`旧章换稿待复核：${String(row.reason)}`);if(running[0])blockers.unshift(`旧章换稿处理处于 ${String(running[0].status)} 状态。`);return blockers;
}
...
async function buildPromptSections(taskContractVersionId:string,contextPreviewId:string):Promise<Array<Omit<AiRunPromptSection,"id">>>{
 const pool=await getNewDesignPool(),task=assertFound((await pool.query("SELECT output_schema_version,output_schema,prompt_recipe_version_id FROM new_design.task_contract_versions WHERE id=$1",[taskContractVersionId])).rows[0],"任务合同版本不存在。");
 const slots=await readRecipeSlots(pool,String(task.prompt_recipe_version_id)),components=await readRecipeComponents(pool,String(task.prompt_recipe_version_id)),recipeRows:{rows:Row[]}={rows:[]};
 for(const slot of slots){const bound=components.filter(row=>row.slot_id===slot.id);if(!bound.length)recipeRows.rows.push({slot_key:slot.slot_key});for(const component of bound){const version=(await pool.query("SELECT values FROM new_design.card_versions WHERE id=$1",[component.component_version_id])).rows[0];recipeRows.rows.push({slot_key:slot.slot_key,component_card_id:component.component_card_id,component_version_id:component.component_version_id,component_type:version?.values.component_type,component_key:version?.values.component_key});}}
 const contextRows={rows:(await listRecordCards(pool,"context_preview_decision",{where:{preview_id:contextPreviewId,decision:"included"}})).sort((a,b)=>Number(a.sort_order)-Number(b.sort_order))};
...
export async function createAiRunPreview(input:CreateAiRunPreviewInput):Promise<AiRunPreview>{
  ...
  try{await client.query("BEGIN");await client.query("SELECT pg_advisory_xact_lock(hashtextextended($1,0))",[`ai-run-preview:${book.space_id}:${input.idempotencyKey}`]);
const duplicate=(await runRows(client,"ai_run_preview",{space_id:book.space_id,idempotency_key:input.idempotencyKey}))[0];
if(duplicate){if(duplicate.request_hash!==requestHash)throw new NewDesignError("幂等键对应另一份运行预览请求。",409);const receipt=await mapPreview(duplicate,client);await client.query("COMMIT");return receipt;}
const row=await insertRunRecord(client,"ai_run_preview",{id,space_id:book.space_id,book_id:input.bookId,task_key:contract.task_key,task_group:contract.task_group,task_node_key:input.taskNodeKey,source_route:input.sourceRoute,source_kind:input.sourceKind,source_id:input.sourceId??null,task_contract_version_id:input.taskContractVersionId,prompt_recipe_version_id:contract.prompt_recipe_version_id,context_preview_id:contextPreview.id,context_manifest_id:manifest.id,model_route_snapshot_id:route.id,input_snapshot:input.inputSnapshot,input_hash:inputHash,safe_checkpoint:input.safeCheckpoint,budget_snapshot:budgetSnapshot,prompt_plan:promptPlan,route_plan:routePlan,blocker_snapshot:blockers,preview_hash:previewHash,status,idempotency_key:input.idempotencyKey,request_hash:requestHash,created_by:input.createdBy??"user"});for(const section of sections)await insertRunRecord(client,"ai_run_prompt_section",{id:randomUUID(),preview_id:id,slot_key:section.slotKey,section_kind:section.sectionKind,label:section.label,source_refs:section.sourceRefs,token_estimate:section.tokenEstimate,sort_order:section.sortOrder,content_hash:section.contentHash});const receipt=await mapPreview(row,client);await client.query("COMMIT");return receipt;}catch(error){await client.query("ROLLBACK");if((error as {code?:string}).code==="23505"){const repeated=(await runRows(pool,"ai_run_preview",{space_id:book.space_id,idempotency_key:input.idempotencyKey}))[0];if(repeated&&String(repeated.request_hash)===requestHash)return mapPreview(repeated);}throw error;}finally{client.release();}
}

export async function getAiRunPreview(id:string,bookId?:string):Promise<AiRunPreview>{const row=await readRunPreview(await getNewDesignPool(),id);if(bookId&&row.book_id!==bookId)throw new NewDesignError("运行预览不存在。",404);return mapPreview(row);}
export async function listAiRunPreviews(bookId:string,limit=30):Promise<AiRunPreview[]>{const rows=(await runRows(await getNewDesignPool(),"ai_run_preview",{book_id:bookId})).sort((a,b)=>iso(b.created_at).localeCompare(iso(a.created_at))||b.id.localeCompare(a.id)).slice(0,Math.min(100,limit)),result:AiRunPreview[]=[];for(const row of rows)result.push(await mapPreview(row));return result;}
...
async function readByRequest(bookId:string,key:string,operation:'preview'|'submission'):Promise<AiRunPreview|null> {
 ...
 const client=await(await getNewDesignPool()).connect();try {
  ...
  const rows:Row[]=[];
  if(operation==='preview')rows.push(...await runRows(client,'ai_run_preview',{book_id:bookId,idempotency_key:key}));
  else for(const submission of await runRows(client,'ai_run_submission',{idempotency_key:key})){const row=await readRunPreview(client,String(submission.preview_id));if(row.book_id===bookId)rows.push(row);}
...
export async function submitAiRunPreview(id:string,input:{expectedRevision:number;idempotencyKey:string;submittedBy?:string}):Promise<AiRunPreview>{
 const pool=await getNewDesignPool(),repeated=(await runRows(pool,"ai_run_submission",{idempotency_key:input.idempotencyKey}))[0];
 ...
 const client=await pool.connect();try{
  ...
  const prior=(await runRows(client,'ai_run_submission',{idempotency_key:input.idempotencyKey}))[0];
  ...
  const head=assertFound(await readRunPreview(client,id,true),'原预览不存在。');
  ...
  await transitionRunPreview(client,id,"submitted",input.expectedRevision);
  await insertRunRecord(client,'ai_run_submission',{id:randomUUID(),preview_id:id,ai_task_id:task.id,submitted_revision:input.expectedRevision,idempotency_key:input.idempotencyKey,submitted_by:input.submittedBy??'user'});
