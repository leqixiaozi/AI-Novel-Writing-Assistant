import {settlementRecordCtes,updateSettlementRecords,insertSettlementRecords,lockedSettlementQuery} from '../recordStorage';
...
async function mutate(bookId:string,key:string,input:unknown,operation:"save_draft"|"publish",action:(client:PoolClient,book:Row)=>Promise<{draftId:string;versionId:string;relationTypeId:string|null}>):Promise<SettlementRelationConfigurationReceipt>{
 ...
 try{await client.query("BEGIN");await lockKey(client,bookId,key);const hash=stableHash({bookId,operation,input});
  const prior=(await client.query(`WITH ${settlementRecordCtes.settlement_relation_configuration_receipts}
SELECT input_hash,receipt FROM settlement_relation_configuration_receipts WHERE book_id=$1 AND request_key=$2`,[bookId,key])).rows[0];
  ...
  await insertSettlementRecords(client,"settlement_relation_configuration_receipt",`SELECT ($1)::uuid AS id,($2)::uuid AS book_id,($3)::text AS request_key,($4)::text AS input_hash,($5::jsonb)::jsonb AS receipt`,[randomUUID(),bookId,key,hash,JSON.stringify(receipt)]);
...
export async function saveSettlementRelationConfigurationDraft(bookId:string,raw:SettlementRelationDraftInput):Promise<SettlementRelationConfigurationReceipt>{
 ...
 return mutate(bookId,input.requestKey,input,"save_draft",async(client,book)=>{
  ...
  if(input.draftId){const row=(await lockedSettlementQuery(client,`WITH ${settlementRecordCtes.settlement_relation_configuration_drafts}
SELECT * FROM settlement_relation_configuration_drafts WHERE id=$1 AND book_id=$2`,[draftId,bookId])).rows[0];if(!row||Number(row.revision)!==input.expectedRevision)relationError(bookId,"草稿已更新，请先核对保存回执。",409);version=Number((await client.query(`WITH ${settlementRecordCtes.settlement_relation_configuration_versions}
SELECT MAX(version) version FROM settlement_relation_configuration_versions WHERE draft_id=$1`,[draftId])).rows[0].version)+1;
  ...
  }else await insertSettlementRecords(client,"settlement_relation_configuration_draft",`SELECT ($1)::uuid AS id,($2)::uuid AS book_id,(1)::integer AS revision,('draft')::text AS status`,[draftId,bookId]);
  const versionId=randomUUID();await insertSettlementRecords(client,"settlement_relation_configuration_version",`SELECT ($1)::uuid AS id,($2)::uuid AS draft_id,($3)::integer AS version,($4::jsonb)::jsonb AS definition,($5)::uuid AS relation_type_id,($6)::uuid AS source_relation_type_id,($7)::integer AS expected_relation_type_revision`,[versionId,draftId,version,JSON.stringify(input.definition),input.relationTypeId??null,input.sourceRelationTypeId??null,input.expectedRelationTypeRevision]);
  await updateSettlementRecords(client,"settlement_relation_configuration_draft",`WITH ${settlementRecordCtes.settlement_relation_configuration_drafts}
SELECT settlement_relation_configuration_drafts.*,($2)::uuid AS current_version_id,($3)::uuid AS relation_type_id,($4)::uuid AS source_relation_type_id,($5)::integer AS expected_relation_type_revision,('draft')::text AS status,(revision+$6)::integer AS revision,(now())::timestamptz AS updated_at FROM settlement_relation_configuration_drafts WHERE id=$1`,[draftId,versionId,input.relationTypeId??null,input.sourceRelationTypeId??null,input.expectedRelationTypeRevision,input.draftId?1:0]);
...
async function validateExistingValues(client:PoolClient,bookId:string,spaceId:string,definition:SettlementRelationDefinition,ids:string[]):Promise<void>{
 ...
 const rows=(await client.query(`WITH ${settlementRecordCtes.current_state_projections}
SELECT * FROM current_state_projections WHERE book_id=$1 AND subject_kind='relation' AND subject_id=ANY($2::uuid[]) ORDER BY subject_id,state_key`,[bookId,ids])).rows;
...
export async function publishSettlementRelationConfigurationDraft(bookId:string,raw:SettlementRelationPublishInput):Promise<SettlementRelationConfigurationReceipt>{
 ...
 return mutate(bookId,input.requestKey,input,"publish",async(client,book)=>{
  ...
  const space=String(book.space_id);const draft=(await lockedSettlementQuery(client,`WITH ${settlementRecordCtes.settlement_relation_configuration_drafts},
${settlementRecordCtes.settlement_relation_configuration_versions}
SELECT draft.*,version.definition,version.id version_id FROM settlement_relation_configuration_drafts draft JOIN settlement_relation_configuration_versions version ON version.id=draft.current_version_id AND version.draft_id=draft.id WHERE draft.id=$1 AND draft.book_id=$2`,[input.draftId,bookId])).rows[0];
  ...
  await insertSettlementRecords(client,"state_relation_capability",`SELECT ($1)::uuid AS space_id,($2)::text AS relation_key,($3)::text AS settlement_capability,($4)::text AS state_mode`,[space,relationKey,definition.capability,definition.mode],{keys:["space_id","relation_key"],update:(previous,incoming)=>({settlement_capability:incoming.settlement_capability,state_mode:incoming.state_mode,revision:Number(previous.revision)+1,updated_at:new Date().toISOString()})});
  await updateSettlementRecords(client,"state_relation_dimension",`WITH ${settlementRecordCtes.state_relation_dimensions}
SELECT state_relation_dimensions.*,('derived')::text AS settlement_policy,('derived')::text AS state_mode,(revision+1)::integer AS revision,(now())::timestamptz AS updated_at FROM state_relation_dimensions WHERE space_id=$1 AND relation_key=$2 AND NOT(dimension_key=ANY($3::text[]))`,[space,relationKey,definition.dimensions.map(d=>d.fieldKey)]);
  const inherited=(await lockedSettlementQuery(client,`WITH ${settlementRecordCtes.state_relation_dimensions}
SELECT * FROM state_relation_dimensions WHERE space_id='00000000-0000-4000-8000-000000000001' AND relation_key=$1 AND NOT(dimension_key=ANY($2::text[]))`,[relationKey,definition.dimensions.map(d=>d.fieldKey)])).rows;
  for(const retired of inherited)await insertSettlementRecords(client,"state_relation_dimension",`SELECT ($1)::uuid AS space_id,($2)::text AS relation_key,($3)::text AS dimension_key,($4)::text AS label,($5)::text AS direction,('derived')::text AS settlement_policy,('derived')::text AS state_mode`,[space,relationKey,retired.dimension_key,retired.label,retired.direction],{keys:["space_id","relation_key","dimension_key"],update:(previous,incoming)=>({settlement_policy:'derived',state_mode:'derived',revision:Number(previous.revision)+1,updated_at:new Date().toISOString()})});
  for(const dimension of definition.dimensions)await insertSettlementRecords(client,"state_relation_dimension",`SELECT ($1)::uuid AS space_id,($2)::text AS relation_key,($3)::text AS dimension_key,($4)::text AS label,($5)::text AS direction,($6)::text AS settlement_policy,($7)::text AS state_mode`,[space,relationKey,dimension.fieldKey,dimension.label,dimension.direction,dimension.policy,dimension.mode],{keys:["space_id","relation_key","dimension_key"],update:(previous,incoming)=>({label:incoming.label,direction:incoming.direction,settlement_policy:incoming.settlement_policy,state_mode:incoming.state_mode,revision:Number(previous.revision)+1,updated_at:new Date().toISOString()})});
  ...
  await updateSettlementRecords(client,"settlement_relation_configuration_draft",`WITH ${settlementRecordCtes.settlement_relation_configuration_drafts}
SELECT settlement_relation_configuration_drafts.*,($2)::uuid AS relation_type_id,(NULL)::uuid AS source_relation_type_id,($3)::integer AS expected_relation_type_revision,('published')::text AS status,(revision+1)::integer AS revision,(now())::timestamptz AS updated_at FROM settlement_relation_configuration_drafts WHERE id=$1`,[input.draftId,relationId,target?Number(target.revision)+1:1]);
...
export async function readSettlementRelationConfigurationReceipt(bookId:string,key:string):Promise<SettlementRelationConfigurationReceipt|null>{const client=await (await getNewDesignPool()).connect();try{await client.query("BEGIN");await lockKey(client,bookId,key);await readBook(client,bookId);const row=(await client.query(`WITH ${settlementRecordCtes.settlement_relation_configuration_receipts}
SELECT receipt FROM settlement_relation_configuration_receipts WHERE book_id=$1 AND request_key=$2`,[bookId,key])).rows[0];await client.query("COMMIT");return row?row.receipt as SettlementRelationConfigurationReceipt:null;}catch(error){await client.query("ROLLBACK");throw error;}finally{client.release();}}
