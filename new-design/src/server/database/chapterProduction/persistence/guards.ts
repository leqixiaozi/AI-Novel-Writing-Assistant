...
export async function guardProductionRecord(client:PoolClient,typeKey:string,next:Values,current?:Values):Promise<void>{
  ...
  if(typeKey==='chapter_writing_request'){
    ...
    if(next.input_body_version_id){body=assertFound((await client.query('SELECT * FROM new_design.chapter_body_versions WHERE id=$1 AND chapter_document_id=$2',[next.input_body_version_id,next.chapter_document_id])).rows[0],'章节冻结输入正文不存在。');if(body.content_hash!==next.input_body_hash)reject('章节冻结输入正文哈希不匹配。');}
