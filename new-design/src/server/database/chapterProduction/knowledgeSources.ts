import {assetRows} from './persistence';
...
export async function readChapterKnowledgeSources(client:PoolClient,bookId:string,selection:ChapterKnowledgeSelection[]=[]){
  ...
  for(const raw of selection){
    ...
    const parsed=assertFound((await client.query(`SELECT version.asset_id FROM new_design.asset_versions version JOIN ${assetRows} asset ON asset.id=version.asset_id AND asset.book_id=$1 WHERE version.id=$2 AND version.book_id=$1`,[bookId,source.parsedVersionId])).rows[0],'选择的知识解析版本不属于本书，请返回知识参考核对。');
