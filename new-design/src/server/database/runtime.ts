...
export async function getPrivateRuntimeDiagnostics():Promise<PrivateRuntimeDiagnostics>{
  ...
  const applied=Number(migrationsResult.rows[0]?.value??0);replace("runtime.migrations",applied===migrations.length?"passed":"failed",`已登记 ${applied}/${migrations.length} 个迁移。`,"按顺序完成 001—034，不允许跳号。");
