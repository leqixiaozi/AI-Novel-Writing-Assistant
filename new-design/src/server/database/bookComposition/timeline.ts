...
async function readSource(client:PoolClient,bookId:string,lock=false):Promise<AuthorTimelineWorkspace>{
 ...
 const cards=(await client.query(`SELECT card.id,card.current_version_id,card.title,card.updated_at,type.type_key,type.name AS type_name FROM new_design.cards card JOIN new_design.card_types type ON type.id=card.card_type_id WHERE card.space_id=$1 AND card.status='active' AND card.current_version_id IS NOT NULL ORDER BY card.id ${lock?"FOR SHARE OF card":""}`,[book.space_id])).rows;
 const plans=(await client.query(`SELECT id,parent_object_id,card_id,status,revision,current_version_id,adopted_version_id FROM new_design.planning_objects WHERE book_id=$1 ORDER BY id ${lock?"FOR SHARE":""}`,[bookId])).rows;
 ...
 const timeRows=(await client.query(`SELECT id FROM new_design.story_time_proposals WHERE book_id=$1 ORDER BY id ${lock?"FOR UPDATE":""}`,[bookId])).rows,relationRows=(await client.query(`SELECT id FROM new_design.story_relation_proposals WHERE book_id=$1 ORDER BY id ${lock?"FOR UPDATE":""}`,[bookId])).rows;
 if(lock)await client.query("SELECT id FROM new_design.story_event_narrative_occurrences WHERE book_id=$1 ORDER BY id FOR UPDATE",[bookId]);
...
async function preview(client:PoolClient,bookId:string,input:AuthorTimelinePreviewInput,source:AuthorTimelineWorkspace):Promise<AuthorTimelinePreview>{
 ...
 if(events.length){const affected=(await client.query("SELECT DISTINCT object.title FROM new_design.planning_objects object JOIN new_design.planning_version_references reference ON reference.planning_object_id=object.id AND reference.planning_version_id IN (object.current_version_id,object.adopted_version_id) WHERE object.book_id=$1 AND object.status='active' AND reference.reference_role='event' AND reference.card_id=ANY($2::uuid[]) ORDER BY object.title",[bookId,events])).rows;changes.push({label:"当前引用该事件的规划范围",before:affected.map(row=>String(row.title)).join("、")||"暂无已保存的规划引用",after:review?"正式安排改变后请在这些原规划中逐项复核，不自动改计划或正文。":"保存此提案或位置不自动改动上述原规划。"});}
 const warnings=["不修改章节正文，不自动采用正文，也不结算对象或关系状态。",review?"确认会改变正式故事时间或事件关系；依赖该安排的后续计划需要作者复核。":"时间与关系先保存提案，必须再明确审核后才生效；叙述位置保存为原事件的新引用。",...(input.command.operation==="occurrence_create"?["这是新叙述位置，已有章节中的第一次出现保持不变。"]:[])];
