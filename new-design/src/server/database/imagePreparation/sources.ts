import {cardVersionLocalValueRows,planningObjectRows,planningVersionRows} from './records';
...
export async function readImageMaterials(client:PoolClient,scope:ImageMaterialScope):Promise<ImageMaterialSource>{
 ...
 if(scope.kind==='public_character')data={profile:await readSource(client,scope.resourceId,scope.resourceVersionId)};
  ...
  const cards=(await client.query(`SELECT card.id,version.id "versionId",version.title,version.type_version_id "typeVersionId",version.values,spec.fields,
   COALESCE((SELECT jsonb_agg(jsonb_build_object('definitionId',definition.id,'versionId',local_spec.id,'field',local_spec.field_schema,'value',local.value) ORDER BY definition.field_key) FROM ${cardVersionLocalValueRows} local JOIN new_design.field_definitions definition ON definition.id=local.field_definition_id JOIN new_design.field_definition_versions local_spec ON local_spec.id=local.field_definition_version_id AND local_spec.field_definition_id=definition.id WHERE local.card_version_id=version.id),'[]'::jsonb) "localFields"
   FROM new_design.cards card JOIN new_design.card_types type ON type.id=card.card_type_id AND NOT type.is_internal JOIN new_design.card_versions version ON version.id=card.current_version_id AND version.card_id=card.id JOIN new_design.card_type_versions spec ON spec.id=version.type_version_id WHERE card.space_id=$1 AND card.status='active' ORDER BY card.id LIMIT 1001`,[book.space_id])).rows;
  const plans=(await client.query(`SELECT object.id,version.id "versionId",object.level,object.title,version.content,version.content_hash "contentHash" FROM ${planningObjectRows} object JOIN ${planningVersionRows} version ON version.id=object.adopted_version_id AND version.object_id=object.id WHERE object.book_id=$1 AND object.status='active' AND object.level='story' AND version.status='adopted' AND version.stale_at IS NULL ORDER BY object.id`,[scope.bookId])).rows;
