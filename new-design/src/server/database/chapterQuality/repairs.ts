...
async function source(client:PoolClient,bookId:string,candidateId:string){return assertFound((await client.query(`SELECT candidate.*,version.target_chapter_document_id,version.target_body_version_id,version.patch,issue.current_status issue_status,issue.current_version_id issue_version_id,issue.report_id,report.stale_at
 FROM new_design.quality_fix_candidates candidate JOIN new_design.quality_fix_candidate_versions version ON version.id=candidate.current_version_id JOIN new_design.quality_issues issue ON issue.id=candidate.issue_id JOIN new_design.quality_audit_reports report ON report.id=issue.report_id JOIN new_design.chapter_quality_requests receipt ON receipt.report_id=report.id AND receipt.book_id=$1
...
export async function recordChapterQualityRepair(bookId:string,candidateId:string,input:{candidateVersionId:string;chapterWritingRequestId:string;requestKey:string}){return qualityTransaction(bookId,async client=>{
 ...
 const request=assertFound((await client.query("SELECT * FROM new_design.chapter_writing_requests WHERE id=$1 AND book_id=$2 AND status='succeeded' AND controlled_snapshot IS NOT NULL",[input.chapterWritingRequestId,bookId])).rows[0],'原修复正文候选尚未成功保存。');
 ...
 const existing=(await client.query('SELECT * FROM new_design.quality_fix_adoptions WHERE idempotency_key=$1 OR candidate_id=$2',[input.requestKey,candidateId])).rows[0];
 ...
 const originalReceipt=assertFound((await client.query('SELECT frozen_snapshot,input_payload FROM new_design.chapter_quality_requests WHERE report_id=$1 AND book_id=$2',[candidate.report_id,bookId])).rows[0],'原诊断快照不存在。'),original=originalReceipt.frozen_snapshot.input;
 ...
 for(const plan of original.plans){if(!(await client.query("SELECT 1 FROM new_design.planning_objects object JOIN new_design.planning_versions version ON version.id=object.adopted_version_id WHERE object.id=$1 AND object.book_id=$2 AND object.adopted_version_id=$3 AND object.status='active' AND version.stale_at IS NULL",[plan.objectId,bookId,plan.versionId])).rowCount)throw new NewDesignError('原诊断规划已变化，请对新正式范围重新诊断，不将旧修复标记解决。',409);}
