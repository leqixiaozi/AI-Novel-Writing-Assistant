import {productionDirectorRunRows,qualityAuditReportRows,qualityIssueRows} from '../chapterProduction/persistence';
...
const BOOK_QUERY=`SELECT book.id,book.name,book.revision,book.status,book.updated_at,
 ...
 (SELECT count(*) FROM ${productionDirectorRunRows} production_director_run_rows_record WHERE book_id=book.id) directors_total,
 (SELECT count(*) FROM ${productionDirectorRunRows} production_director_run_rows_record WHERE book_id=book.id AND status='running') directors_running,
 (SELECT count(*) FROM ${productionDirectorRunRows} production_director_run_rows_record WHERE book_id=book.id AND status='paused') directors_paused,
 (SELECT count(*) FROM ${productionDirectorRunRows} production_director_run_rows_record WHERE book_id=book.id AND status='waiting_recovery') directors_recovery,
 (SELECT count(*) FROM ${qualityIssueRows} quality_issue_rows_record WHERE book_id=book.id) quality_total,
 (SELECT count(*) FROM ${qualityIssueRows} quality_issue_rows_record WHERE book_id=book.id AND current_status IN ('open','acknowledged','deferred','fix_proposed')) quality_unresolved,
 (SELECT count(*) FROM ${qualityIssueRows} quality_issue_rows_record WHERE book_id=book.id AND is_quality_debt=true AND current_status IN ('open','acknowledged','deferred','fix_proposed')) quality_debt,
 (SELECT count(*) FROM ${qualityIssueRows} issue JOIN ${qualityAuditReportRows} report ON report.id=issue.report_id WHERE issue.book_id=book.id AND (issue.current_status='stale' OR report.stale_at IS NOT NULL)) quality_stale
