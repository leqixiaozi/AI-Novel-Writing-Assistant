...
export interface TemplatePayload { cardTypes:PayloadType[];dictionaries:PayloadDictionary[];tagDimensions?:PayloadTagDimension[];tagBindings?:PayloadTagBinding[];relationTypes:PayloadRelation[];forms:PayloadForm[];seedCards:PayloadCard[];viewConfigs?:PayloadViewConfig[];menu:{defaultPage:string;pages:string[]}; }
...
export async function previewBookSync(bookId:string,targetVersionId:string):Promise<TemplateSyncPreview>{
  ...
  const types=await pool.query("SELECT type.type_key,version.fields FROM new_design.card_types type JOIN new_design.card_type_versions version ON version.id=type.current_version_id WHERE type.space_id=$1",[book.space_id]);
...
async function applyTreeAdditions(client:PoolClient,spaceId:string,treeAdditions:NonNullable<TemplateSyncPreview["treeAdditions"]>):Promise<void>{
  ...
  for(const dimension of treeAdditions.tagDimensions){
    ...
    for(const node of dimension.nodes){const nodeId=tagIds.get(node.sourceId)!;if((await client.query("SELECT 1 FROM new_design.material_tags WHERE id=$1",[nodeId])).rowCount)continue;await client.query("INSERT INTO new_design.material_tags(id,space_id,tag_key,dimension_id,sort_order,visibility,source_tag_id,created_by,updated_by) VALUES($1,$2,$3,$4,$5,'space',$6,'template_sync','template_sync')",[nodeId,spaceId,node.key,target.id,node.sortOrder,node.sourceId]);}
    for(const node of dimension.nodes)if(node.parentSourceId)await client.query("UPDATE new_design.material_tags SET parent_id=$2 WHERE id=$1",[tagIds.get(node.sourceId),tagIds.get(node.parentSourceId)]);
...
export async function applyBookSync(syncId:string):Promise<TemplateSyncPreview>{
  ...
  try{
    ...
    for(const addition of sync.additions as TemplateSyncPreview["additions"]){
      ...
      const mappedFields=await remapTreeFieldsForBook(client,String(book.space_id),addition.fields),fields=[...(type.draft_fields as FieldDefinition[]),...mappedFields];
    ...
    await client.query("UPDATE new_design.books SET template_version_id=$2,installed_payload=$3::jsonb,revision=revision+1,updated_at=now() WHERE id=$1",[book.id,target.id,JSON.stringify(target.payload)]);
    await client.query("UPDATE new_design.book_template_syncs SET status='applied',applied_at=now() WHERE id=$1",[syncId]);
    await client.query("COMMIT");
    return{id:syncId,bookId:String(sync.book_id),fromTemplateVersionId:String(sync.from_template_version_id),toTemplateVersionId:String(sync.to_template_version_id),additions:sync.additions as TemplateSyncPreview["additions"],treeAdditions,conflicts:sync.conflicts as TemplateSyncPreview["conflicts"],status:"applied"};
