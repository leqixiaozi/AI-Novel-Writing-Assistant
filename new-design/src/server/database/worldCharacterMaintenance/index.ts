...
async function readObjects(client:PoolClient,spaceId:string,focusCardId?:string):Promise<{objects:ProfessionalObject[];truncated:boolean}>{
 ...
 const local=(await client.query(`SELECT local.card_version_id,definition.field_key,local.value FROM new_design.card_version_local_values local
...
export async function getWorldCharacterMaintenanceWorkspace(bookId:string,input:WorldCharacterQuery):Promise<WorldCharacterMaintenanceWorkspace>{
 ...
 try{
  ...
  const capabilities=(await client.query("SELECT * FROM new_design.state_relation_capabilities WHERE space_id=ANY($1::uuid[]) ORDER BY CASE WHEN space_id=$2 THEN 1 ELSE 0 END",[["00000000-0000-4000-8000-000000000001",book.spaceId],book.spaceId])).rows as Row[];
  const dimensions=(await client.query("SELECT * FROM new_design.state_relation_dimensions WHERE space_id=ANY($1::uuid[]) ORDER BY CASE WHEN space_id=$2 THEN 1 ELSE 0 END",[["00000000-0000-4000-8000-000000000001",book.spaceId],book.spaceId])).rows as Row[];
