...
export async function readProfessionalState(client:PoolClient,bookId:string,objects:ProfessionalObject[],relations:Row[],objectLabels:Map<string,string>,dictionaryLabels:Map<string,string>):Promise<{items:ProfessionalState[];truncated:boolean}>{
 ...
 FROM new_design.current_state_projections projection
 LEFT JOIN new_design.entity_initial_state_versions initial_version ON initial_version.id=projection.source_initial_version_id
 LEFT JOIN new_design.entity_initial_states initial ON initial.id=initial_version.initial_state_id
 LEFT JOIN new_design.state_changes change ON change.id=projection.source_state_change_id
...
export async function readProfessionalKnowledge(client:PoolClient,bookId:string,objects:ProfessionalObject[],objectLabels:Map<string,string>,dictionaryLabels:Map<string,string>):Promise<{items:ProfessionalKnowledge[];truncated:boolean}>{
 const rows=(await client.query(`SELECT projection.*,claim.subject_card_id,claim.predicate,claim.value_kind,claim.value_json,claim.object_card_id,claim.truth_fact_id,
 ...
 FROM new_design.current_knowledge_state_projections projection JOIN new_design.epistemic_claims claim ON claim.id=projection.claim_id AND claim.book_id=projection.book_id
 LEFT JOIN new_design.knowledge_state_changes change ON change.id=projection.source_change_id
 LEFT JOIN new_design.knowledge_state_proposals proposal ON proposal.id=change.proposal_id AND proposal.book_id=projection.book_id
 LEFT JOIN new_design.knowledge_state_proposal_versions version ON version.id=change.proposal_version_id AND version.proposal_id=proposal.id
 ...
 LEFT JOIN new_design.chapter_text_anchors anchor ON anchor.id=version.text_anchor_id
 LEFT JOIN new_design.canonical_facts fact ON fact.id=claim.truth_fact_id AND fact.book_id=projection.book_id
 LEFT JOIN new_design.chapter_settlement_items item ON item.knowledge_proposal_id=proposal.id
 LEFT JOIN new_design.chapter_adoption_sessions session ON session.id=item.session_id AND session.book_id=projection.book_id AND session.chapter_document_id=document.id AND session.body_version_id=body.id
 WHERE projection.book_id=$1 ORDER BY projection.holder_kind,projection.holder_key,projection.claim_id LIMIT 201`,[bookId])).rows as Row[];
...
export async function readProfessionalMilestones(client:PoolClient,bookId:string):Promise<{items:ProfessionalMilestone[];truncated:boolean}>{
 const rows=(await client.query(`SELECT milestone.*,session.id session_id,
 ...
 FROM new_design.state_milestone_snapshots milestone LEFT JOIN new_design.chapter_documents document ON document.id=milestone.chapter_document_id
 ...
 LEFT JOIN new_design.chapter_adoption_sessions session ON session.settlement_id=settlement.id AND session.book_id=milestone.book_id AND session.chapter_document_id=document.id AND session.body_version_id=body.id
...
export async function readProfessionalIssues(client:PoolClient,bookId:string,objects:ProfessionalObject[]):Promise<{items:ProfessionalConsistencyIssue[];truncated:boolean}>{
 const rows=(await client.query(`SELECT conflict.id,conflict.reason,a.subject_card_id subject_a,b.subject_card_id subject_b FROM new_design.canonical_fact_conflicts conflict
 JOIN new_design.canonical_facts a ON a.id=conflict.fact_a_id AND a.book_id=conflict.book_id JOIN new_design.canonical_facts b ON b.id=conflict.fact_b_id AND b.book_id=conflict.book_id
