import {runRows,readRunPreview,insertRunRecord} from "../aiRunOrchestration/records";
...
export async function readPreviewRow(client:PoolClient,id:string):Promise<Row>{
  const row=await readRunPreview(client,id);if(row.source_kind!==DEBUG_KIND)throw new NewDesignError("组合预览不存在，请重新读取组合后生成预览。",404);const submission=(await runRows(client,"ai_run_submission",{preview_id:id}))[0];return{...row,ai_task_id:submission?.ai_task_id??null};
}
...
export async function readDebugPreviewByRequest(key:string,context?:CompositionDatabaseContext):Promise<CompositionDebugPreview|null>{
  ...
  return database(context,async client=>{await lock(client,`preview-key:${key}`);const rows=await runRows(client,"ai_run_preview",{idempotency_key:key,source_kind:DEBUG_KIND});if(rows.length>1)throw new NewDesignError("组合预览请求标识出现多条结果，请到运行维护核对，不要再次试运行。",409);return rows[0]?previewFromRow(await readPreviewRow(client,rows[0].id),(await runRows(client,"ai_run_submission",{preview_id:rows[0].id}))[0]?.ai_task_id??null):null;},true);
}
...
async function freezeTaskContract(client:PoolClient,bundle:DebugPreviewBundle):Promise<string>{
  ...
  await insertContractRecord(client,"ai_contract_publication",{id:randomUUID(),entity_kind:"task_contract",entity_id:config.id,from_version_id:config.published_version_id,to_version_id:id,entity_revision:revision,action:"publish",actor:"prompt_composition",idempotency_key:`composition_contract_${id}`});
  return id;
}
async function freezeContext(client:PoolClient,bundle:DebugPreviewBundle,contractId:string,snapshotId:string|null):Promise<string>{
  ...
  for(const [index,key]of ["author_additions","explicit_context"].entries()){
    const slotId=randomUUID();await insertContractRecord(client,"context_manifest_slot",{id:slotId,manifest_id:id,slot_key:key,sort_order:index,required:false,token_budget:null});
    ...
    for(const [order,item]of selected.entries()){
      ...
      await client.query("INSERT INTO new_design.context_manifest_items(id,manifest_id,slot_id,source_type,stable_object_id,exact_version_id,source_space_id,content_hash,inclusion_reason,priority,token_estimate,transform_status,sort_order,source_revision,content_role,layer_label,knowledge_segment) VALUES($1,$2,$3,$4,$5,$6,$7,$8,'作者明确选定的精确版本，不提升指令信任',0,$9,'full',$10,$11,$12,$13,$14::jsonb)",[randomUUID(),id,slotId,item.reference.kind,item.reference.cardId,item.reference.versionId,item.spaceId,item.hash,Math.ceil(JSON.stringify(item.reference).length/4),order,item.revision,item.reference.role==="formal"?"required":"reference",key==="author_additions"?"普通组件补充":"明确参考资料",segment?JSON.stringify(segment):null]);
    }
  }
  for(const source of bundle.knowledgeSources??[]){
    if(!source.segment)continue;
    const resource=assertFound((await client.query("SELECT resource.id,resource.space_id FROM new_design.dependency_resources resource WHERE resource.resource_kind='embedding_chunk' AND resource.book_id=$1 AND resource.stable_object_id=$2 AND resource.exact_version_id=$2",[bundle.recipe.context.bookId,source.segment.chunkId])).rows[0],"选中的原知识分块缺少来源记录，不能冻结为可执行预览。");
    const states=await listRecordCards(client,"dependency_resource_state",{where:{resource_id:resource.id,book_id:bundle.recipe.context.bookId},lock:true});
    if(states.length!==1||!["fresh","recomputed"].includes(String(states[0].state)))throw new NewDesignError("选中的原知识分块已失效或缺少有效状态，不能冻结为可执行预览。",409);
...
export async function saveDebugPreview(bundle:DebugPreviewBundle,input:DebugPreviewInput,context?:CompositionDatabaseContext):Promise<CompositionDebugPreview>{
  ...
  return database(context,async client=>{
    ...
    const receipts=await runRows(client,"ai_run_preview",{idempotency_key:input.idempotencyKey,source_kind:DEBUG_KIND});
    if(receipts.length){if(receipts.length!==1||receipts[0].request_hash!==requestHash)throw new NewDesignError("同一预览请求包含不同输入，请核对服务器结果，不要重复提交。",409);return previewFromRow(receipts[0],(await runRows(client,"ai_run_submission",{preview_id:receipts[0].id}))[0]?.ai_task_id??null);}
    ...
    const row=await insertRunRecord(client,"ai_run_preview",{id,space_id:book.space_id,book_id:book.id,task_key:`prompt_composition_${bundle.recipe.id}_${bundle.recipe.taskType}`,task_group:"prompt_composition_debug",task_node_key:"prompt_composition_debug",source_route:COMPOSITION_ROUTE,source_kind:DEBUG_KIND,source_id:bundle.recipe.id,task_contract_version_id:taskContractVersionId,prompt_recipe_version_id:bundle.recipe.versionId,context_preview_id:null,context_manifest_id:contextManifestId,model_route_snapshot_id:snapshotId,input_snapshot:bundle.taskInput,input_hash:inputHash,safe_checkpoint:{trialOnly:true},budget_snapshot:route?.policy??{maxOutputTokens:bundle.maxTokens},prompt_plan:plan,route_plan:route??{},blocker_snapshot:blockers,preview_hash:hash,status:route?"ready":"blocked",idempotency_key:input.idempotencyKey,request_hash:requestHash,created_by:"prompt_composition"});
    for(const [index,message]of bundle.messages.entries())await insertRunRecord(client,"ai_run_prompt_section",{id:randomUUID(),preview_id:id,slot_key:message.role==="system"?"controlled_asset":"author_additions",section_kind:message.role==="system"?"instruction":"reference",label:message.role==="system"?"受控系统合同":"用户输入与普通组件补充",source_refs:message.role==="system"?[{assetId:bundle.assetId,version:bundle.assetVersion}]:bundle.components.map(({cardId,versionId,enabled})=>({cardId,versionId,enabled})),token_estimate:Math.ceil(message.content.length/4),sort_order:index,content_hash:stableHash(message.content)});
