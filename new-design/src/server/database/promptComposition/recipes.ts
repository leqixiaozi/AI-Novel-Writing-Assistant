...
import {readRecipeSlots,readRecipeComponents,readContractPublication,insertContractRecord} from '../aiContracts/records';
...
import {loadCompositionKnowledge,readCompositionKnowledgeCandidates,compositionContextTypes} from "./knowledge";
...
async function recipeVersion(client:PoolClient,recipe:Row,version:Row):Promise<CompositionRecipe>{
  const metadata=version.variables_schema?.[marker],bindingRows=await readRecipeComponents(client,version.id);
...
export async function loadCompositionRecipeVersion(id:string,versionId:string,context?:CompositionDatabaseContext):Promise<LoadedCompositionRecipe>{
  return database(context,async client=>{
    ...
    const knowledgeSources=await loadCompositionKnowledge(client,loaded.summary);
    return {recipe:loaded.summary,...exact,...(knowledgeSources.length?{knowledgeSources}:{})};
  });
}
...
export async function getCompositionCatalog():Promise<CompositionCatalog>{
  const [prompts,books,types,recipes,rankingSnapshots]=await Promise.all([
    ...
    database(undefined,async client=>{const snapshots=(await listRecordCards(client,'market_source_snapshot',{where:{status:'succeeded'}})).sort((a,b)=>String(b.captured_at).localeCompare(String(a.captured_at))||a.id.localeCompare(b.id)),items:Array<{id:string;label:string}>=[];for(const row of snapshots){if((await listRecordCards(client,'market_ranking_item',{where:{snapshot_id:row.id}})).length)items.push({id:row.id,label:`${row.platform} · ${row.list_label} · ${iso(row.captured_at).slice(0,10)}`});if(items.length===100)break;}return items;}),
  ]);
...
export async function readCompositionSaveByRequest(key:string,context?:CompositionDatabaseContext):Promise<SaveCompositionResult|null>{
  return database(context,async client=>{await lock(client,`save-key:${key}`);const receipt=await readContractPublication(client,key);if(!receipt||receipt.entity_kind!=='prompt_recipe'||receipt.actor!=='prompt_composition')return null;const {recipe,version,summary}=await loadVersion(client,receipt.entity_id,receipt.to_version_id);return {recipe:summary,savedVersionId:version.id,savedVersion:Number(version.version),active:recipe.status==="active"&&recipe.published_version_id===version.id,repeated:true};},true);
}
export async function saveComposition(value:SaveCompositionInput,context?:CompositionDatabaseContext):Promise<SaveCompositionResult>{
  ...
  return database(context,async client=>{
    ...
    await loadCompositionKnowledge(client,input);
    ...
    const slotId=randomUUID();await insertContractRecord(client,'prompt_recipe_slot',{id:slotId,recipe_version_id:versionId,slot_key:'author_additions',sort_order:0,required:false,allowed_content_types:['prompt_component'],variable_contract:{required:input.variables.map(item=>item.key)}});
    await insertContractRecord(client,'prompt_recipe_slot',{recipe_version_id:versionId,slot_key:'explicit_context',sort_order:1,required:false,allowed_content_types:compositionContextTypes(input),variable_contract:{}});
    for(const [index,binding]of input.components.entries())await insertContractRecord(client,'prompt_recipe_slot_component',{recipe_version_id:versionId,slot_id:slotId,component_card_id:binding.cardId,component_version_id:binding.versionId,sort_order:index,required:false,enabled:binding.enabled});
    ...
    await insertContractRecord(client,'ai_contract_publication',{entity_kind:'prompt_recipe',entity_id:recipe.id,from_version_id:previous,to_version_id:versionId,entity_revision:revision,action:'publish',actor:'prompt_composition',idempotency_key:input.idempotencyKey});
