...
export async function saveComposition(value:SaveCompositionInput,context?:CompositionDatabaseContext):Promise<SaveCompositionResult>{
  ...
  return database(context,async client=>{
    ...
    await client.query("INSERT INTO new_design.prompt_recipe_slots(id,recipe_version_id,slot_key,sort_order,required,allowed_content_types,variable_contract) VALUES($1,$2,'explicit_context',1,false,$3::text[],'{}'::jsonb)",[randomUUID(),versionId,compositionContextTypes(input)]);
