...
async function validateFrozen(client:PoolClient,row:Row):Promise<{plan:DebugFrozenPlan;snapshot:ManagedModelSnapshot}>{
  ...
  if(stableHash(exact.components)!==stableHash(plan.components)||stableHash(exact.sources)!==stableHash(plan.sources)||stableHash(exact.recipe.variables)!==stableHash(plan.recipe.variables)||stableHash(exact.recipe.context)!==stableHash(plan.recipe.context)||exact.recipe.taskType!==plan.recipe.taskType)throw new NewDesignError("精确组件或资料已不可用，请重新选择并生成预览。",409);
  ...
  const expectedReferences=[...plan.components.filter(item=>item.enabled).map(item=>({kind:"prompt_component",...item,role:"reference"})),...plan.sources.map(item=>({kind:"card_version",...item}))];
  const storedEntries=(await client.query("SELECT entry.*,version.revision AS exact_revision,version.type_version_id AS exact_type_version_id,version.title AS exact_title,version.values AS exact_values,card.space_id AS exact_space_id,slot.slot_key FROM new_design.context_manifest_entries entry JOIN new_design.context_manifest_slots slot ON slot.id=entry.slot_id JOIN new_design.card_versions version ON version.id=entry.exact_version_id AND version.card_id=entry.stable_object_id JOIN new_design.cards card ON card.id=version.card_id WHERE entry.manifest_id=$1 ORDER BY slot.sort_order,entry.sort_order,entry.id",[manifest.id])).rows;
  ...
  for(const [index,entry]of storedEntries.entries()){
    const reference=expectedReferences[index]!,hash=stableHash({revision:entry.exact_revision,typeVersionId:entry.exact_type_version_id,title:entry.exact_title,values:entry.exact_values});
    if(entry.source_type!==reference.kind||entry.stable_object_id!==reference.cardId||entry.exact_version_id!==reference.versionId||entry.content_hash!==hash||entry.source_space_id!==entry.exact_space_id||entry.source_revision!==entry.exact_revision||entry.content_role!==(reference.role==="formal"?"required":"reference")||entry.slot_key!==(reference.kind==="prompt_component"?"author_additions":"explicit_context"))throw new NewDesignError("冻结上下文内容、角色或精确引用不一致，请重新生成预览。",409);
    reconstructed.push({reference,spaceId:entry.exact_space_id,revision:Number(entry.exact_revision),hash});
  }
