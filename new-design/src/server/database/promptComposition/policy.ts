...
export const settingsSchema=z.object({taskType:taskSchema,components:z.array(z.object({cardId:z.string().uuid(),versionId:z.string().uuid(),enabled:z.boolean()}).strict()).max(100),variables:z.array(variable).max(100),context:z.object({bookId:z.string().uuid().nullable(),sources:z.array(z.object({cardId:z.string().uuid(),versionId:z.string().uuid(),role:z.enum(["formal","reference"])}).strict()).max(100)}).strict()}).strict().superRefine((input,ctx)=>{
  ...
  if(!input.context.bookId&&input.context.sources.length)ctx.addIssue({code:"custom",path:["context","bookId"],message:"选择参考资料前必须选择书籍。"});
});
