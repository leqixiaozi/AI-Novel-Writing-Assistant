...
export function createIndependentAiGateway(options:ExecutionDependencies={}):NewDesignAiGateway {
  async function execute<T>(taskType:PromptTaskType,input:unknown):Promise<AiResearchRunResult<T>> {
    ...
    const managedTask=visible?"form_assist":taskType==="story_workspace_batch"?((input as import("../../common/storyWorkspace").StoryBatchPromptInput).mode==="setting"?"form_assist":"planning_candidate"):taskType;
    let result;
