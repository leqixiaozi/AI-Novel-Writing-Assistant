...
export const characterResourceFocusAsset:PromptAsset={
 ...
 prepare(value){
  const input=value as ResourceFocusPromptInput,schema=resourceFocusSchemaFor(input),ledger=input.snapshot.ledger;
  // The full ledger stays in the original receipt; the model needs visible fields and scope identity.
  ...
  return{input:{...input,snapshot:{...input.snapshot,ledger:modelLedger}},schema};
 },
};
