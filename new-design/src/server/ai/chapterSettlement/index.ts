...
async function importSaved(claim:SettlementAiClaim):Promise<ChapterSettlementAiReceipt>{
  ...
  const prompt=preparePrompt(claim.plan.input.stableCorrection?'stable_resource_correction':claim.plan.input.stableSupplement?"stable_resource_supplement":claim.plan.input.resourceScope?"character_resource_backfill":"chapter_settlement",claim.plan.input);
...
export async function runChapterSettlementAiExtraction(sessionId:string,value:ChapterSettlementAiInput,dependencies:ExecutionDependencies={}):Promise<ChapterSettlementAiReceipt>{
  ...
  const claim=claimed,prompt=preparePrompt(claim.plan.input.stableCorrection?'stable_resource_correction':claim.plan.input.stableSupplement?"stable_resource_supplement":claim.plan.input.resourceScope?"character_resource_backfill":"chapter_settlement",claim.plan.input),started=Date.now();
