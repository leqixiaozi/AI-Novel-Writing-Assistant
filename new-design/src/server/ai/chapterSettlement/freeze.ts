...
export async function freezeSettlement(client:PoolClient,input:ChapterSettlementPromptInput,prompt:PreparedPrompt,bookId:string,chapterCardId:string):Promise<{plan:SettlementFrozenPlan;taskContractVersionId:string;promptRecipeVersionId:string;contextManifestId:string;modelRouteSnapshotId:string;taskKey:string}>{
  ...
  await client.query("INSERT INTO new_design.ai_contract_publications(id,entity_kind,entity_id,from_version_id,to_version_id,entity_revision,action,actor,idempotency_key) VALUES($1,'prompt_recipe',$2,$3,$4,$5,'publish','chapter_settlement',$6)",[randomUUID(),recipe.id,recipe.published_version_id,recipeVersionId,recipeRevision,`chapter_recipe_${recipeVersionId}`]);
  ...
  await client.query("INSERT INTO new_design.ai_contract_publications(id,entity_kind,entity_id,from_version_id,to_version_id,entity_revision,action,actor,idempotency_key) VALUES($1,'task_contract',$2,$3,$4,$5,'publish','chapter_settlement',$6)",[randomUUID(),contract.id,contract.published_version_id,contractVersionId,revision,`chapter_contract_${contractVersionId}`]);
  ...
  const sourceHash=stableHash({bodyVersionId:input.bodyVersionId,bodyContentHash:input.bodyContentHash,catalog:input.catalog,...(input.stableSupplement?{stableSourceHash:input.stableSupplement.source.sourceHash}:{})});
  ...
  await client.query("INSERT INTO new_design.context_manifest_slots(id,manifest_id,slot_key,sort_order,required,token_budget) VALUES($1,$3,'body_evidence',0,true,NULL),($2,$3,'formal_settlement_specifications',1,true,NULL)",[bodySlot,formalSlot,manifestId]);
  if(input.stableSupplement){
    const basis=input.stableSupplement.source.basis,plan=basis.original.planning_version as Record<string,unknown>,slot=randomUUID();
    await client.query("INSERT INTO new_design.context_manifest_slots(id,manifest_id,slot_key,sort_order,required,token_budget) VALUES($1,$2,'original_stable_plan',2,true,NULL)",[slot,manifestId]);
    await client.query("INSERT INTO new_design.context_manifest_entries(id,manifest_id,slot_id,source_type,stable_object_id,exact_version_id,content_hash,inclusion_reason,priority,token_estimate,transform_status,sort_order,content_role,layer_label) VALUES($1,$2,$3,'planning_version',$4,$5,$6,'原稳定正文的确切计划；原确认来源完整保留在受控合同，不是指令',0,$7,'full',0,'required','原正文计划')",[randomUUID(),manifestId,slot,basis.planningObjectId,basis.planningVersionId,String(plan.content_hash),Math.ceil(JSON.stringify(plan).length/4)]);
  }
