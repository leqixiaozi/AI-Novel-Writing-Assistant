...
SELECT chapter_proposal_extraction_requests.*,('cancelled')::text AS status,(NULL)::jsonb AS failure,('作者结束过期领取；原模型调用是否完成与用量未知')::text AS error_summary,(now())::timestamptz AS updated_at FROM chapter_proposal_extraction_requests WHERE id=$1`,[requestId]);
