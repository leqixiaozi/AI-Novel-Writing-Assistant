...
async function receive(config:ModelConfiguration,url:string,init:RequestInit,step:string,fetcher:typeof fetch=fetch):Promise<Record<string,unknown>> {
  ...
  try {
    ...
    if(!response.ok){const message=response.status===401||response.status===403?"模型服务拒绝凭据，请打开模型设置检查授权。":response.status===429?"模型服务调用额度或频率受限，请稍后在来源页重试。":"模型服务未完成请求，请检查模型服务连接和模型名称。";throw new AiExecutionError(step,message);}
    const value:unknown=await response.json();
    if(value===null||typeof value!=="object"||Array.isArray(value))throw new AiExecutionError(step,"模型服务回执格式无效，请检查接口兼容性。");
    return value as Record<string,unknown>;
  }catch(error){if(error instanceof AiExecutionError)throw error;throw new AiExecutionError(step,controller.signal.aborted?"等待模型回复超时。模型服务可能仍在处理，请回来源页核对状态后再重试。":"无法取得模型服务回执，请打开模型设置检查连接。已发送请求可能仍在处理。");}
  finally {clearTimeout(timer);}
}

export async function probeModelConnection(config:ModelConfiguration,fetcher:typeof fetch=fetch):Promise<{available:boolean;modelFound:boolean;models:string[]}> {
  ...
  if(!modelFound)throw new AiExecutionError("核对创作模型","连接成功，但配置的模型不在服务返回列表中。请检查模型名称或在模型服务中准备该模型。",422);
