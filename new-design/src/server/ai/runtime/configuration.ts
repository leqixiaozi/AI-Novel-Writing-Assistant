...
export interface ModelConfiguration {provider:"ollama"|"openai-compatible";endpoint:string;model:string;apiKey:string;timeoutMs:number;maxTokens:number;identity:string;}
...
export function readModelConfiguration(environment:NodeJS.ProcessEnv=process.env):ModelConfiguration {
  ...
  if(provider!=="ollama"&&provider!=="openai-compatible")throw new AiExecutionError("配置模型","请选择 Ollama 或兼容 OpenAI 接口的模型服务。",422);
  let endpoint:URL;
  ...
  if(provider==="openai-compatible"&&!apiKey&&!local)throw new AiExecutionError("配置模型","远程模型凭据未配置。请按“模型设置”的凭据说明配置后重启新设计服务。",422);
