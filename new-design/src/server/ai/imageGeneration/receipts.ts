...
const allRefsSchema=z.union([bookRefs,publicRefs,dialogueRefs]);
const schema=z.union([bookRefs.extend(replyShape).strict(),publicRefs.extend(replyShape).strict(),dialogueRefs.extend({contract:z.literal('public_character_dialogue_reply_v1'),reply:dialogueReply,replyHash:z.string().regex(/^[a-f0-9]{64}$/)}).strict()]);
