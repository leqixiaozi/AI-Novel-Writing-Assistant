...
const publish=z.object({sourceTemplateVersionId:uuid,templateRevision:z.number().int().positive(),previewHash:z.string().regex(/^[a-f0-9]{64}$/),requestKey:uuid}).strict();
...
export function referenceParityRouter(){
  ...
  router.get("/reference-specifications/:versionId",async(req,res,next)=>{try{res.json({success:true,data:await previewReferenceSpecification(uuid.parse(req.params.versionId))});}catch(error){next(error);}});
