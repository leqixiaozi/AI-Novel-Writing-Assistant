...
export function worldUsageRouter(ai?:NewDesignAiGateway):Router{
 ...
 router.post('/books/:bookId/world-usage/:rootCardId/candidates',(req,res,next)=>{void Promise.resolve().then(()=>{const value=scope(req.params),input=worldUsagePrepareInputSchema.parse(req.body);if(input.mode==='ai')throw new WorldUsageError('AI 整理建议待项目提示词入口确认，尚未调用模型或写入候选。',503,'not_written');return prepareWorldUsageCandidate(value.bookId,value.rootCardId,input,ai);}).then(data=>res.status(201).json({success:true,data})).catch(error=>fail(error,next));});
