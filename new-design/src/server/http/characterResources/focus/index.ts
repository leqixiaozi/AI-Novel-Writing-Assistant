...
export function resourceFocusRouter(ai?:NewDesignAiGateway){
 ...
 router.get(`${base}/preview`,(req,res,next)=>{void Promise.resolve().then(()=>previewResourceFocus(uuid.parse(req.params.bookId),uuid.parse(req.params.characterId),resourceFocusSelectionSchema.parse(req.query))).then(data=>res.json({success:true,data})).catch(next);});
