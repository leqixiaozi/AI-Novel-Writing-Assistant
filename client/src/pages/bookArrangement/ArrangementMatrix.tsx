...
export function ArrangementMatrix({ workspace, draft, chapters, selectedId, scope, onSelect, onScope, onSpan, onDraft, characterSearch = "", onCharacter, onHistory, selectedVolumeId, onVolume, onVolumeEdit, windowStart, onWindowStart }: Props) {
  ...
  const cell = (chapterId: string, content: ReactNode, className = "") => <button type="button" key={chapterId} data-chapter-id={chapterId} className={`ba-cell ${className} ${chapterId === selectedId ? "is-selected" : ""}`} onClick={() => onSelect(chapterId)}>{content}</button>;
    ...
    <div className="book-arrangement-scroll" tabIndex={0} aria-label="章节矩阵，可横向滚动">
      <div className="ba-matrix ba-compact-matrix" style={{ "--ba-count": chapters.length } as CSSProperties}>
        ...
        {groups.events ? <div className="ba-track-content" /> : chapters.map(chapter => { const events = workspace.events.filter(event => event.chapterId === chapter.id); const scenes = workspace.scenes.filter(scene => scene.chapterId === chapter.id).sort((a, b) => a.sortOrder - b.sortOrder || a.id.localeCompare(b.id)); return cell(chapter.id, <>
          <span className="ba-event-dots">{events.slice(0, 3).map(event => <span key={event.id} title={`${event.title} · ${eventStatusLabels[event.status] || "状态未标注"}`}><Circle size={9} fill={event.status === "occurred" || event.status === "resolved" ? "currentColor" : "none"} /></span>)}{events.length > 3 && <small>+{events.length - 3}</small>}{!events.length && <span className="ba-unset">无事件</span>}</span>
          <span className="ba-scene-tags">{scenes.slice(0, 3).map((scene, index) => <span key={scene.id} className="ba-scene" title={scene.title}>S{index + 1}</span>)}{scenes.length > 3 && <small>+{scenes.length - 3}</small>}{!scenes.length && <span className="ba-unset">无场景</span>}</span>
        </>, "ba-event-scene-cell"); })}
        ...
        {groups.reviews ? <div className="ba-track-content" /> : (workspace.checks ?? []).some(check => check.chapterIds.some(id => chapters.some(chapter => chapter.id === id))) ? chapters.map(chapter => { const checks = (workspace.checks ?? []).filter(check => check.chapterIds.includes(chapter.id)); return <div key={chapter.id} className={`ba-cell ba-check-cell ${chapter.id === selectedId ? "is-selected" : ""}`} data-chapter-id={chapter.id}>{checks.slice(0, 2).map(check => {
          ...
          return <button key={check.id} type="button" className={`ba-check-result ${handledLabel ? "is-handled" : "is-open"}`} data-status={check.status} title={`${handledLabel ?? "待处理"} · ${check.title}\n${check.evidenceLabel}\n${check.summary}`} onClick={() => history(chapter.id)}>{handledLabel ? <CheckCircle2 size={14} /> : <TriangleAlert size={14} />}<span>{check.title}<small className="ba-check-status">{handledLabel ?? "待处理"}</small><small>{check.evidenceLabel}</small></span></button>;
