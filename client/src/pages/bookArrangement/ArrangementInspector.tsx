...
import { characterSpanError, mergeCharacterSpans, splitCharacterSpan } from "./panels/characterEditing";
...
export function ArrangementInspector({ workspace, draft, selectedId, scope, spanId, spanSelection, onSpan, onDraft, busy,
  characterId, historySelection, onSelectChapter, onAddAppearance, onScope, onPreview, previewDisabled, dirty, onObject }: {
  ...
  const [spanNotice, setSpanNotice] = useState("");
  useEffect(() => { if (spanId) setTab("arrangement"); }, [spanId, spanSelection]);
  useEffect(() => { setTab(historySelection ? "history" : "arrangement"); }, [historySelection]);
  ...
  const updateSpan = (patch: Partial<BookArrangementCharacterSpan>) => {
    if (!span) return; const next = { ...span, ...patch }; const error = characterSpanError(draft, next); if (error) { setSpanNotice(error); return; } onDraft({ ...draft, characterSpans: draft.characterSpans.map(item => item.id === span.id ? next : item) });
  };
  ...
  const chapterSettings = (<details open={!span} className="ba-detail ba-chapter-settings"><summary>本章备注与表达参数</summary><label className="ba-lock-control"><input type="checkbox" aria-label="锁定本章编排" checked={edit.locked} onChange={event => onDraft(updateChapterEdit(draft, chapter.id, { locked: event.target.checked }))} />锁定本章编排</label><fieldset disabled={edit.locked} className="ba-span-fields"><label className="ba-field"><span>编排备注</span><textarea aria-label="编排备注" rows={3} className="ba-input" value={edit.note} onChange={event => onDraft(updateChapterEdit(draft, chapter.id, { note: event.target.value }))} /></label><details open={!span} className="ba-detail"><summary>本章五种表达参数</summary><WritingControlsForm controls={edit.controls} characters={workspace.characters} definitions={[]} onChange={controls => onDraft(updateChapterEdit(draft, chapter.id, { controls }))} /></details></fieldset><details className="ba-detail"><summary>已应用的后续要求</summary>{workspace.appliedSettings[chapter.id] ? <ArrangementRequirements settings={workspace.appliedSettings[chapter.id].settings} workspace={workspace} /> : <p>尚未应用编排要求。</p>}</details></details>);
  return <aside className="ba-inspector">
    ...
    <Button size="sm" variant="outline" onClick={() => onObject?.({ kind: "check", id: "chapter", chapterId: selectedId })}>核对本章</Button><div className="ba-inspector-tabs" role="tablist" aria-label="编排编辑"><button type="button" role="tab" aria-selected={tab === "arrangement"} onClick={() => setTab("arrangement")}>{span ? "出场安排" : "章节设置"}</button><button type="button" role="tab" aria-selected={tab === "history"} onClick={() => setTab("history")}>历史依据</button></div>
    <fieldset disabled={busy} className="ba-inspector-body">
      {tab === "arrangement" ? <>
        {!span && chapterSettings}
        <section className="ba-inspector-card"><header className="ba-card-heading"><span className="ba-status-dot" />{span ? "人物参与计划" : "出场安排"}</header>
          <div className="ba-card-content">
            ...
            {span && <><fieldset disabled={spanLocked} className="ba-span-fields">
              ...
              <div className="space-y-2"><Button size="sm" variant="outline" disabled={!scope.length || scope.some(id => chapterEdit(draft, id).locked)} onClick={() => { const id = crypto.randomUUID(); const copy = { ...span, id, chapterIds: workspace.chapters.filter(chapter => scope.includes(chapter.id)).map(chapter => chapter.id) }; const error = characterSpanError(draft, copy); if (error) { setSpanNotice(error); return; } onDraft({ ...draft, characterSpans: [...draft.characterSpans, copy] }); onSpan(id); }}>复制到勾选章节</Button><div className="flex flex-wrap gap-2"><select className="ba-input ba-inline" aria-label="人物区段拆分章节" value={splitAt} onChange={event => setSplitAt(event.target.value)}><option value="">从哪章拆为后一段</option>{workspace.chapters.filter(chapter => span.chapterIds.includes(chapter.id)).slice(1).map(chapter => <option key={chapter.id} value={chapter.id}>{chapter.order} · {chapter.title}</option>)}</select><Button size="sm" variant="outline" disabled={!splitAt} onClick={() => { const pieces = splitCharacterSpan(span, workspace.chapters, splitAt, crypto.randomUUID()); if (!pieces) { setSpanNotice("请选择区段内的后一章作为拆分点。"); return; } onDraft({ ...draft, characterSpans: draft.characterSpans.flatMap(item => item.id === span.id ? pieces : [item]) }); setSplitAt(""); setSpanNotice("已拆成两段，分别保留参与规则与备注。"); }}>拆分</Button></div><div className="flex flex-wrap gap-2"><select className="ba-input ba-inline" aria-label="合并人物区段" value={mergeId} onChange={event => setMergeId(event.target.value)}><option value="">选择相邻且要求一致的区段</option>{draft.characterSpans.filter(item => mergeCharacterSpans(span, item, workspace.chapters) && !item.chapterIds.some(id => chapterEdit(draft, id).locked)).map(item => <option key={item.id} value={item.id}>{item.chapterIds.map(id => workspace.chapters.find(chapter => chapter.id === id)?.order).join("、")} 章</option>)}</select><Button size="sm" variant="outline" disabled={!mergeId} onClick={() => { const target = draft.characterSpans.find(item => item.id === mergeId); const merged = target && mergeCharacterSpans(span, target, workspace.chapters); if (!merged || target?.chapterIds.some(id => chapterEdit(draft, id).locked)) { setSpanNotice("只能合并同一人物、相同要求且连续的未锁定区段。"); return; } onDraft({ ...draft, characterSpans: draft.characterSpans.filter(item => item.id !== mergeId).map(item => item.id === span.id ? merged : item) }); setMergeId(""); setSpanNotice("已合并区段。"); }}>合并</Button></div>{spanNotice && <p role="status" className="ba-help">{spanNotice}</p>}</div>
        ...
        <div className="ba-card-content">
          ...
          {events.map(event => <article key={event.id} className="ba-evidence-item">
            ...
            <h3>{event.title}</h3><p>{event.summary}</p><Button size="sm" variant="outline" onClick={() => onObject?.({ kind: "event", id: event.id, chapterId: chapter.id })}>查看与调整此事件</Button><Link to={sourceUrl}>打开来源章节<ChevronRight size={14} aria-hidden="true" /></Link>
          </article>)}
          {projections.map(item => <article className="ba-evidence-item" key={item.id}><span className="ba-evidence-status">{item.evidenceLabel}</span>{(item.sourceEntity === "AuditIssue" || item.sourceEntity === "OpenConflict") && <span className="ba-check-status">状态：{checkStatusLabels[item.status] ?? item.status}</span>}<h3>{item.title}</h3><p>{item.summary}</p><Button size="sm" variant="outline" onClick={() => onObject?.({ kind: item.sourceEntity === "TimelineHook" ? "hook" : item.sourceEntity === "ForeshadowState" ? "foreshadow" : item.sourceEntity === "AuditIssue" || item.sourceEntity === "OpenConflict" ? "check" : "relation", id: item.sourceId, chapterId: chapter.id })}>打开此项</Button></article>)}
          ...
          {workspace.scenes.filter(scene => scene.chapterId === chapter.id).map(scene => <article className="ba-evidence-item" key={scene.id}><h3>{scene.title}</h3><p>{scene.objective || "未记录场景目标。"}</p><Button size="sm" variant="outline" onClick={() => onObject?.({ kind: "scene", id: scene.id, chapterId: chapter.id })}>查看与调整此场景</Button></article>)}
        </div>
      </section>}
    </fieldset>
