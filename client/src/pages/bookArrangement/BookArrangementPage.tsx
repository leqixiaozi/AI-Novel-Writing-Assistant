...
function ArrangementEditor({ initialWorkspace, onDirty, novelPicker }: { initialWorkspace: BookArrangementWorkspace; onDirty: (value: boolean) => void; novelPicker: ReactNode }) {
  ...
  const [panel, setPanel] = useState<"book" | "relations" | "tracks" | "hooks" | "chapter" | "volume" | "controls" | "planning" | "requirements" | "volume-preview" | "object" | null>(null);
  ...
  return <div className="ba-editor">
    ...
    <Dialog open={panel !== null} onOpenChange={open => { if (!open) closePanel(); }}><AppDialogContent title={panel === "book" ? "整书资料与范围" : panel === "relations" ? "人物关系" : panel === "hooks" ? "线索与伏笔" : panel === "tracks" ? "表达轨道" : panel === "volume" ? "卷段编排" : panel === "volume-preview" ? "卷段调整预览" : panel === "planning" ? "AI 重编排大纲" : panel === "requirements" ? "后续写作要求" : panel === "controls" ? "表达参数" : panel === "object" ? "故事资料与调整" : "章节与人物编排"} description={panel === "hooks" ? "在线索池中筛选对象，沿统一章节轴安排建立、强化、误导、揭示、回收与余波。" : "查看资料、调整草稿，再预览和应用。关闭面板保留编排草稿。"} className="ba-side-dialog" bodyClassName="ba-side-body" onInteractOutside={event => { if (busy) event.preventDefault(); }} onEscapeKeyDown={event => { if (busy) event.preventDefault(); }} footer={<><span className="text-xs text-muted-foreground mr-auto">{panel === "object" ? objectDirty ? "此项编辑已暂存本地，尚未应用" : "对象调整在本面板预览与应用" : panel === "hooks" ? "节点通过候选预览后单独应用" : dirty ? "有未保存的编排" : saved.revision ? "编排草稿已保存" : "尚未保存编排"}</span>{panel !== "object" && <Button variant="outline" disabled={Boolean(busy)} onClick={closePanel}>关闭</Button>}{panel !== "object" && panel !== "book" && panel !== "relations" && panel !== "hooks" && <Button disabled={Boolean(busy) || (!dirty && saved.revision > 0)} onClick={() => void save()}>保存编排草稿</Button>}</>}>
      ...
      {panel === "hooks" && <HookLifecyclePanel workspace={workspace} selectedChapterId={selectedId} windowStart={window.start} windowSize={window.chapters.length} onWindowStart={setWindowStart} onSelectChapter={selectChapter} onObject={selection => { setSelectedObject(selection); setPanel("object"); }} />}
