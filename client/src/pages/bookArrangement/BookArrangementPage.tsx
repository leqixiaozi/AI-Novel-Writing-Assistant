...
function ArrangementEditor({ initialWorkspace, onDirty, novelPicker }: { initialWorkspace: BookArrangementWorkspace; onDirty: (value: boolean) => void; novelPicker: ReactNode }) {
  ...
  const openChapter = (id: string) => { selectChapter(id); setScope([id]); setSpanId(""); setCharacterId(""); setHistorySelection(0); setPanel("chapter"); };
  const addAppearance = () => {
    setSelectedVolumeId("");
    setHistorySelection(0);
  ...
  const currentChapter = workspace.chapters.find(chapter => chapter.id === selectedId);
  const panelTitle = panel === "book" ? "整书资料与范围" : panel === "relations" ? "人物关系" : panel === "hooks" ? "线索与伏笔" : panel === "tracks" ? "场景表达轨道" : panel === "expression-point" ? "场景表达详细设置" : panel === "volume" ? "卷段编排" : panel === "volume-preview" ? "卷段调整预览" : panel === "planning" ? "AI 重编排大纲" : panel === "requirements" ? "后续写作要求" : panel === "controls" ? "表达参数" : "章节与人物编排";
  const activePanelScope = panelScopeLabel(panel, currentChapter?.order ?? 1, scope.length);
  ...
  return <div className="ba-editor">
    ...
    <fieldset disabled={Boolean(busy)} className="ba-layout">
      ...
      <div id="ba-chapter-matrix"><ArrangementMatrix workspace={workspace} draft={draft} chapters={window.chapters} selectedId={selectedId} onSelect={selectChapter} onSpan={selectSpan} onDraft={updateDraft} characterSearch={characterSearch} onCharacter={(id, chapterId) => { setSelectedVolumeId(""); setCharacterId(id); if (chapterId) selectChapter(chapterId); }} onHistory={(chapterId, personId) => { selectChapter(chapterId); setCharacterId(personId ?? ""); setSpanId(""); setHistorySelection(value => value + 1); setPanel("chapter"); }} selectedVolumeId={selectedVolumeId} onVolume={id => { setSelectedVolumeId(id); setSpanId(""); setPanel("volume"); }} onVolumeEdit={updateVolume} windowStart={window.start} onWindowStart={setWindowStart} expressionDefinitions={expressionDefinitions} expressionPoints={expressionPoints} selectedExpression={selectedExpression ? expressionPointKey(selectedExpression.sceneId, selectedExpression.dimensionKey) : undefined} onExpressionPoints={points => { setExpressionPoints(points); setNotice(""); }} onExpressionSelect={(sceneId, dimensionKey, open) => { setSelectedExpression({ sceneId, dimensionKey }); const scene = workspace.scenes.find(item => item.id === sceneId); if (scene) selectChapter(scene.chapterId); if (open) setPanel("expression-point"); }} onChapterMenuAction={(action, chapterId) => {
    ...
    <Dialog open={panel !== null} onOpenChange={open => { if (!open) closePanel(); }}><AppDialogContent title={<span className="ba-dialog-title"><span>{panelTitle}</span><small>{activePanelScope}</small></span>} description={panel === "hooks" ? "在线索池中筛选对象，沿统一章节轴安排建立、强化、误导、揭示、回收与余波。" : panel === "expression-point" ? "调整当前场景的写法档位；剧情内容和场景任务保持不变。" : "查看资料、调整草稿，再预览和应用。关闭面板保留编排草稿。"} className="ba-side-dialog" bodyClassName="ba-side-body" onInteractOutside={event => { if (busy) event.preventDefault(); }} onEscapeKeyDown={event => { if (busy) event.preventDefault(); }} footer={<><span className="text-xs text-muted-foreground mr-auto">{panel === "hooks" ? "节点通过候选预览后单独应用" : dirty ? "有未保存的编排" : saved.revision || savedExpression.points.length ? "编排草稿已保存" : "尚未保存编排"}</span><Button variant="outline" onClick={closePanel}>关闭</Button>{panel !== "book" && panel !== "relations" && panel !== "hooks" && <Button disabled={Boolean(busy) || !dirty} onClick={() => void save()}>保存编排草稿</Button>}</>}>
    ...
    <Dialog open={panelLayers.objectOpen && selectedObject !== null} onOpenChange={open => { if (!open) closeObjectPanel(); }}><AppDialogContent title={<span className="ba-dialog-title"><span>故事资料与调整</span><small>{selectedObject?.kind === "scene" ? "单个场景" : "章节对象"}</small></span>} description="查看资料、调整草稿，再预览和应用。关闭当前层后返回原来的编排位置。" className="ba-side-dialog" bodyClassName="ba-side-body" onInteractOutside={event => { if (busy) event.preventDefault(); }} onEscapeKeyDown={event => { if (busy) event.preventDefault(); }} footer={<span className="text-xs text-muted-foreground mr-auto">{objectDirty ? "此项编辑已暂存本地，尚未应用" : "对象调整在本面板预览与应用"}</span>}>
      {error && <p role="alert" className="text-sm text-destructive">{error}</p>}{(busy || notice) && <p role="status" className="text-sm text-muted-foreground">{busy || notice}</p>}
      <fieldset disabled={Boolean(busy)} className="min-w-0 space-y-4">
