...
import { BookOpen, ChevronLeft, ChevronRight, Layers3, MoreHorizontal, Search } from "lucide-react";
...
function ArrangementEditor({ initialWorkspace, onDirty, novelPicker }: { initialWorkspace: BookArrangementWorkspace; onDirty: (value: boolean) => void; novelPicker: ReactNode }) {
  ...
  const [panel, setPanel] = useState<"book" | "chapter" | "volume" | "controls" | "planning" | "requirements" | "volume-preview" | "object" | null>(null);
  ...
  return <div className="ba-editor">
    ...
    }} /></label><div className="ba-scope-switcher" aria-label="按范围编辑"><Button size="sm" variant="ghost" onClick={() => setPanel("book")}><BookOpen size={15} />整书资料</Button><Button size="sm" variant="ghost" onClick={() => { if (!currentVolume) { setNotice("当前章节还没有归属卷段。"); return; } setSelectedVolumeId(currentVolume.id); setPanel("volume"); }}><Layers3 size={15} />当前卷</Button><Button size="sm" variant="ghost" onClick={() => openChapter(selectedId)}>当前章</Button></div>
    ...
    <Dialog open={panel !== null} onOpenChange={open => { if (!open) closePanel(); }}><AppDialogContent title={panel === "book" ? "整书资料与范围" : panel === "volume" ? "卷段编排" : panel === "volume-preview" ? "卷段调整预览" : panel === "planning" ? "AI 重编排大纲" : panel === "requirements" ? "后续写作要求" : panel === "controls" ? "表达参数" : panel === "object" ? "故事资料与调整" : "章节与人物编排"} description="查看资料、调整草稿，再预览和应用。关闭面板保留编排草稿。" className="ba-side-dialog" bodyClassName="ba-side-body" onInteractOutside={event => { if (busy) event.preventDefault(); }} onEscapeKeyDown={event => { if (busy) event.preventDefault(); }} footer={<><span className="text-xs text-muted-foreground mr-auto">{panel === "object" ? objectDirty ? "此项编辑已暂存本地，尚未应用" : "对象调整在本面板预览与应用" : dirty ? "有未保存的编排" : saved.revision ? "编排草稿已保存" : "尚未保存编排"}</span><Button variant="outline" disabled={Boolean(busy)} onClick={closePanel}>关闭</Button>{panel !== "object" && panel !== "book" && <Button disabled={Boolean(busy) || (!dirty && saved.revision > 0)} onClick={() => void save()}>保存编排草稿</Button>}</>}>
      ...
      {panel === "book" && <section className="ba-book-scope-panel"><div className="ba-book-facts"><span>题材<strong>{workspace.genre?.name || "未设置"}</strong></span><span>章节<strong>{workspace.chapters.length} 章</strong></span><span>卷段<strong>{workspace.volumes.length} 卷</strong></span><span>人物<strong>{workspace.characters.length} 人</strong></span></div><div className="ba-scope-level"><h3>整书必须明确</h3><p>题材定位、故事主线、世界规则、核心人物和全书终局。它们会影响所有卷和章节。</p><div><Link target="_blank" rel="noreferrer" to={`/novels/${workspace.novelId}/edit?stage=story_macro`}>编辑故事主线</Link><Link target="_blank" rel="noreferrer" to={`/novels/${workspace.novelId}/edit?stage=world`}>编辑世界观与规则</Link><Link target="_blank" rel="noreferrer" to={`/novels/${workspace.novelId}/edit?stage=character`}>编辑核心人物</Link></div></div><div className="ba-scope-level"><h3>卷段需要明确</h3><p>本卷承诺、主角变化、高潮和通往下一卷的衔接。点击顶部“当前卷”在右侧修改。</p></div><div className="ba-scope-level"><h3>章节需要明确</h3><p>章节目标、事件、场景、人物参与和表达参数。点击顶部“当前章”或矩阵中的对象修改。</p></div><p className="ba-panel-note">世界观继续由底座原“世界观准备”维护，避免产生两份互相冲突的世界设定。链接会在新窗口打开，当前编排位置和草稿保留。</p></section>}
