...
import { BookOpen, ChevronLeft, ChevronRight, MoreHorizontal, Network, Search, UsersRound } from "lucide-react";
...
function ArrangementEditor({ initialWorkspace, onDirty, novelPicker }: { initialWorkspace: BookArrangementWorkspace; onDirty: (value: boolean) => void; novelPicker: ReactNode }) {
  ...
  const [panel, setPanel] = useState<"book" | "characters" | "relations" | "chapter" | "volume" | "controls" | "planning" | "requirements" | "volume-preview" | "object" | null>(null);
  ...
  return <div className="ba-editor">
    ...
    }} /></label><div className="ba-scope-switcher" aria-label="整书编排入口"><Button size="sm" variant="ghost" onClick={() => setPanel("book")}><BookOpen size={15} />整书资料</Button><Button size="sm" variant="ghost" onClick={() => setPanel("characters")}><UsersRound size={15} />书籍角色</Button><Button size="sm" variant="ghost" onClick={() => setPanel("relations")}><Network size={15} />人物关系</Button></div>
    ...
    <Dialog open={panel !== null} onOpenChange={open => { if (!open) closePanel(); }}><AppDialogContent title={panel === "book" ? "整书资料与范围" : panel === "characters" ? "书籍角色" : panel === "relations" ? "全书人物关系" : panel === "volume" ? "卷段编排" : panel === "volume-preview" ? "卷段调整预览" : panel === "planning" ? "AI 重编排大纲" : panel === "requirements" ? "后续写作要求" : panel === "controls" ? "表达参数" : panel === "object" ? "故事资料与调整" : "章节与人物编排"} description="查看资料、调整草稿，再预览和应用。关闭面板保留编排草稿。" className="ba-side-dialog" bodyClassName="ba-side-body" onInteractOutside={event => { if (busy) event.preventDefault(); }} onEscapeKeyDown={event => { if (busy) event.preventDefault(); }} footer={<><span className="text-xs text-muted-foreground mr-auto">{panel === "object" ? objectDirty ? "此项编辑已暂存本地，尚未应用" : "对象调整在本面板预览与应用" : dirty ? "有未保存的编排" : saved.revision ? "编排草稿已保存" : "尚未保存编排"}</span><Button variant="outline" disabled={Boolean(busy)} onClick={closePanel}>关闭</Button>{panel !== "object" && panel !== "book" && panel !== "characters" && panel !== "relations" && <Button disabled={Boolean(busy) || (!dirty && saved.revision > 0)} onClick={() => void save()}>保存编排草稿</Button>}</>}>
      ...
      {panel === "characters" && <section className="ba-book-catalog"><header><div><h3>本书角色名册</h3><p>这里汇总整本书的角色。角色设定仍由底座角色准备维护。</p></div><Link target="_blank" rel="noreferrer" to={`/novels/${workspace.novelId}/edit?stage=character`}>管理全部角色</Link></header>{workspace.characters.map(character => { const eventCount = workspace.events.filter(event => event.participantIds.includes(character.id)).length; const relationCount = (workspace.relations ?? []).filter(relation => relation.sourceCharacterId === character.id || relation.targetCharacterId === character.id).length; return <article key={character.id}><div><strong>{character.name}</strong><span>{character.role || "角色定位未设置"}</span></div><small>{eventCount} 个事件 · {relationCount} 个关系阶段</small><Link target="_blank" rel="noreferrer" to={`/novels/${workspace.novelId}/edit?stage=character`}>编辑角色资料</Link></article>; })}</section>}
      {panel === "relations" && <section className="ba-book-catalog"><header><div><h3>全书人物关系</h3><p>先查看全书关系阶段，再点击具体关系进入对应配置。</p></div><Link target="_blank" rel="noreferrer" to={`/novels/${workspace.novelId}/edit?stage=character`}>打开角色关系网</Link></header>{(workspace.relations ?? []).map(relation => { const source = workspace.characters.find(character => character.id === relation.sourceCharacterId)?.name || "未知角色"; const target = workspace.characters.find(character => character.id === relation.targetCharacterId)?.name || "未知角色"; return <button type="button" className="ba-relation-catalog-item" key={relation.id} onClick={() => { setSelectedObject({ kind: "relation", id: relation.id, chapterId: relation.chapterId ?? undefined }); setPanel("object"); }}><span><strong>{source} → {target}</strong><small>{relation.sourceType || relation.status || "关系阶段"}</small></span><b>{relation.title}</b><em>{relation.summary || relation.evidenceLabel}</em></button>; })}{!(workspace.relations ?? []).length && <p className="ba-empty-state">本书尚未建立人物关系阶段。请从角色关系网或矩阵中新建。</p>}</section>}
