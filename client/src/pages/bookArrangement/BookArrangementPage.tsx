...
function ArrangementEditor({ initialWorkspace, onDirty, novelPicker }: { initialWorkspace: BookArrangementWorkspace; onDirty: (value: boolean) => void; novelPicker: ReactNode }) {
  ...
  return <div className="ba-editor">
    ...
    <fieldset disabled={Boolean(busy)} className="ba-layout">
      ...
      <div className="flex flex-wrap gap-2"><Button size="sm" variant="ghost" onClick={() => setSceneEditor({ chapterId: selectedId })}>编排所选章节场景</Button><Button size="sm" variant="ghost" onClick={() => openChapter(selectedId)}>章节与人物设置</Button><Button size="sm" variant="ghost" onClick={() => { setControlKey(undefined); setPanel("controls"); }}>批量表达参数</Button><Button size="sm" variant="ghost" onClick={() => setPanel("volume-preview")}>卷段预览</Button>{dirty && <Button size="sm" variant="ghost" disabled={Boolean(busy)} onClick={() => { if (globalThis.confirm("放弃尚未保存的编排编辑，并载入服务端草稿？")) void run("重载服务端草稿", {}, async () => { await reload(true); return true; }); }}>放弃本地编排并重载</Button>}</div>
