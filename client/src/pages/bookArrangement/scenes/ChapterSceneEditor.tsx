...
export function ChapterSceneEditor({ workspace, chapterId, initialSceneId, busy, run, reload, onBack, onDirty, onObject }: Props) {
  ...
  const beginBoundary = (event: ReactPointerEvent<HTMLButtonElement>, index: number) => { const width = event.currentTarget.closest(".ba-scene-budget-track")?.getBoundingClientRect().width ?? 1; boundary.current = { index, x: event.clientX, width, base: scenes }; event.currentTarget.setPointerCapture(event.pointerId); };
  ...
  return <section className="ba-scene-editor" aria-label={`第${chapter.order}章场景编排`}>
    ...
    <section className="ba-scene-budget"><header><div><h2>场景篇幅分配 <em>必须补</em></h2><p>左右拖动分界线调整相邻场景占比，参考字数会自动更新。</p></div><div className="ba-scene-budget-total">本章参考字数 <strong>{totalWords}</strong> 字 <Button variant="outline" size="sm" disabled={scenes.length >= 8} onClick={add}><Plus size={16} />添加场景</Button></div></header>
      <div className="ba-scene-budget-track">{scenes.map((scene, index) => <div key={scene.id} className={`ba-scene-budget-segment is-${colors[index % colors.length]} ${selected?.id === scene.id ? "is-selected" : ""}`} style={{ width: `${sceneBudgetPercent(scene, scenes)}%` }} onClick={() => setSelectedId(scene.id)}><b>S{index + 1}</b><span>{scene.title}</span><strong>{sceneBudgetPercent(scene, scenes)}%</strong><small>约{scene.targetWordCount}字</small>{index < scenes.length - 1 && <button type="button" className="ba-scene-boundary" aria-label={`调整场景 S${index + 1} 与 S${index + 2} 的篇幅`} onPointerDown={event => beginBoundary(event, index)} onPointerMove={dragBoundary} onPointerUp={() => { boundary.current = null; }} onKeyDown={event => { if (event.key === "ArrowLeft" || event.key === "ArrowRight") { event.preventDefault(); change(resizeSceneBoundary(scenes, index, event.key === "ArrowRight" ? 30 : -30)); } }}><span aria-hidden="true">Ⅱ</span></button>}</div>)}</div>
