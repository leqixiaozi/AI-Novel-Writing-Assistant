...
export function ArrangementRequirements({ settings, workspace }: { settings: WritingSettingsPayload; workspace: BookArrangementWorkspace }) {
  return <div className="ba-requirements space-y-2 text-xs"><p className="text-muted-foreground">{settings.enabled ? "启用可选要求" : "未启用要求"}</p>{arrangementTracks.filter(track => settings.controls[track.key]).map(track => {
...
export function ArrangementPreview({ preview, workspace, selected, onSelected, disabled, applied, onApply }: {
  ...
  return <section aria-label="后续要求预览" className="ba-requirements-preview space-y-3">
