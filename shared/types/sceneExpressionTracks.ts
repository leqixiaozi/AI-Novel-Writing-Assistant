...
export type SystemSceneExpressionDimensionKey = typeof SCENE_EXPRESSION_DIMENSION_KEYS[number];
export type SceneExpressionDimensionKey = string;
export type SceneExpressionLevel = 1 | 2 | 3 | 4 | 5;
export type SceneExpressionColor = "blue" | "orange" | "violet" | "teal" | "rose";
...
const dimension = (
  value: Omit<SceneExpressionDimensionDefinition, "promptAssetKey" | "origin" | "enabled" | "sortOrder">,
  sortOrder: number,
): SceneExpressionDimensionDefinition => ({ ...value, origin: "system", enabled: true, sortOrder, promptAssetKey: "novel.scene.expression_controls" });
...
export const SCENE_EXPRESSION_DIMENSIONS: readonly SceneExpressionDimensionDefinition[] = [
  ...
  ], invariants: ["不得增删或调换事件", "不得跳过必要因果", "不得用新冲突制造快节奏"] }, 1),
  ...
  ], invariants: ["不得删减必要信息", "不得制造无意义断句", "不得改变事件推进来代替语言节拍"] }, 2),
  ...
  ], invariants: ["不得新增物件、证据或环境规则", "不得新增人物能力", "不得加入影响推理方向的信息"] }, 3),
  ...
  ], invariants: ["不得切换人称或视角人物", "不得进入无权知道的内心", "不得把推测写成事实"] }, 4),
  ...
  ], invariants: ["不得模仿具体作者", "不得用比喻确认新事实", "不得覆盖本书文风或人物口吻"] }, 5),
] as const;

export function cloneSceneExpressionDefinitions(definitions: readonly SceneExpressionDimensionDefinition[] = SCENE_EXPRESSION_DIMENSIONS): SceneExpressionDimensionDefinition[] {
  return definitions.map(item => ({ ...item, bands: item.bands.map(band => ({ ...band })), invariants: [...item.invariants] }));
}

export function sceneExpressionDefinition(key: SceneExpressionDimensionKey, definitions: readonly SceneExpressionDimensionDefinition[] = SCENE_EXPRESSION_DIMENSIONS) {
  return definitions.find(item => item.key === key);
}

export function sceneExpressionBand(key: SceneExpressionDimensionKey, level?: SceneExpressionLevel, definitions: readonly SceneExpressionDimensionDefinition[] = SCENE_EXPRESSION_DIMENSIONS) {
  return level === undefined ? undefined : sceneExpressionDefinition(key, definitions)?.bands.find(item => item.level === level);
}

export function deserializeSceneExpressionDefinitions(value?: string | null): SceneExpressionDimensionDefinition[] {
  if (!value) return cloneSceneExpressionDefinitions();
  try {
    const parsed = JSON.parse(value) as unknown;
    if (!Array.isArray(parsed) || !parsed.length) return cloneSceneExpressionDefinitions();
    const valid = parsed.every(item => {
      if (!item || typeof item !== "object") return false;
      const definition = item as Partial<SceneExpressionDimensionDefinition>;
      return typeof definition.key === "string" && typeof definition.label === "string" && typeof definition.description === "string"
        && typeof definition.enabled === "boolean" && typeof definition.sortOrder === "number"
        && Array.isArray(definition.bands) && definition.bands.length === 5 && Array.isArray(definition.invariants);
    });
    return valid ? cloneSceneExpressionDefinitions(parsed as SceneExpressionDimensionDefinition[]).sort((a, b) => a.sortOrder - b.sortOrder) : cloneSceneExpressionDefinitions();
  } catch {
    return cloneSceneExpressionDefinitions();
  }
}

export interface SceneExpressionTrackCatalog {
  revision: number;
  definitions: SceneExpressionDimensionDefinition[];
  updatedAt: string | null;
}

export interface SceneExpressionTrackCatalogSaveRequest {
