...
`new-design/` 使用独立 PostgreSQL 正本、模型路由和不可变诊断回执，不调用旧版 Creative Hub Runtime。从首页、本书、章节或运行记录进入时，深链携带作品、章节和原任务的稳定标识；只复用绑定完全相同的当前会话，否则建立新会话。更换作品时必须清空原章节和原任务范围。

新设计前端保留旧版的持续会话、资源绑定、上下文侧栏、单一推荐、快速问题、历史证据和归档恢复价值。流式分支、对话内审批和业务写入不直接复刻：每次问题冻结当时正式状态，原键只恢复原回执，业务动作通过诊断结果回到具有完整上下文的新版正式工作台。
