...
## 新设计 PostgreSQL 上下文装配

`new-design` 的 037 迁移把上下文选择拆成三个阶段：版本化规则先确定候选，装配预览再计算激活条件、去重和 Token 预算，受控运行最后冻结只读快照。系统、任务组、任务节点、书、卷、章、场景和本次调整按固定层级合并；智能视图与 pgvector 既有检索轨迹只提供候选，不能绕过规则直接写入正式运行。

> 🏠 **白话比喻**：规则像选菜标准，manifest 像这次真正装进餐盘的逐项小票。对应到上下文系统：检索像服务员递来的备选菜单，预览负责选菜和核算份量，运行快照才是这次真正上桌内容的凭证。

> 🧠 **速记方法**：**检索给候选，规则做筛选，预览算预算，快照锁版本。**

必须事实不会因预算不足被静默裁剪；缺少必须来源或必须事实超预算时，预览失败关闭。来源换版会让尚未运行的预览进入需重新预览状态，但历史运行快照保留原确切版本与原因。详见 [上下文绑定、装配预览与运行快照](../../../new-design/docs/context-management-and-assembly.md)。
