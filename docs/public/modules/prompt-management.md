...
新设计的提示词配方不直接查询小说资料。高级设置中的上下文规则先按任务与书籍范围选出候选，装配预览列出确切版本、装配位置、优先级、必须事实、参考资料、Token 和每项决定；正式运行开始前，再把任务合同、提示词配方、模型路由和最终资料版本一起冻结为运行快照。

> 🏠 **白话比喻**：提示词配方像厨师的做菜步骤，上下文规则像选菜标准，manifest 像这次真正装进餐盘的逐项小票。对应到系统：配方声明怎么使用材料，规则决定哪些材料有资格进入，快照证明这次实际用了哪些版本。

> 🧠 **速记方法**：**配方管怎么说，规则管看什么，快照管这次用了什么。** 必须事实不足或超预算时停止运行，参考资料才可以按规则裁剪。
