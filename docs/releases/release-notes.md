...
- 新版增加独立“创作中枢”入口：可建立持续诊断会话，绑定作品、章节或原任务，从本书、章节工作台和运行记录带当前对象进入，并复用相同范围的会话。中枢保留每次问题、冻结状态与回复，提供唯一下一步、阻塞、正式来源、快速诊断、诊断证据及会话归档恢复。中枢不直接生成或保存正文、不采用候选、不审批或取消业务任务；数据库迁移仍须明确安装后才能使用。
- 新版世界样本库增加独立生成与深化工作区：可从灵感、模板、属性和固定参考版本建立候选，保留人工修订、重新生成与六层深化历史；只有明确发布才会生成公共世界固定版本，并同步保留规则、势力、地点和结构化关系。对应数据库迁移仍须明确安装，真实模型与作者页面操作尚待验收。
