...
- 新版书内世界使用范围支持从正式世界资料保存人工候选、获取 AI 整理建议、明确采用版本，并在来源变化时阻止旧范围继续供人物、规划与章节生成使用。AI 建议要求会保留未提交填写，只保存候选，原请求结果未知时可按原键核对；106 手动结构默认关闭，未安装作者库，尚未进行真实模型与作者页面验收。
- 卷战略首屏加入全书伏笔兑现压力账本，按已采用规划和稳定章节的正式结算区分待兑现、紧急、逾期与已回收；目标窗口可用原请求编号恢复。107 手动结构未安装作者库。
