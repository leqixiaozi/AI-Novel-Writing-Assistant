...
- 新版模型设置可分别选择 OpenAI 兼容、Anthropic Messages 兼容和本机 Ollama；OpenRouter 走 OpenAI 兼容路线。连接检查只说明模型目录和名称是否可读取，不再把列表命中误称为生成可用。各接口回复统一进入创作合同校验与用量记录。当前作者服务尚待加载新协议代码和凭据，本条不代表在线创作已通过。
