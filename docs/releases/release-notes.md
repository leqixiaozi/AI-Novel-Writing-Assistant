...
- 整章生成先从当前配置提炼带来源的主干检查项，再最多生成并核对三轮候选。系统综合配置错误、主干不确定项和篇幅距离选择候选并自动载入，候选仍需作者阅读和明确采纳。
- 生成候选会显示配置核对结果：未发现明确错误、仍有问题待处理或核对未完成。生成与后续独立核对复用同一份背景快照，避免配置变化后使用两套依据。
- 写法合同、章节表达控制、人物关注和场景表达轨道只向模型发送具体写作要求，不再发送百分数、档位名称或内部级别；对白控制会保留谈论事项。
- 全量章节写作保留完整文风合同，避免上下文摘要只留下前段叙事规则而漏掉人物、语言、反 AI 和自检规则。
- MiniMax 内置模型列表补充 MiniMax-M3。

#### 修复

- 章节人物只读取任务或场景结构明确关联的本书参与者；关系状态按章节边界取最新记录，本章旧稿状态、未来关系计划和未分章秘密不会作为新正文开场事实。
- 配置核对逐项验证必须动作和禁止项。正文缺少动作、出现相反事实、审核没有可定位原文或未进入配置篇幅范围时，候选会保留为待处理，不再误报通过。
- 重新生成只接收当前配置与上一轮问题，不把失败稿当作续写素材；三轮都未通过或审核服务异常时仍保留最佳候选，不覆盖正式正文。
- 全书编排的表达轨道与人工调整的表达档位不再直接显示 `L1—L5` 和百分数等内部参数，作者看到中文表达方式和具体执行要求。

### 2026-09-13（待发布）

#### 新增

- 全书编排新增线索与伏笔生命周期：线索池可筛选，六阶段节点沿统一章节轴展示；计划用虚线、正文记录用实线，节点可关联原事件和原场景，未来计划节点可拖到待写章节后进入原候选预览与应用。
- 生命周期风险诊断检查缺少回收、中段断档、章序倒置、已写章节残留计划、失效引用和正文证据定位。正文记录保存精确原文证据，不复制章节事实。
- 提供独立的“全书编排”工作台：选择作品后，沿统一章节轴查看卷段、事件、场景、人物参与和五种表达目标，支持全书概览与章节窗口切换。
- 编排草稿可保存恢复；可排除锁定章节、比较有效要求后应用所选范围，也可让 AI 根据编排生成大纲候选，比较后再选择采纳。
- 卷段支持点击编辑名称、目标、主角变化、高潮与下卷衔接，拖动移动或调整起止章节；草稿预览展示归属冲突和影响范围，明确应用后更新卷规划。
- 事件、场景、关系阶段和伏笔支持右侧详情、来源查看及修改影响预览；场景可排序和移章，历史快照保留为只读依据。
- 章节内场景新增彩色篇幅分配条和场景卡编辑：支持拖动相邻预算、排序、拆分、合并、新增、删除，以及在右侧调整故事安排、节奏表达和人物边界；先预览再整章应用，不改已有正文。
- 人物计划区段支持移动、拉伸、复制、拆分和合并。新增五条通用场景表达轨道：同章多个圆形场景点共用一个章节列，以空心到全填充表示 L1—L5，可上下拖动或右键精调；书级“用于后续写作”默认关闭，明确启用后才由正常章节生成读取，只改变节奏、句段、细节、镜头距离和修饰程度，不改变剧情资料。原有章节表达参数入口继续保留。
- 可从章节或具体核对问题发起复查、生成修复候选，并沿原稿件比较、手改、核对与采纳流程完成修改；未完成编辑可在关闭后恢复。
- 小说规划页和章节编辑器提供可选的人工调整面板，可选择表达档位、保留要求、章节范围及本书／本章默认写法。
- 支持独立保存候选和手改稿、比较正文、按实际稿件核对、定位历史依据及明确采纳；未使用人工调整时继续原创作流程。
- 支持指定章节接管、结束交接和导演续接；可查看采纳后的资料同步状态，并重试未完成部分。
- 支持多章规划预览和选择采纳、范围干预启停，以及共享事件的人物／时间调整与影响回查。
- 增量数据库迁移随代码提供；真实模型的文学效果评测另行记录。

#### 优化

- 全书编排使用底座原导航、主题和表单组件，保留蓝橙卷段与多色人物区段；对象编辑、依据和预览集中在右侧，主章节轴保留当前位置。
- 顶部章节导航支持连续拖动窗口、逐章键盘移动和首尾定位；默认展示八章，各条轨道共享同一章节轴。
- 章节导航两端可直接拖动，在 4—16 章之间扩大或缩小当前窗口；上方卷段缩略可快速跳到对应卷。顶部操作按整书、当前卷、当前章分组，次要操作收入“更多”，事件和场景标记支持悬停摘要。
- 章节表头显示数字加名称，长名称可悬停查看；卷段拖动支持章节吸附、边缘滚动、键盘调整与撤销，保留蓝橙色区分。
- 关系阶段、线索伏笔和核对结果复用作品原有资料，显示来源与记录性质；没有资料时保留空态。
- 人物关系图新增自动排布，可在拖动后恢复清晰位置；多条关系按目标方向使用人物卡四周的独立连接点，不再挤在同一端口。人物卡和关系标签都可直接点击，右侧就地修改的人物资料或初始关系会保存回当前作品，并供后续章节规划读取。
- 章节表头改为整格点击并在纵向浏览时吸顶，横向滚动仍与矩阵章节列同步；场景表达轨道移除重复章节行，直接复用同一表头定位场景点。
- 全书、当前章节、批量章节和单个场景操作显示明确范围标识；常驻批量范围选择已移入对应操作面板，页面顶部重复的场景编排与场景表达轨道入口已收口到章节右键菜单和全书入口。

#### 修复

- 核对结果的弹窗标题、详情标题和悬浮摘要改用中文问题名称，不再向作者直接显示内部英文规则键；未知规则会根据中文核对摘要生成可读标题。
- 修复部分右侧编排面板在请求期间点击顶部叉号或底部“关闭”没有反应的问题；显式关闭现在只移除当前面板层，未保存编排草稿仍保留。
