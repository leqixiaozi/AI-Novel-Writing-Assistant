...
- 私有运行时、上下文装配、书籍概览与规划中心本批仅完成代码和静态审查；仓库当前没有已验收的 PG17 `age.dll` 与 `vector.dll`，所以没有生成合格安装包。干净 Windows 安装、真实 initdb／启停、扩展加载、001—038 实库迁移、上下文与规划端到端运行、备份恢复和升级回滚仍属于 Release Gate。
