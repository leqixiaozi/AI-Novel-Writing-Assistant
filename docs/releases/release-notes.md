...
- 创作概览、故事规划和章节目录统一展示稳定章节、待结算、结算需处理与待看改稿影响；中途关闭确认面板后可以继续同一会话。迁移 SQL、运行包清单与数据同步文档同步到 001—041。
...
- 私有运行时、上下文装配、书籍概览、规划中心、章节创作工作台、章节稳定结算与旧章选择性重算本批仅完成代码和静态审查；仓库当前没有已验收的 PG17 `age.dll` 与 `vector.dll`，所以没有生成合格安装包。干净 Windows 安装、真实 initdb／启停、扩展加载、001—041 实库迁移、上下文／规划／章节创作端到端运行、备份恢复和升级回滚仍属于 Release Gate。
