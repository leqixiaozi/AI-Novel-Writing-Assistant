...
- 新版新增独立“漫画工作台 Beta”的项目入口：按来源、内容、形态、画风四步创建，支持小说已采用正文导入、原创灵感和本地文本；来源冻结为可追溯版本，重复创建可按原请求核对。分集大纲、整话分格脚本和角色／场景文字设定均可人工保存候选并明确采用，旧版本不被覆盖。角色与场景还可上传视觉图片候选、保留历史版本并明确采用；AI 自动生成分格图片和导出仍待接入。
...
- 新版漫画项目详情按旧版工作区习惯分为“大纲／角色／场景／分镜／来源”；分镜页只列已有采用大纲的分集，错误的标签或分集深链会提示，不会暗中改选。导出尚未实现，因此不展示空的导出入口。
