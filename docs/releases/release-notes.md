...
- 新版模型设置可分别选择 OpenAI 兼容、Anthropic Messages 兼容和本机 Ollama；OpenRouter 走 OpenAI 兼容路线。连接检查只说明模型目录和名称是否可读取，不再把列表命中误称为生成可用。MiniMax 的 OpenAI 兼容路线已在新版系统中生成章节候选并记录实际用量；候选存在连续性问题，尚未采用。Anthropic 与 OpenRouter 的在线任务仍待验证。
