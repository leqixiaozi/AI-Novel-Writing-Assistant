...
- 新版继续简化底层数据维护：新建环境采用纯表卡片内核，保留原页面、候选与明确采用流程，不再依赖业务兼容视图。代码与文档正在收尾，既有作者库不会随服务启动自动转换。
- 空环境补入开书模板、表单、字典和真实任务合同；模型初始化可明确选择保留原模型路线与加密凭据，不虚构默认供应商、不调用模型，也不将密钥加入版本管理。
- 后续数据库更新默认只做增量。新功能涉及数据库变化时先说明影响；重大结构或数据变更先确认，不再把整库备份、恢复和重建作为普通调试的固定步骤。
- 新版编译问题和失效依赖已修复；纯表数据库的开书、资料、规划、正文和创作中枢人工流程已在独立测试库通过。完整业务回归与页面仍待验收，尚未切换作者数据库或重启服务，不作为可运行交付。
- 新版作者数据库已在完整备份和独立恢复演练后原子安装卡片内核 v2，物理结构收敛为 79 张应用表和 4 张 AGE 投影表；作品、正文、研究原文和任务等核心记录保持一致，旧版数据库不受影响。
- 新版创作中枢、世界生成、漫画和短剧继续使用原页面与操作语义，底层稳定对象、版本、采用和关系统一进入卡片内核；服务重启后主要页面与能力入口均可读取。
- 数据库结构快照、数据模型、核心架构和恢复说明已同步。当前兼容视图继续承接尚未改写的业务仓储调用，因此这次更新只确认物理结构完成收敛，不把内部兼容迁移期描述成全部逻辑改写完成。

#### 修复

- 最终迁移为恢复后的数据库函数固定解析范围，避免服务重启后兼容写入因找不到卡片内核对象而失败。
- 空库和已有作者库升级现在使用同一套字段选项同步逻辑，避免兼容视图上的冲突写法阻断首次安装或升级。
...
- 新版完成漫画视觉生产链：角色／场景可基于已采用设定生成三视图、表情、服装和场景图候选；整话可按采用分镜逐格成图、记录失败停止位置并明确采用。项目可冻结采用图片后导出文字气泡预览、无字原图或项目版本清单，来源变化会拒绝旧预览。
- 新版新增独立短剧工作台，覆盖来源冻结、改编策略、人物、分集、台本、质量检查、修复候选、分镜、媒体提示词、供应商任务状态和 SRT／timeline JSON／项目 Markdown／JSON 导出；所有生成结果先进入候选，未配置媒体供应商时不伪造成功。
- 新版漫画工作台接入 AI 来源整理、分话大纲和整话分镜候选：生成时冻结当前原文、已采用来源整理或正式分话版本，保留原请求编号供只读找回；结果只进入候选列表，仍需作者明确采用，正式分话变化后旧分镜会标记为过期且不能采用。
- 新版增加独立创作向导，按创作环境、灵感与方向、开书准备、生产方式和首章成稿显示真实进度；未读取状态不会被当成完成，每次只推荐一个可回到正式来源的下一步。
- 新版高级设置增加系统设置总入口，可集中进入模型任务、上下文、知识、质量规则、运行记录和运行维护；界面密度与减少动态效果只在本次浏览会话中保留。
- 新版增加独立“创作中枢”入口：可建立持续诊断会话，绑定作品、章节或原任务，从本书、章节工作台和运行记录带当前对象进入，并复用相同范围的会话。中枢保留每次问题、冻结状态与回复，提供唯一下一步、阻塞、正式来源、快速诊断、诊断证据及会话归档恢复。中枢不直接生成或保存正文、不采用候选、不审批或取消业务任务；本机作者开发库已启用对应数据层并完成未绑定会话的新建、归档与读回。
- 新版世界样本库增加独立生成与深化工作区：可从灵感、模板、属性和固定参考版本建立候选，保留人工修订、重新生成与六层深化历史；只有明确发布才会生成公共世界固定版本，并同步保留规则、势力、地点和结构化关系。本机作者开发库已启用对应数据层和发布门禁，本次未调用真实模型生成候选。
- 新版漫画项目入口按来源、内容、形态、画风四步创建，支持小说已采用正文导入、原创灵感和本地文本；来源、分集、分镜、设定、图片与导出均保留可追溯版本和原请求回执。
- 新版世界样本库支持可恢复归档：归档样本不再供新书导入或继续发布，已导入作品、固定版本与来源记录保留；查看归档样本后可明确恢复。作者环境尚待启用和页面验收。
- 漫画来源页增加人工梗概、情节节拍和角色线索整理：每次保存均为候选，明确采用后保留原文及旧版，未知结果可按原请求找回。

#### 优化

- 新版创作首页增加“创作提醒”和“最近作品”，在主推荐之外集中展示等待确认、失败或质量问题，并为每本近期作品给出直接继续的正式入口。
- 书内八步与辅助入口改为应用内切换并缓存左侧流程状态，减少换页时导航闪烁；离开未保存页面仍需确认。短剧创建、候选、采用、台本、质量、修复和分镜写入会保留原请求，未知结果先核对再决定是否按原输入重试。
- 新版创作资源总览补齐题材基底、推进模式、标题、写法、反 AI、知识、世界、人物、视觉、提示词、市场雷达、拆书、研究记录和参考包入口；资源仍由各自工作区维护，不另造数据或采用流程。
- 新版书架筛选和继续创作入口只使用正式作品约定与已采用故事总纲中的形式、发布分类、原创／续写、平台及创作体验；来源未确认的同值信息按未设置处理，避免把普通资料误作正式分类。
- 新版漫画项目详情按“大纲／角色／场景／分镜／预览与导出／来源”组织；分镜页只列已有采用大纲的分集，错误的标签或分集深链会提示，不会暗中改选。
- 漫画项目创建、分集、整话分格及角色／场景设定在本次浏览器会话中保留完整原请求输入；失败后只允许按相同输入重试，只有旧请求键而无原输入时仅可只读核对，不会误用刷新后的表单重发。采用分集版本前也会确认未保存填写。

#### 修复

- 新版创作中枢在共同暗色外壳中不再出现白底白字；空会话状态改为居中提示并收紧无内容高度，写入门禁改为统一警示面板，同时移除页面内重复的主区域语义。
- 新版模型任务兼容 DeepSeek Flash、Pro、V4 Flash、V4 Pro 与 Reasoner：需要结构化结果时会关闭思考输出，减少模型只返回思考内容而没有可用表单结果的情况；其他兼容接口不受影响。

### 2026-09-20

#### 新增

- 新版创作资源增加独立的世界样本库：可浏览已发布世界的固定版本、分区和关系，明确选择目标书与世界档案后进入原有导入预览；也可从目录选择一本书，进入原有公开候选、发布或同步更新流程。没有档案时可先创建，查看和选择不会自动导入或覆盖本书内容。
- 新版视觉资源库可跨书浏览封面与插图，按名称、说明、书名、类型和图片来源筛选，继续加载更多素材；可见原图状态、候选／已采用版本，并返回所属书籍管理确切版本。跨书浏览不会自动复制或绑定图片。
- 新版书内世界使用范围支持从正式世界资料保存人工候选、获取 AI 整理建议、明确采用版本，并在来源变化时阻止旧范围继续供人物、规划与章节生成使用。AI 建议要求会保留未提交填写，只保存候选，原请求结果未知时可按原键核对；没有可选正式资料时不会发送模型请求。作者环境已启用世界范围能力，真实模型与作者操作仍待验收。
- 卷战略首屏加入全书伏笔兑现压力账本，按已采用规划和稳定章节的正式结算区分待兑现、紧急、逾期与已回收；目标窗口可用原请求编号恢复。对应结构已在新版作者环境安装，未替作者填写或采用目标窗口。
- 知识参考列表提供继续读取更早资料与精确搜索结果定位，避免历史资料因首页截断而不可达。
- 新版模型设置可将服务密钥加密保存在新版数据库，原有模型路线与凭据引用保持可用；模型目录只显示凭据就绪状态，不回显密钥。文字、图片与知识索引连接都使用同一凭据引用。
- 新版首页参照旧版展示创作向导、继续创作、阶段进度、作品封面及资产状态，数据仍来自新版作品和任务。
- 新版书内项目设定和故事规划在页面开头说明当前创作任务；项目设定优先呈现读者承诺与目标篇幅，规划页显示已采用的规划数量。
- PC 书内八步侧栏分别显示正式采用进度与本步待补项，并提供下一步入口；故事、卷、章节规划先呈现当前对象、前置条件和专用字段缺项，多列表总览改为按需展开。
- 1280px PC 章节写作默认收紧书内侧栏，手动展开状态在本次会话保留；正文与卷章目录居于工作区，资料和 AI 操作置于右侧。
- 质量步骤以所选章节的采用正文和原诊断回执展示状态：无记录可直达该章诊断，有待处理问题可携原问题进入既有修复流程；概览优先展示来源明确的下一项缺口。

#### 修复

- Windows 桌面版更新至 `0.4.27`。兼容 DeepSeek 最新模型名 Flash / Pro；用这些模型做开书简报、卷战略等结构化任务时，系统会像原来的 V4 模型一样先关闭思考再生成结构结果，减少空转后返回空白内容。
- 新版质量页的章节选择优先采用正式正文档案的章节顺序，未建档章节按标题编号兜底；已登记正式修复的章节显示“待复检”，不再误作无问题；到章节修复的深链核对原报告绑定并突出原问题，无效或跨章节来源会明确报错。
- 新版首页把有记录的创作环节显示为“已开展环节”，不再误作全书完稿进度；正式来源需要复核时计入“等待你确认”，下一步直接进入本书来源复核页。
- 本书内容设置与内容类型页首次打开即可读取已发布字段，不必手动重试；质量页按章节数字顺序选择章节，正确识别已采用正文并提供对应诊断入口。
- 新版资源树、选项关联和上下文编辑在切换对象或离开页面前保留未保存填写；标签保存与写入中的重复操作受到保护。
- 拆书分析请求使用原请求编号恢复；研究记录支持归档筛选、恢复和不可变来源追加版本，避免重新发送已确认或结果未知的操作。
- 人物档案的分组表单保留隐藏组的原字段值，字段校验失败时可揭示对应分组；伏笔账本的数据库读取改为同一事务内顺序查询。
- 新设计菜单在开书前标题等子页面只标记当前入口；书内“完本与导出”等辅助页面显示所在位置，本书设置与八步流程分别标记。
- 书内八步创作导航与项目菜单可分别切换；八步创作时收起辅助工具，进入辅助页面后仍可找到当前入口。
- 世界与角色设定在未指定对象时打开首个有效档案；无效的指定对象仍提示错误，未保存填写不被自动切换覆盖。项目设定的版本与来源信息移至次级区域。
- 章节工作区的目录、正文、资料、候选切换在中等宽度窗口实际控制可见面板，不再只改变按钮样式。
- 多维视图显示具体视图名称与当前对象；明确指定但无效的对象不再回退到上次选择。质量导航在未读取具体章节报告时不宣称已检查或通过。
- 新版图投影接入后台执行：首次建立图世代、增量同步、中断后恢复和已归档书籍跳过均按来源核对；修复书籍节点错误读取旧字段导致首次构建失败。
- 新版依赖人工复核请求在后台按业务待判断结束自动执行，原请求保留于“来源变化与复核”；作者可检查变化并填写风险与原因后明确保留旧来源，运行维护不再提供误导性的通用重试。
- 新版后台会核对已结束的章节 AI 请求：8 份已有正文候选只补齐回执，2 次原失败保留为业务失败；运行维护不能把这类回执当作新模型请求重试或重放。
- 新版页面可在旧版服务未启动时独立进入；旧版共用请求的失败不会反复弹窗遮挡新版顶部操作，旧版页面仍按原启动状态等待其服务。
- 新版运行维护按实际加载的后台处理器显示受限状态，按书查看作业时可读到超过前 50 条的记录，避免将待处理任务误报为正常。
- 新版书内表单与规划编辑区为字段留出左右空间，窄屏页面保留内容边距；创建、资料审阅和章节结算的底部操作随内容排列，不再压住填写区域。

### 2026-09-19

#### 新增

- 新版热门题材雷达沿用旧版的三段操作：选择平台和榜单作品、在分析结果中勾选并收藏市场信号、从所选或已收藏题材进入开书。开书表单会带入市场方向和确切研究来源；原有填写不会被入口覆盖。
- 人物与作者谈话可明确带入、不带入或撤销创作倾向，限定后续章节范围，并保留原谈话和只读指导版本。
- 开书前可准备中文标题组，存入标题库、筛选使用记录或移出；采用书名时须明确选择小说并确认。
- 本书历史可保存规划和正文、逐项比较，并通过原规划采用、正文候选与结算流程恢复；后来新增的章节和原稿版本保留。
- 图片支持完整资料草稿、中英文优化及发送选择，封面可管理主图与替补；书架读取正式分类来源，简易创作展示审校并保存模式偏好。

#### 修复

- 新版市场雷达参照旧版收紧顶部导航、扫描信息和结果间距，打开榜单即可在首屏看到并排作品；榜单内部滚动，来源与历史仍可展开。分析内容留出两侧边距，创作操作条不再遮挡信号卡片；统一入口不再因开发热更新连接周期性断开而反复刷新。
- 市场分析候选参照旧版以并排卡片展示，类别、热度、拥挤度和趋势使用中文标签；完整来源可展开查看，确认保存按钮保持横排可点。
- 市场分析可对原输入明确新增运行版本；已配置的 MiniMax 兼容连接完成 15 条作品分析，生成 18 条待确认候选，失败版本与新结果分别保留。
- 本地统一入口保留旧版顶部 AI 实况、模型选择和旧版菜单，同时展示新版菜单；从新版书架进入书籍后，顶部和两组菜单保持一致。新版与旧版 API 仍分别连接各自服务；书架只高亮“我的书籍”，卡片继续按钮恢复横排。
- 新版模型设置可分别选择 OpenAI 兼容、Anthropic Messages 兼容和本机 Ollama；OpenRouter 走 OpenAI 兼容路线。连接检查只说明模型目录和名称是否可读取，不再把列表命中误称为生成可用。已配置的 MiniMax 兼容路线在新版系统中生成章节候选并记录实际用量；候选存在连续性问题，尚未采用。独立的 Anthropic 官方服务与 OpenRouter 在线任务仍待验证。
- 章节生成恢复核对资料内容时保留原提示词与请求身份，资料字段的存储顺序变化不会将同一原请求误判为不同内容；已失败的原调用仍需从章节来源页明确发起新请求。
- 新版完本检查现在核对已采用的卷章规划；计划章节缺正文或尚未采用正文、卷未拆章时会列出来源并阻止误登记完本。指定卷与明确选章导出不会静默漏章；检查页明确显示已建正文档案数，旧快照在来源变化后保留但失效。
- 新版章节执行页和单章链接可显示正文编辑器；正文输入、选区修改、撤销重做、保存候选与刷新重开保持可用。章节诊断未启用或模型缺少服务地址时显示具体原因，模型设置可直接打开。
- 旧版与新版可从共同本地入口访问；“开书前标题”可从新版菜单进入，旧版外壳承载两组菜单和新版页面。
- 小说列表与我的书籍只在各自对应的页面高亮；进入新版书籍工作区时，旧版小说列表不再显示为选中。
- 开书表单左侧资料目录的中文长标题按可用宽度换行，关系与规划入口保持可读，底部保存按钮不遮挡目录操作。
- 关系与规划保存成功后，旧的引用核对提醒会消失；书籍正式创建后不再显示无法操作的核对入口。
- 卷章场景规划可直接建立对应资料；回读失败时保留保存凭证和填写内容，未知请求先核对原键，迟到回复保护人工修改。
...
- 人物资源页可查看并返回原道具编辑叙事用途、摘要、预计使用窗口和限制，读者与持有人所知分别核对确认来源；模板可预览后逐书新增这些可选策划字段。
- 人物时间线可从明确选择的小传准备单人或批量经历候选，保留原文出处与未知日期；选择本书事件后检查、预览、保存时间提案，再单独审核，原正文与人物状态保持。
- 人物资源页可按实际持有关系与状态维度查看已确认来源，从可编辑章节的采用正文准备资源变化候选，在原章节审阅并结算；未知数量保持未确认。
- 人物外显页可补全单个或批量角色资料，也可调整已有外显；逐字段比较后载入原表单保存，或明确写入已选批量结果，保留其余人工填写。
- 开书模板可准备体态、穿着、标志细节、声音口吻与登场印象字段，旧书逐书预览后新增，原样貌、语言与动作保留。

- 开书模板可准备人物与世界的关联表单；本书可预览安装、明确选择来源版本，并逐个升级既有关联，保留原引用、局部补充与历史。
...
- 查看资料目录、研究记录或缺失来源时保持研究任务原状态，页面访问不会替作者恢复或结束其他研究运行。
- 经历候选与时间修改遇到响应中断时保留完整原请求，可只读核对当次结果；人物来源变化会阻止旧候选保存，同一候选不会重复创建时间提案。
- 资源回填响应中断时保留完整原范围与凭证，可只读核对当次结果，避免重复生成或把其他角色与章节的结果带入。
- 外显批量保存遇到其中一项失败时整批保留原内容；响应中断或刷新失败可核对当次结果，运行记录返回原人物的外显页。

- 故事设定的关联安装与升级待核对时保留原位置，切换对象、详情或页面需先完成原操作核对。

- 关联资料及来源编辑可在原页面核对当次操作；响应中断或刷新失败时保留填写与原凭证，并发变化需比较后确认，防止重复写入。

- 旧版左侧菜单直接在当前端口打开目标页面，便于从新版工作台返回小说、世界和系统设置；桌面版保留原有导航方式。
...
- 修复本机已有健康数据库镜像时仍强制访问远端仓库并重复构建的问题；服务重启会直接复用本地镜像，新机器没有镜像时才按 Dockerfile 构建。
- 修复点击“AI 规划”后提示服务暂时不可用的问题，故事、卷、章或场景的规划候选可以正常进入生成流程。
- 修复展开“高级设置”时页面变为空白的问题，并在桌面端与移动端补齐“运行维护”入口。
- 修复“完本与导出”因失效引用检查字段不匹配而无法打开的问题，完本检查、导出预览和历史记录可继续正常使用。
- 修复开发环境在系统重启后只启动页面、数据请求长时间等待，以及业务页反复显示发布运行包错误的问题。
- 修复真实 PostgreSQL 连续升级时的图投影、后台任务排序和上下文唯一约束兼容问题，现有开发数据可继续升级到 001—044。
...
- “创作资源”的字典与标签改为左侧紧凑树目录、右侧编辑：系统资源、开书模板和本书独立资源可折叠，当前分组自动展开，节点支持搜索和同级排序，长目录占用更少。
- 内容类型可以为字典字段和多维标签分别设置单选／多选、指定分支、开放层数、末级限制和数量范围；表单只展示中文名称与完整路径，底层稳定身份和旧版本显示不会因改名或移动丢失。
- 创作表单的主要内容类型和允许资料改为可搜索、可折叠的中文分类树；选择资料关系后只显示兼容类型，表单与资料位置的稳定标识由系统维护，日常使用无需查看或输入英文。
- 当前开发数据库增加经过完整性校验的逻辑快照并随 Git 同步；其他开发机器可按 Docker 构建文件重建数据库环境后恢复现有创作数据，镜像和数据卷不进入仓库。
- 开发环境可通过固定镜像启动 PostgreSQL 17、Apache AGE、pgvector 和 pg_trgm，自动复用开发数据卷；发布运行包仍保持独立的严格校验。
- 业务页只提示数据服务未就绪，原始运行时诊断信息集中保留在“运行维护”，避免技术错误在多个创作页面重复出现。
- 每本书新增“创作概览”和“故事规划”主入口：概览从资料、规划、正文、事实、状态、质量和任务记录汇总真实进展，并给出来源与更新时间；缺少数据时明确显示“暂无”。
- 故事规划使用树形目录和表单组织故事、卷、章节与场景；事件安排、人物弧和伏笔安排从同一计划读取，作者无需接触底层卡片结构。
- 前三章规划链支持人工填写、候选比较、明确采用、历史回退、归档与恢复。保存修改只追加候选，多页面冲突会保留本地填写并提示服务器修订。
- 章节计划可选择视角、地点、参与者、事件、道具、组织／势力与伏笔动作，并记录必须发生、必须保留、禁止事项和预期变化；只有采用版本会交给后续章节创作，规划不会提前改写正文或正式事实。
- “高级设置”新增“上下文管理”：用规则树和表单配置系统、任务、本书、卷、章、场景等范围的参考资料，支持激活条件、优先级、必须事实、预算、去重和裁剪；普通作者查看业务表单和装配结果，无需理解底层卡片结构。
- 装配预览会逐项解释资料为何被选入、排除、去重或裁剪，并显示来源版本与预算。必须事实超预算或确切来源不可用时，正式运行入口会关闭，不会静默丢掉关键事实。
- “本书资料”可按内容类型、标签、分组目录和智能视图整理；同一资料可加入多个分组，自定义视图支持列表／表格并始终读取当前内容。创作资源使用同一套管理方式，但各书与各公共资源空间严格隔离。
- 标签、分组和智能视图归档只移除组织方式，不删除资料。资料归档会先列出创作表单、关系、事实、时间、规划、状态、素材和待重算影响；引用变化或预览过期后必须重新检查，阻塞项不会被强行归档。
- 已发布创作表单中的“参与人物、发生地点、涉及道具”等关联资料现在可以直接维护：可搜索本书资料、新建并加入、排序、移除、查看来源及历史；来源更新后会先提示，作者明确采用才切换引用版本。
- 每次关联可以单独填写称谓、立场、出场目的等局部信息，也能继续添加仅此关联可用的信息；这些内容不会写回人物、地点等来源资料。并发冲突会保留主表单、来源编辑和局部信息输入。
- 人物、世界、事件、章节等填写页现在可以直接“添加信息”：先明确只影响当前资料还是本书全部同类资料，再查看影响并确认；页面会标明核心、模板、本书新增和仅此处补充，作者无需进入底层卡片配置。
- 当前资料的补充信息可继续改名、查看历史或隐藏，稳定身份和旧修订不会丢失；本书级新增会形成该书独立的内容规格和表单版本，不会回写公共模板或其他作品。
- 书内“创作概览、人物、世界设定、剧情与事件、章节、线索与伏笔、本书资料”统一为一套业务表单体验；系统自动采用本书创作表单或当前内容规格，空白、模板和 AI 生成的正式资料都能继续手动修改。
- 保存会标明实际使用的内容规格与创作表单版本；多人或多窗口同时修改时保留本地填写，并允许读取、比较后明确采用服务器最新修订，不再静默覆盖。
- “新设计”顶层入口统一为“创作首页、我的书籍、创作资源、研究与分析”，面向维护者的内容配置收进默认折叠的“高级设置”；进入高级设置子页时会自动展开并保持当前位置可见。
- 每本书按“创作概览、人物、世界设定、剧情与事件、章节、线索与伏笔、本书资料、本书设置”组织任务，作者直接填写表单和可视化视图，无需理解底层卡片结构。
- 公共“创作资源”和单书“本书资料”明确分区，原有书籍、资料、设置和视图地址继续有效，已保存书签可以照常打开。

#### 新增

- 七种开书方式共用创建前审阅表单：可逐项编辑人物、世界观与其他资料，新增或移除预填内容，保存未填完的草稿后继续让 AI 完善；确认资料后再创建书籍。
- 新增统一字典树、多维标签树和字段模板。模板开书会复制并重映射树资源，模板升级只增加新树、新节点和新绑定；已有节点或规则变化进入冲突预览，不覆盖作者在本书中的调整。
- 字典值保存稳定节点身份和当时的中文路径快照；服务端会复核范围、层级和选择数量，节点停用前显示后代与正式引用影响，历史卡片和旧导出仍保留原显示。
- “创作概览”的“开始规划”旁新增“AI 规划”。作者可指定当前故事、卷、章或场景以及本次重点，AI 会读取本书资料与已采用上级规划，生成可编辑、可比较的候选版本；候选不会自动采用或覆盖既有规划。
- 每本书新增“完本与导出”：完本检查会冻结真实来源与问题清单，正式稿只允许从连续稳定章节生成；审阅稿可供校读但带明确标记。Markdown、UTF-8 纯文本和 DOCX 都先生成不可变预览，再由后台任务输出文件。
- “高级设置”新增“运行维护”，集中展示私有 PostgreSQL、扩展、迁移、后台作业、AI 恢复来源、整库备份、跨机书籍包和 Release Gate；发布检查没有真实证据时保持未执行或阻塞。
- 数据迁移扩展到 001—048。SQL、内置定义和数据说明随 Git 同步；开书草稿、AI 规划运行、作者正文、树资源、版本和运行账本继续通过 PostgreSQL 备份或书籍数据包跨机器同步。

- 每本书提供统一“多维视图”，以标签切换章节、人物、人物关系、事件、线索／伏笔、道具流转、状态变化、世界规则、整章对照和质量检查；各视图读取同一份 PostgreSQL 正本。
- 质量检查支持客观／主观分栏、证据与版本追溯、确认、暂后处理、忽略、误报和备注；AI 修订走统一运行预览，只生成等待比较和采用的整章候选。
- 研究结果进入书籍前会先生成可编辑采用预览。作者可以只选择需要的候选，逐条修改标题和表单内容、忽略不适用项，再统一写入本书正式资料；同一研究来源可供多本书分别采用，来源更新只提示新版本。
- “高级设置／上下文管理”新增统一运行前确认：提交 AI 任务前可核对任务要求、提示词分区、正式资料与参考资料、Token 预算、模型来源、技术故障回退和旧章换稿阻断项；确认后的冻结内容保持只读。
- 模型路由支持按具体创作任务覆盖任务组默认模型，并继续接受本书和本次运行设置；章节创作与章节变化提取已接入统一确认和 Outbox 排队，产出仍先进入正文候选或待确认变化。
- 稳定章节换稿新增持久化影响预览：作者可先比较新旧正文，再查看事实、角色所知、人物状态、关系、道具、事件、伏笔、后续正文和可重建内容的原因链与风险，逐项选择自动重算、人工复核、标记过期或保留人工纠正。
- 换稿执行增加二次确认，并从前一稳定检查点选择性重算；历史正文与历史结算完整保留，后续章节正文只显示复核提示，不会被后台自动改写。人工纠正会得到明确保护。
- 章节目录、故事规划和创作概览会显示换稿处理中、换稿需恢复与后续正文待复核；来源页面可直接确认保留正文或完成调整，失败任务继续使用既有重试／取消入口。
- 章节创作新增四步采用确认：作者先确认整章正文，再核对事实、角色所知、人物、关系、道具、事件和伏笔变化；每项变化都能展开正文证据和来源，可修改、纳入、不纳入或稍后处理，全部处理后才形成供下一章读取的稳定结果。
- AI 可按冻结的任务与上下文整理本章变化，结果先进入待确认清单。低风险自动确认默认关闭，重要与核心变化始终由作者决定；稳定章节换稿会先停在影响预览，不会静默重算后续内容。
- 创作概览、故事规划和章节目录统一展示稳定章节、待结算、结算需处理与待看改稿影响；中途关闭确认面板后可以继续同一会话。迁移 SQL、运行包清单与数据同步文档同步到 001—043。
- 每本书新增“章节创作”工作台：桌面端同时展示卷章目录、正文编辑、计划资料和候选比较，窄屏可在四个面板间切换；切换章节前会处理未保存内容，避免误丢稿件。
- 人工保存、复制改稿和 AI 续写／改写／扩写等操作统一生成不可变正文候选。AI 请求锁定章节计划、上下文、任务要求、提示词、模型路由和输入正文，生成结果只进入候选库，不会覆盖编辑区或直接成为正式正文。
- 候选区支持整章比较、复制为人工候选、刷新任务状态和准备采用确认；刷新服务器内容时保留本地未保存稿件。采用确认只冻结正文、计划与依赖来源，事实和状态结算留给后续确认流程。
- 新设计新增版本化上下文规则、装配预览和运行快照。正式运行开始前会冻结任务要求、提示词组合、模型路由、确切资料版本、检索轨迹和逐项决定；来源后续修改不会改写历史快照，尚未运行的预览会提示重新生成。
- 上下文来源可选择明确资料、内容类型、标签、智能视图、资料关系、故事范围、研究资料包、提示词组件和既有检索轨迹；智能视图与语义检索只提供候选，不能绕过规则直接进入正式运行。
- 新增字段影响预览与安全默认值规则：已有同类资料时，新必填信息必须先提供统一默认值；尚未实现的逐条补齐和关联局部编辑不会显示成可用功能。
- 新设计建立统一备份、恢复与可移植导入导出合同。整库备份统一覆盖 PostgreSQL 逻辑数据、受管附件与 manifest；单书提供继续创作和完整审计两种配置，模板与资源通过稳定标识和版本在机器间映射。
- 导入必须先做只读预检，显示兼容性、冲突和 ID 映射后再由用户确认；实际写入全新隔离区，校验通过后才发布，失败不会覆盖当前数据库或附件。
- 归档会拒绝绝对路径、路径穿越、符号链接、重解析点、大小写碰撞和异常压缩比。SHA-256 只用于完整性检查，不被当作加密。
- 备份与导入导出复用现有 PostgreSQL Outbox 和后台作业，不创建第二套队列；整库恢复没有普通接口，只允许本机维护模式的高权限合同。
- 新设计新增 Windows x64 私有运行包清单、离线 assembler、固定参数 PowerShell／CLI、受控 PostgreSQL 启停、只读状态／诊断 API，以及安装、健康和升级审计迁移 032。运行时不会从系统 PATH、注册表或外部数据库连接串回退。
- 用户数据与安装目录分离，数据库世代、附件、备份、导入、日志、凭据和临时运行区各自独立；停止操作只有在 instance token、data dir、PID 与启动时间都匹配时才执行，不会根据陈旧 PID 盲目杀进程。

#### 说明

- 本次先完成数据账本、API 边界、静态安全门禁和同步文档。受控打包／解包、数据库与附件一致性快照、原子切换和真实恢复演练仍是发布前验证项；运行组件未接入时会明确返回不可用。
- 私有运行时、上下文装配、书籍概览、规划中心、章节创作工作台、章节稳定结算、旧章选择性重算、研究采用、统一运行确认与多维质量工作台完成代码和静态审查；仓库当前没有已验收的 PG17 `age.dll` 与 `vector.dll`，所以没有生成合格安装包。干净 Windows 安装、真实 initdb／启停、扩展加载、001—043 实库迁移、上下文／规划／章节创作端到端运行、备份恢复和升级回滚仍属于 Release Gate。

### 2026-09-15

#### 新增

- 左侧新增“新设计”工作台，可以创建人物等内容类型，配置字段名称、说明、类型、必填、默认值、选项、分组和顺序，并在编辑时实时查看最终表单。
- 卡片库会按已发布的元卡片类型生成实际填写表单，支持创建、连续编辑、版本查看、归档和恢复；新增非必填字段后，已有卡片仍可继续打开和保存。
- 新设计数据使用应用自带的本地 PostgreSQL 独立保存，首次进入会自动初始化，刷新、重启和归档恢复均不会丢失卡片内容，无需安装 Docker 或系统数据库。
- 新设计提供人物、组织、地点、道具、世界规则、事件、目标、冲突、真相、线索、伏笔、悬念、剧情线、剧情节点、弧线、主题、卷、章节和场景 19 类通用资料，并可声明正文、时间、状态、关系、生命周期、创作目标和正典事实等组合能力。
- 卡片库内置原创仙侠项目《照骨山河》的 55 张生产样例，覆盖第一卷前八章和第一章五个连续场景；新机器拉取代码后也会得到同一套可编辑样例。
- 新设计新增真实“我的书籍”空间，《照骨山河》已作为一本可进入的样例书安装；19 类卡片不再依靠书名前缀做前端假筛选。
- 新增稳定字典、带方向和数量约束的关系类型、卡片挂载与可版本化卡片组表单；事件规划可组合主事件、人物、地点、道具和剧情线，并单独保存人物在本事件中的目标、立场与结果。
- 新增模板组与不可变模板版本。新建书籍会复制类型、字典、关系、表单和种子卡片快照；模板升级只安全追加非必填稳定字段，不覆盖书内定制。
- “新建书籍”新增空白、模板、一句话灵感、没有想法、市场方向、拆书参考和续写已有作品七种入口；无论从哪里开始，最终都进入同一套创作表单和本书资料库。
- 新设计补齐题材策略、推进模式、写法配置、质量规则、世界总览、能力／科技／修炼体系、种族、文化、宗教和参考资料，共 29 种内置资料规格；通用长篇模板发布新版本，既有书籍保持原样。
- 新设计新增“资源中心／创作策略”，内置 12 项可直接采用的题材、推进、写法和质量资源；可以在开书时选择，也可以安装到已有书籍，安装后形成互不回写的独立资料快照。
- 新设计新增“创作资源”三类资源树：创作策略、AI 创作资源和各书资料入口；新增唯一的“提示词组件”类型和四条中性组件，可用动态表单新建、编辑、查看修改记录、归档和恢复。
- 每本书新增章节、线索／伏笔、角色、事件／时间、世界和资源六种基础视图；所有页面共用同一份资料，支持独立调整故事时间与叙事章节、双向人物称谓以及伏笔埋设／揭示锚点。
- 故事时间、叙事章节、人物关系和伏笔正文锚点新增“影响预览—确认—统一应用”流程；确认前不改业务事实，确认后在单个事务中完成，重复或过期预览会被拒绝。
- 新设计新增“研究与分析”入口和研究记录页，可保存真实文本来源及不可变版本；市场扫描、市场分析、拆书和稿件诊断将共用同一套可追溯记录，运行结果、证据、候选采用和开书引用均能锁定精确版本。
- 新设计市场雷达可手动扫描番茄、起点和晋江的公开榜单；扫描不调用模型且按来源独立保存，失败不会清掉其他来源或旧版本。作者明确勾选作品后才启动 AI 分析，九类结论先作为候选展示，逐条确认后才保存为可追溯的市场信号资料，不会自动创建书籍。
- 新设计市场雷达补齐旧项目已验证的榜单证据口径：新书榜、新晋作者榜会标为近期机会的主要证据并优先进入默认分析样本，畅销、月票、阅读等成熟榜单只作持续需求和拥挤度的辅助验证；AI 输入保留平台、榜单和证据层级，番茄混淆字体条目会尝试从公开详情页恢复。
- 新设计“拆书”接入真实文本不可变版本，支持全文／字符范围、参考学习／续写整理／稿件诊断和快速／标准／完整预算。报告固定覆盖故事结构、人物、世界、冲突、节奏、钩子与兑现、写作技法、质量风险八维；证据保留原文字符位置和置信度，候选只能按当前已发布资料规格生成，并由作者手动新建、合并、保存为方法资源、仅作参考或忽略。
- 新设计新增版本化“研究参考包”，可组合已完成研究的精确运行版本；开书时既可直接选研究记录，也可锁定参考包旧版本。系统保存选择时的编译快照，研究候选只补空白字段，同名资料已有内容保持不变并提示冲突，建书后保留可追溯引用。
- 新设计新增章节正文不可变版本与精确文本锚点数据层。人工稿、AI 候选、修订和导入稿可在同一章节并存，AI 候选不会自动成为正式正文；用户采用、回退或再次采用时通过修订号与幂等键原子切换，并完整保留历史。锚点锁定正文版本、字符范围、摘录和哈希，切换正式版本后旧锚点会标为陈旧而不会删除。
- 新设计新增统一事实、证据、冲突与修正链。文本、数值、布尔、JSON 和卡片引用事实都先进入提案，证据只能精确指向正文锚点、卡片版本或研究证据；相互矛盾的事实并存等待人工处理，不会自动覆盖。确认、驳回、修正取代和正文切版引发的陈旧状态都有审计记录。
- 新设计新增可配置章节状态结算。人物数值、人物关系、道具持有／失去和伏笔生命周期均从版本化初始值出发，AI 只能提交带正文与原因证据的变化提案；用户确认当前正式正文后才形成不可变状态流水。撤销结算或切换正文会作废对应流水并重建当前投影，初始、卷末、重大修订、正文切换和手工里程碑均可留档，数值区间语义还能锁定提示词组件版本。
- 新设计新增人物与读者知情状态。客观事实、人物相信／怀疑／误解和读者已知内容分开保存，AI 结果生成后立即成为可编辑草案但不会自动确认为正典；用户可修订、确认或驳回，按叙事位置查询会阻止后续章节信息提前泄漏，正文切版会保留并标记旧认知链失效。
- 拆书等研究 AI 候选生成后会立即入库并保存初始版本，用户在采用前可以修改标题和表单字段；每次修改新增候选版本并检查修订号，不会覆盖历史或已处理候选。
- 新设计新增完整故事时间、并行与因果底座。事件可以记录绝对日期、自定义历法标签、相对时间、部分未知或完全未知范围，并分别保存计划发生、实际发生、取消和失效状态；同一事件可跨章讲述，同一章也可容纳多个故事时间点。
- AI 提取的故事时间和事件关系会自动保存为可编辑草案，用户确认后才进入正式时间线。系统支持前后、同时、重叠、包含以及导致、促成、阻止、依赖关系，并可查询时间范围、并发事件和因果上下游；正文切版会让依赖旧稿的记录失效但保留历史。
- 新设计为故事、卷、章和场景建立独立规划版本。人工稿、AI 候选、修订和正文反向建议会保留完整历史，只有作者采用的版本才成为后续执行依据；采用、回退和再次采用均留有可追溯回执。
- 上级规划切换版本后，依赖旧版的下级规划、故事时间和因果关系会进入待复核状态，不会被静默改写；章或场景规划变化只提示正文可能受影响，不会直接覆盖已经采用的正文。
- 新设计新增可追溯的 AI 执行合同：每次生产可锁定任务要求、提示词组合、实际采用与排除的上下文以及最终模型路由。后续调整配置不会污染旧记录，技术故障回退与内容质量判断分开处理，模型密钥不会进入公开执行数据。
- 新设计新增通用 AI 任务、步骤和尝试记录。技术故障可按既定次数与等待时间恢复，内容不满意只会暂停并等待用户明确重开；进程中断后可以从最后安全位置接续，旧执行结果不能覆盖新尝试，审批和用量也会保留历史。
- 新设计新增可追溯质量检查记录。每份报告会锁定被检查的正文、规划、事实和当次 AI 运行依据；问题证据、修复建议、人工接受／驳回与复检结果均保留历史。修复建议先作为候选保存，不会直接覆盖正文，正文或依赖换版后旧结论会自动标为过期。
- 新设计新增统一依赖、影响、失效与重算账本。正式对象必须引用可验证的确切版本，上游切版会保存直接／间接传播路径并建立待重算请求；循环、跨书和重复依赖会被拒绝，迟到重算结果不能覆盖更新的陈旧状态。
- 新设计补齐可重建的关系查询投影底座。人物、事件因果、线索、道具、地点、规划、研究与附件关系只读取当前有效版本；重建完成并核对数量后才切换到新索引，失败仍保留旧索引可用。关系表继续保存唯一原始记录，图索引不能反向改写小说事实。
- 新设计新增 PostgreSQL pgvector 语义检索底座。卡片、正文、事实、状态、故事时间、规划、研究、AI 结果、质量证据和附件解析文本均锁定精确来源版本后再分块；多模型、多维向量按书籍建立独立索引世代，覆盖率核验通过后才切换，来源换版、失败重试、迟到回执和逐条召回原因均保留记录。
- 新设计新增 PostgreSQL Outbox 与后台作业底座。依赖重算、附件派生、关系投影、语义分块／嵌入／索引和 AI 任务会与专业请求在同一事务登记；后台提供受控并发领取、租约心跳、过期重领、指数退避、死信、取消、消费者／书籍暂停和人工重放，重复投递不会绕过专业账本的幂等保护。
- 新设计可统一登记封面、插图、文档、音视频等附件：相同内容可以安全复用，业务引用彼此独立；替换附件会保留旧版本，移除一处引用不会误删其他位置仍在使用的内容。
- 缩略图、文字识别、转码、抽帧、文本解析和封面变体会记录确切来源与生成参数；来源换版后旧派生结果会进入待更新状态，迟到结果不能覆盖新版本。跨机器迁移作品时需要同时备份数据库与受管附件目录。

#### 优化

- 动态卡片表单会直接提示缺失必填项、错误数字和无效字段；同时通过修订检查阻止两个页面互相覆盖刚保存的内容。
- 新设计的布局、资料表单和状态提示统一跟随主应用主题，可在墨砚、暖纸、夜航及其明暗模式下保持一致；进入本书资料时会直接打开已有的第一条资料，减少空白等待和多余点击。
- 侧栏底部新增独立的“新设计”分组，新设计入口从创作区移入该分组；创作、资产、系统和新设计分组标题均可点击展开或收起，长菜单浏览更轻松。
- 新设计采用统一导航层级：创作首页默认引导进入“我的书籍”，高级设置集中提供内容类型、选项与关联、创作表单和开书模板；旧小说工作台及其 SQLite 数据路径保持不变。
- 元卡片类型改为六类可折叠目录，支持按分类或类型搜索并保留完整路径；分类只负责管理位置，不会把字段规则隐式传给子类型。
- AI 开书会先给出三个可选方向，再预览初始资料并由用户确认；生成进度、失败阶段、重试结果和字段来源都会保存，已手动修改的内容不会被静默覆盖。
- 本书资料库把 29 项类型下拉框改为六类可折叠树，支持搜索后保留命中类型的分类路径，并直接切换对应资料表单。
- 提示词组件与小说事实严格隔离：书籍模板仍只安装 29 种小说资料；题材、推进、写法、质量、人物、世界和事件全部复用既有类型，不新增基础角色、历史事件或时间线等同义类型。
- 书籍多视图统一使用右侧资料检查器；并发修订、归档目标、跨书或非法关系、结束早于开始的故事时间都会给出可理解提示，不会产生页面私有副本。
- 拆书输出约束按用户预算收紧，快速分析也能在有限 token 内完整返回八维报告、关键证据和少量高价值候选；无法精确回到原文的摘录会自动降为低置信度，不伪造字符位置。
