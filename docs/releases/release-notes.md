...
- 新版现有开发数据已原地切换至纯表卡片内核，保留作品、正文、规划、旧版本与模型设置；移除业务兼容视图，不增加数据库，也不要求重新开书。
...
- 新版编译问题和失效依赖已修复，本地开发服务可启动，书架、规划、章节、质量、创作中枢及衍生工作台的主要内容可读取。完整写入流程与页面交互仍待验收，本次未调用真实模型。
...
- 数据库结构快照、数据模型、核心架构和交付说明已同步至当前开发状态；原有停用功能继续保持停用，不借升级自动开放。

#### 修复

- 已完成增量升级的开发环境能正确识别创作与世界发布入口，避免被仅针对空环境的检查误拦截。
