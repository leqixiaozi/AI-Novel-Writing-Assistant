...
- 新版独立创作中枢已接入持续诊断会话、作品／章节／原任务精确绑定、同范围深链复用、冻结状态、唯一下一步、快速诊断、诊断证据和会话归档恢复；共同暗色外壳中的面板、门禁提示与空会话状态已统一为新版主题。中枢可解释进度与失败、分析人工修改影响并返回已有来源页面处理，但不会直接生成、保存、采用、审批或取消业务内容。对应数据库迁移仍须明确安装。
- 新版创作首页增加“创作提醒”和“最近作品”，将等待确认、失败、质量问题和近期继续入口收在首页，不必在多个页面间寻找当前作品。
- 新版世界样本库新增独立生成与深化工作区，可从灵感、模板、属性和固定参考版本生成候选，保留人工修订、重新生成与六层深化历史；明确发布后形成包含规则、势力、地点和结构化关系的公共固定版本。对应数据库迁移仍须明确安装。
