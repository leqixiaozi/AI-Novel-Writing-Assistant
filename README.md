...
- 新版创作资源增加世界样本库与跨书视觉资源库：世界固定版本可明确选择书籍进入导入预览，也能从目录进入本书世界的发布与同步；封面和插图可按类型及来源浏览并返回所属书籍管理。浏览本身不自动写入或覆盖作品。
- 新版质量问题深链会把原问题置顶并高亮；问题跨书、跨章或不存在时明确报错，保留当前章节和修复稿。质量章节按正式章序排列，未建正文档案时按编号兜底；已登记修复但尚未复检的章节单列“待复检”。
- 新版首页将已开展的创作环节与完本进度分开显示；来源变化待复核的作品会出现在“等待你确认”，可直接返回原复核页。内容类型首次打开可编辑，质量页会按章节顺序识别采用正文并引导诊断。
- 新版资源树、选项关联与上下文编辑保留未保存填写，研究分析补齐原请求核对、归档恢复和不可变来源续版；知识参考可查看更早资料并精确打开搜索结果。
- 书内世界使用范围可保存人工候选或获取 AI 整理建议，建议要求会保留未提交填写；候选均须明确采用，来源变化会拦截旧范围。没有正式世界资料时不会发送模型请求。卷战略首屏显示只认正式结算的伏笔兑现压力账本。相关结构已在新版作者环境安装；真实模型、作者操作与全页验收仍待进行。
- 新版模型密钥可在模型设置中加密保存到新版数据库；重新启动后无需模型环境变量，原模型路线继续可用。
- 新版首页补齐旧版的创作向导、续写、阶段进度、封面和资产概览布局。
- 新设计菜单在子页面只标记当前入口，书内辅助页面也会显示所在位置。
- 书内八步创作与项目菜单分开呈现；项目设定先显示读者承诺，规划页先说明当前任务与采用进度。
- PC 创作侧栏区分采用进度与本步待补，规划页优先显示当前编辑对象和正式版本的专用字段缺项。
- PC 章节写作在 1280px 默认收紧书内导航，并将资料和 AI 操作放在正文旁；中等宽度的面板切换现在会切换内容。
- 质量页按具体章节显示原检查状态，未检查可进入该章诊断，有问题可定位原修复流程；概览先给出下一项真实进度缺口。
- 旧版服务未启动时，新版页面仍可独立打开。
- 新版运行维护按实际后台处理器显示状态，并补全按书查看的作业记录。
- 已结束的章节 AI 请求会核对原结果并补齐后台回执，不会因此重新调用模型；原失败仍显示失败。
- 新版图投影已能首次建立并恢复中断；人工依赖复核在来源页展示和处理，后台不会自动接受旧上下文。
- 新版书内表单和规划字段增加左右留白，窄屏内容保留边距；创建、资料审阅和章节结算的底部操作不再盖住填写区域。
