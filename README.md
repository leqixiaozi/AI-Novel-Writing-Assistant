...
- 开书模板可预览并发布人物性别、背景、成长方向、当前行动目标和起始处境草稿，旧书逐书预览后新增可选字段，长期目标与正式当前状态分别保留。

- 支持阅读已保存稿与正式采用稿，复制本章、下载本章及整本 TXT；待采用内容与本次版本清单随稿件标明。
- 工作台可在本书弹层查看推进记录、精确执行来源与原导演范围，并明确继续符合条件的原范围。
- 封面管理提供图库与 AI 准备视图、风格及避免内容输入；可按所选张数顺序准备 1—4 张独立候选，结果待核对时停止后续生成。
- 我的书籍提供书架与工作台视图、作品搜索、创作状态筛选、排序分页和原开书草稿入口；可阅读正式采用章节，管理封面并进入正文导出。
- 作品可移入回收站并恢复原正文和资料；运行、暂停或结果待核对的任务处理完成后才能归档，恢复不会自动启动 AI。

- 故事设定支持按组准备 AI 候选，故事规划支持为全书或指定卷章准备候选；逐项勾选字段后进入原编辑器检查、保存与采用。
...
- 人物填写页按主角、配角和未指定定位整理目录，显示选中人物摘要，提供总览、档案、外显、资源、时间线、关系、动态与智能层入口；基础字段两列、长文本三列，切换档案与外显保留填写和补充信息。
- 世界资料按总览、规则与张力、生成约束、使用范围和同步资产组织查看；关联规格不可用时明确提示，人物模拟与专业视图携带原人物来源。
- 书架采用紧凑布局和默认插画，封面卡片宽度有上限，避免被拉成长页；正在推进和待处理作品置于“继续创作”，操作集中为一排。草稿与长简介按需展开，工作台并列显示项目摘要，视图选择可记忆。
- 知识参考可选择精确段落，保留来源版本及字位范围；切换创作位置时提供保存、放弃或取消，未知保存结果保留原请求。
- 书籍与创作资源导航统一展示当前位置，开发启动及备份恢复说明补充数据保留与核对步骤。

#### 修复

- 人物补充信息按原请求核对当次保存结果；旧书升级拦截过期预览并保留未发布草稿，未安装的关联表单不会标记为已安装。

- 修复切换新版页面后界面空白、旧版菜单无法继续切换的问题；新旧页面可在同一端口来回访问，业务页面按需加载。

- 修复设定顶部保存按钮未跟随填写状态更新的问题；初始值与规划保存可核对原请求回执，部分字段保存中断时保留已成功项，避免重复提交。
- 模型配置无法使用时显示具体原因，并提供模型设置入口，便于核对生效配置及凭据。
...
[完整更新历史](docs/releases/release-notes.md)
