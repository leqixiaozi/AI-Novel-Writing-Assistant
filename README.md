...
- 新版现有开发数据已原地切换至纯表卡片内核，保留作品、正文、规划、旧版本和模型设置，移除业务兼容视图；不增加数据库、不要求重新开书。
- 后续数据库按增量更新；新功能涉及数据库调整先说明影响，重大变更先确认。普通调试不反复整库备份、还原或重建，模型密钥不进入 Git。
- 新版本地开发服务可启动，书架、规划、章节、质量、创作中枢及衍生工作台的主要内容可读取；已完成增量升级的环境能正确识别创作与世界发布入口。
- 数据模型、架构和结构快照同步至当前开发状态；原有停用功能继续保持停用。完整写入流程与页面交互仍待验收，本次未调用真实模型或媒体供应商，旧版系统不受影响。
