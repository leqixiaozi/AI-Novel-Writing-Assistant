...
- 修复“完本与导出”无法打开的问题，完本检查、导出预览和历史记录可正常读取。
- 修复开发环境在系统重启后只启动页面、数据请求长时间等待，以及多个创作页重复显示运行包技术错误的问题。

#### 新增

- 七种开书方式共用创建前审阅表单，可逐项编辑、新增或移除人物、世界观等资料，保存草稿后继续让 AI 完善，确认资料后再创建书籍。
- “创作资源”新增统一字典树、多维标签树和字段模板：左侧按系统、模板、本书范围折叠选择，右侧编辑节点并调整顺序；内容类型可配置分支、层级、单选／多选和数量规则，作者只看到中文表单与完整路径。
- 树节点改名、移动和停用保留不可变版本与卡片路径快照；开书会重映射树资源，模板升级只安全增加新节点和绑定，不覆盖本书调整。
- “创作概览”新增“AI 规划”，可按当前故事、卷、章或场景读取本书资料并生成可编辑候选；AI 不会自动采用或覆盖已确认规划。
- 本地开发可通过固定镜像启动 PostgreSQL 17、Apache AGE、pgvector 和 pg_trgm，并复用已有开发数据；桌面发布运行包仍保持独立严格校验。
- 每本书新增“完本与导出”：检查会冻结真实来源与问题清单，正式稿只从连续稳定章节生成；审阅稿带明确提示，Markdown、纯文本和 DOCX 都先预览再进入后台生成。
- “高级设置”新增“运行维护”，统一查看私有数据库、扩展、迁移、后台作业、AI 恢复来源、整库备份、跨机书籍包和发布验收证据。
- 新设计数据迁移扩展到 001—048；SQL 和说明随 Git 同步，开书草稿、AI 规划运行、作者正文、树资源、版本与运行账本通过 PostgreSQL 备份或书籍数据包跨机器同步。
- 每本书新增统一“多维视图”入口，可按章节、人物、人物关系、事件、线索／伏笔、道具流转、状态变化、世界规则、整章对照和质量检查查看同一份正式数据；查看方式不会复制小说事实。
- 质量检查区分客观连续性与主观审美建议，支持确认、暂后处理、忽略、误报和备注；AI 修订进入统一运行预览并只产出新的整章候选，正文换版后旧报告保留并标出失效原因。
...
- 模型路由支持按具体创作任务覆盖任务组默认模型，并继续接受本书和本次运行设置；章节创作与章节变化提取已接入统一确认和可靠排队，产出仍先进入正文候选或待确认变化。
- 稳定章节换稿可先生成持久化影响预览，比较新旧正文，并逐项处理事实、角色所知、人物状态、关系、道具、事件、伏笔、后续正文与可重建内容；执行前会再次核对影响数量和处理范围。
- 换稿从前一稳定检查点选择性重算，历史正文和结算完整保留，后续正文只进入作者复核，不会被后台自动覆盖；人工纠正可明确保留并持续受到保护。
- 章节目录、故事规划与创作概览会显示换稿处理中、需恢复和后续正文待复核；作者可在来源章节确认保留或完成调整，失败任务沿用既有重试／取消入口。
- 每本书新增“章节创作”工作台：桌面端并排呈现卷章目录、正文、采用计划与候选，窄屏可切换面板；章节切换会先处理未保存内容，服务器刷新则保留本地稿件。
- 人工写稿、复制改稿与 AI 协作统一产生不可变正文候选。AI 续写、改写、扩写、缩写、对白、冲突、修复和整章重生成会冻结计划、上下文、任务、提示词、模型与输入来源，结果只进入候选库。
- 候选可整章比较并进入采用确认准备；该步骤只冻结正文与依赖来源，不会提前采用正文，也不会执行事实和状态结算。
- 每本书新增创作概览和树形故事规划：概览汇总真实资料、卷章规划、正文、待确认内容、质量问题与最近任务；故事规划按故事、卷、章节、场景逐层填写，并支持前三章计划采用、候选比较和历史回退。
- 章节计划可绑定视角、地点、人物、事件、道具、势力与伏笔动作，记录必须发生、必须保留、禁止事项和预期变化；只有作者采用的版本会交给后续章节创作，规划不会直接改写正文或正式事实。
- 新设计新增上下文管理、装配预览和运行快照：可按系统、任务、本书、卷、章、场景配置参考资料与必须事实，运行前逐项核对版本、预算和选入／排除原因；必须事实不足时关闭正式运行入口，正式快照不会被来源后续修改污染。
- “本书资料”可按内容类型、标签、分组目录和智能视图整理；同一资料可加入多个分组，自定义查看方式始终读取当前内容。创作资源复用相同管理方式，各书与公共资源空间仍严格隔离。
- 标签、分组和智能视图归档不会删除资料；资料归档前会冻结引用影响并在确认时重新核对，过期、并发变化和创作主项等阻塞会明确拒绝。
- 左侧新增“新设计”工作台，可以创建人物等内容类型、组合字段并实时预览表单。
- 本书资料会按已发布内容类型生成填写表单，支持创建、编辑、查看修改记录、归档和恢复；新增非必填字段后，已有资料仍可继续使用。
- 新设计数据目标由应用私有 PostgreSQL 独立保存，无需用户安装 Docker 或系统数据库；只有数据库、扩展、迁移和许可证运行包完整通过校验后才会首次初始化，当前缺失已验收扩展二进制时会失败关闭。
- 新设计内置人物、组织、地点、道具、世界规则、事件、目标、冲突、真相、线索、伏笔、悬念、剧情线、剧情节点、弧线、主题、卷、章节和场景 19 类通用资料，并提供原创仙侠项目《照骨山河》的 55 条可编辑生产样例。
- “我的书籍”以真实 PostgreSQL 空间隔离每本作品；《照骨山河》已安装为可直接体验的样例书，并提供事件规划组合表单。
- 高级设置提供内容类型、选项与关联、创作表单和不可变开书模板；模板升级只追加安全的非必填字段，不覆盖书内定制。
...
- 新设计新增“创作资源”三类资源树与提示词组件动态表单；系统预置四条通用 AI 指令组件，同时保持任务合同、提示词配方、模型路由和运行记录不作为普通创作资料。
...
- 当前开发数据库增加经过完整性校验的逻辑快照并随 Git 同步；其他开发机器可按 Docker 构建文件重建环境并恢复现有创作数据，镜像和数据卷不进入仓库。
- “高级设置”补充“上下文管理”表单和作用范围树；维护者可配置激活条件、来源入口、优先级、预算、去重与裁剪，作者侧继续使用“参考资料、必须事实、装配预览、运行快照”等创作语言。
- 创作表单中的关联资料可直接搜索、新建并加入、排序、移除和查看来源；来源资料更新后由作者决定是否采用新版本，移除关联不会删除来源或历史。
- 每次关联可填写或增加仅本次使用的称谓、立场、目的等信息，不会写回人物、地点等来源资料；并发冲突时页面保留尚未保存的输入。
- 人物、世界、事件、章节等填写页可以直接“添加信息”：先选择仅当前资料或本书全部同类资料，再查看影响并确认；页面会说明信息来源和适用范围，无需理解底层卡片。
- 当前资料补充信息支持继续修改、查看版本或隐藏；本书级新增形成该书独立规格，不回写公共模板或其他作品。已有资料新增必填信息时必须先提供安全默认值。
- 书内创作概览、人物、世界、事件、章节、线索和本书资料统一使用业务表单；系统自动选择本书已发布表单或当前内容规格，空白、模板和 AI 资料共享编辑、保存与只读修改记录。
- 保存会记录实际内容规格和创作表单版本；并发修改会保留当前填写并提供显式比较，不会自动覆盖作者内容。标签、分组、智能视图和归档影响检查使用同一套本书资料入口。
- “新设计”统一采用创作语言：顶层入口为“创作首页、我的书籍、创作资源、研究与分析”，内容配置收进默认折叠的“高级设置”；书内导航按人物、世界设定、剧情与事件、章节、线索与伏笔等任务组织。
- 公共“创作资源”和单书“本书资料”明确分开；页面不再要求作者理解底层卡片术语，原有书籍和高级设置地址保持兼容。
- 动态表单会提示缺失必填项、错误数字和无效字段，并阻止多个页面互相覆盖刚保存的内容。
- 新设计统一跟随墨砚、暖纸、夜航及其明暗模式；进入本书资料后会直接打开已有资料，减少空白页和多余点击。
- 侧栏底部新增可收起的“新设计”分组，创作、资产、系统等导航分组也可按需展开或收起。
- 创作首页、我的书籍、创作资源、研究与分析和高级设置采用统一菜单层级；旧小说工作台和 SQLite 数据保持原样。
- 内容类型目录支持折叠、搜索和详情联动；AI 开书过程显示真实阶段，生成失败可重试或保留当前结果，作者修改不会被静默覆盖。
- 本书资料使用六类可折叠树浏览 29 种资料规格，搜索类型后可直接切换到对应资料表单。
