...
- 手工空白开书可建立并填写本书作品约定；故事总纲候选可核对、带入或修改原开书形式与目标字数，作者采用后才影响正式筛选。人物谈话可展开宽工作区并切换分析分区；创作倾向、书级历史及公共标题仍在接续，完整Windows运行包与升级保持待验。

- 公共角色可选择固定来源版本，逐项映射并预览后创建书内独立人物；同名资料保持，未映射内容会提示补齐，响应中断可核对当次导入结果。
- 公共角色样本库提供创建、编辑、收藏、归档和历史核对入口；保存保留原补充字段，刷新目录保留填写时的版本，明确核对后才改用新版。
- 公共角色固定版本可逐轮试聊，检查人物口吻及档案缺失；形象参考图可按1–4张顺序生成，明确设为主图或归档，响应中断保留原请求核对已有回复。
- 新版章节工作台提供诊断、问题修复候选和稳定正文复检入口，保留原正文及逐字证据，修订仍需明确比较、采用和结算。
- 故事规划提供章节张力曲线，可按卷查看、对照有效正文诊断与参考曲线，并将数值、节拍和人工固定点写入章节规划草稿。
- 简易创作提供阅读、正文编辑与导演进度入口；开书可选择短篇及目标字数，采用短篇可作为发展长篇的明确参考来源。
- 世界库提供本书世界公开候选、明确发布、完整资料与关系导入及主动双向同步入口；各书保留独立资料，推送候选须另行确认发布。

本批新增内容属于新版开发分支，功能仍待集中验证，尚未在现有创作环境启用。

#### 优化

- 本书目录按创作流程排列，仅故事规划展开子页面；目录可收起让出编辑空间，设定中的人物模拟、视觉与知识入口及分析中的专业图形入口清晰可达。
- 人物小传时间线同步支持超过20人的分批准备，先检查完整人物范围；中断后核对原批次并继续剩余人物，来源变化或结果未知时停止，已有候选保留。
- 编排可按卷、标题与采用状态查看卷章表；导演可选择完整章范围，并查看逐章候选、结构化决定和当前采用／结算进度。
- 题材与写作策略可筛选、编辑、比较、试验及组合安装，并按历史规格核对完整资料版本。

#### 修复

- 人物经历候选中的创建事件入口可打开正确的资料分类。
- 章节生成可正确核对初始状态与人物认知的完整来源，避免因版本读取错误中断。
- 世界操作保留未确认请求；本地记录无法安全保存时停止新提交，切换书籍后的迟到回复保留在原来源核对。
- 研究资料带入书籍前要求明确选择目标，读取或刷新列表不会隐式改选第一本作品。
