...
- 新版正在完成纯表卡片内核：保留原页面和明确采用流程，移除业务兼容视图依赖；开书模板、表单、字典与模型配置初始化同步补齐。当前作者库不会自动转换，本轮验证和启用结果以交付记录为准。
- 后续数据库按增量更新；新功能涉及数据库调整先说明影响，重大变更先确认。普通调试不反复整库备份、还原或重建，模型密钥不进入 Git。
- 新版编译问题与失效依赖已修复，纯表数据库的开书、资料、规划、正文和创作中枢人工流程已在独立测试库通过；完整业务回归与页面仍待验收，不能直接将本次提交用于作者库切换。
- 新版作者库已在完整备份和独立恢复演练后原子安装 `123`—`131` 卡片内核 v2：迁移数由 119 增至 128，`new_design` 固定为 79 张应用表，AGE 投影保留 4 张表，总物理表数 83；42 部书、正文、研究原文和任务等核心正本数量与摘要保持一致，旧版数据库未操作。
- 5273／5301 源码服务已重启，创作中枢、世界、漫画、短剧及书内主要页面无迁移门禁；创作中枢未绑定会话完成新建、局部切换、归档与读回。验收没有发送诊断问题，也没有调用真实模型或媒体供应商。
- 结构快照、数据模型、核心架构和恢复记录已同步。当前是“物理表已收敛、业务仓储处于兼容迁移期”：344 个兼容视图仍支撑未改写的业务 SQL，不能把 83 张物理表误报为运行源码已经全部切换到卡片仓储。
