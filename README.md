...
- 新设计的章节创作支持四步采用确认：先确定整章正文，再逐项核对事实、角色所知、人物、关系、道具、事件和伏笔变化；所有变化都可查看正文证据、修改或拒绝，完成结算后才成为下一章可读取的稳定结果
- AI 整理出的章节变化先进入待确认清单，低风险自动确认默认关闭，重要与核心变化始终由作者决定；稳定章节换稿会先显示影响边界，不会静默重算后续内容
- 创作概览、故事规划和章节目录使用同一稳定结算口径，中途关闭确认页也能继续原会话；SQL、运行包与跨机器数据说明同步覆盖 001—040
...
- 新版热门题材雷达可按平台扫榜、逐本选择作品、查看 AI 分析、挑选并收藏市场信号；所选方向或收藏题材能带着原研究来源进入开书表单。首屏即可查看并排榜单，分析内容保留两侧留白，底部操作不再遮挡卡片；统一入口的页面不再周期性刷新。
- 市场分析通过已配置的 MiniMax 兼容连接完成原输入的新版本并展示待确认候选；失败版本仍可核对。
- 统一入口保留旧版顶部的“AI 实况”、模型选择和温度控件，旧版与新版菜单同时显示；新版书架及书内页面沿用同一外壳。两版 API 仍各自连接原服务，书架卡片的继续按钮保持横排可读。
- 新版模型设置区分 OpenAI 兼容、Anthropic Messages 兼容与本机 Ollama；OpenRouter 沿用 OpenAI 兼容路径。《我卡世界的Bug》的章节专用路线已用现有 MiniMax 凭据切到 `https://api.minimax.cn/anthropic`，模型探测和正式章节任务均有实际成功回执及用量；独立的 Anthropic 官方服务和 OpenRouter 在线任务仍待验证。候选连续性未通过审阅，尚未采用。
- 新版完本检查核对已采用的卷章规划；计划章节缺正文、尚未采用正文或卷未拆章时给出阻断，导出指定范围不会静默漏章。
- 新版章节执行页恢复正文编辑与单章链接，支持选区修改、撤销重做和候选保存；诊断与模型条件不足时给出明确提示及模型设置入口。
- 旧版和新版共用本地入口地址及顶部与侧栏外壳，业务页面保留独立路径；新版“我的书籍”选中时，旧版“小说列表”不会同时高亮。
- 开书资料目录的长标题在桌面和窄屏正常排版，关系、规划与保存入口可继续使用。
- 关系、规划保存或正式开书后，过期的引用核对提醒不再与成功结果同时显示。
- 人物与作者谈话支持明确带入、不带入或撤销创作倾向，限定后续章范围，并保留原谈话和只读指导版本。
- 开书前可准备中文标题组，存入标题库、筛选使用记录或移出；采用书名需要明确选择小说并确认。
- 本书历史可保存规划和正文、逐项比较，并经原规划采用、正文候选与结算流程恢复；后来新增的章节和原稿版本保留。
- 卷章场景规划可直接建立对应资料，回读失败仍保留保存凭证及规划填写；未知请求先核对原键，迟到回复保护人工修改。
- 图片支持完整资料草稿、中英文优化及发送选择，封面提供主图与替补；书架读取正式分类来源，简易创作展示审校并保存模式偏好。

本批属于新版开发分支，源码编译、应用产物和相关隔离功能链通过。统一页面入口已在现有作者环境启动并核对页面可见；新增手动能力仍未安装，部分原生操作、真实模型质量及用户验收未完成。完整Windows包缺少锁定组件，真实升级维护执行器尚未实现；本地提交不代表全部能力已启用。

完整更新历史见[版本更新说明](docs/releases/release-notes.md)，新版详细交付见[新设计交付记录](new-design/docs/releases.md)，各批证据和未完成项见[逐页进度](new-design/docs/legacy-page-replication-progress.md)。
