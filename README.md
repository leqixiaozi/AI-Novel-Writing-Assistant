...
- 全书编排沿用底座导航、主题与表单组件，保留彩色轨道、计划虚线和历史实线；章节窗口可像滚动条一样拖动，编辑与预览在右侧完成。
- 章节导航窗口可直接拖动两端在 4—16 章间缩放，上方增加全书卷段缩略；顶部集中整书资料和人物关系，关系面板区分初始关系与当前章节，卷段与章节从矩阵对象进入，事件与场景悬停即可查看摘要。
