...
- 新版模型设置区分 OpenAI 兼容、Anthropic Messages 兼容与本机 Ollama；OpenRouter 沿用 OpenAI 兼容路径。MiniMax 的 OpenAI 兼容路线已在新版系统中生成章节候选并记录用量；Anthropic 与 OpenRouter 的在线任务仍待验证。连接检查只确认目录和模型名称，候选仍需审阅后采用。
