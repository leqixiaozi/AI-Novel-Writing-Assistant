...
- 本机新版作者开发库已在完整备份和独立恢复演练通过后，以单一事务安装 `115`—`122` 八项数据层；迁移数由 111 增至 119，42 本书、2078 张卡和 12 份正文版本保持不变。创作中枢、世界生成、漫画成图／导出和短剧入口已解除迁移门禁；验收未发送诊断或调用真实模型。
- 新版漫画工作台已接入来源整理、分话大纲和整话分镜的 AI 候选生成，沿用新版模型任务配置并冻结当前正式来源；生成结果不会自动采用，原请求可只读找回，旧大纲分镜在正式大纲变化后会停止采用。
- 新版增加独立创作向导，按五个真实里程碑显示进度并只推荐一个正式下一步；高级设置增加系统设置总入口，界面偏好仅在本次浏览会话中保留。
- 新版创作资源总览可直达题材、推进、标题、写法、反 AI、知识、世界、人物、视觉、提示词和研究来源，仍由各自工作区维护及明确采用。
- 新版书架的形式、发布分类、原创／续写、平台和创作体验筛选只认正式作品约定与已采用故事总纲，普通资料不会被误作正式分类。
- 新版独立创作中枢已接入持续诊断会话、作品／章节／原任务精确绑定、同范围深链复用、冻结状态、唯一下一步、快速诊断、诊断证据和会话归档恢复；共同暗色外壳中的面板、门禁提示与空会话状态已统一为新版主题。中枢可解释进度与失败、分析人工修改影响并返回已有来源页面处理，但不会直接生成、保存、采用、审批或取消业务内容。本机作者开发库已启用对应数据层并完成未绑定会话的新建、归档与读回。
- 新版创作首页增加“创作提醒”和“最近作品”，将等待确认、失败、质量问题和近期继续入口收在首页，不必在多个页面间寻找当前作品。
- 新版世界样本库新增独立生成与深化工作区，可从灵感、模板、属性和固定参考版本生成候选，保留人工修订、重新生成与六层深化历史；明确发布后形成包含规则、势力、地点和结构化关系的公共固定版本。本机作者开发库已启用对应数据层和发布门禁，本次未调用真实模型生成候选。
- 新版独立漫画工作台已覆盖来源整理、分话大纲、整话分镜、角色／场景设定图、逐格成图、显式采用、文字气泡预览、无字原图和项目清单导出；图片批次保留停止位置，导出提交会拒绝已经变化的采用来源。
- 新版新增独立短剧工作台：支持小说正式正文、原创构思和文本导入，按候选／采用版本维护改编策略、人物、分集和台本，并接入质量检查、修复候选、镜头、关键帧／对白音频／视频提示词及 SRT、timeline JSON、Markdown、JSON 导出。未配置媒体供应商时会明确显示不可运行，不以模拟结果冒充成功。
- 新版书内八步导航改为应用内切换并缓存工作流状态，左侧导航不再因换步整页刷新；未保存填写仍会在离开前确认。短剧核心写入会保留原请求并先核对未知结果，避免换键重复创建候选或采用。
- 新版世界样本库支持可恢复归档：归档后不再供新书导入或继续发布，已导入作品和固定版本保留；可查看归档样本并明确恢复。作者环境启用与页面验收仍待完成。
- 新版模型任务兼容 DeepSeek Flash、Pro、V4 Flash、V4 Pro 与 Reasoner；结构化生成会优先返回可用表单结果，减少只输出思考内容导致的空白。
