...
- 从编排新写整章时不再把本章旧稿交给模型，而是按当前章节计划、场景卡、参与人物、截至本章的关系状态和世界背景生成；局部改写仍严格保留授权范围。
- 整章生成先提炼可追溯的配置主干，再最多生成并核对三轮候选，自动载入综合更稳的一稿；候选会明确显示“通过、待处理或核对未完成”，但仍需作者阅读并决定是否采纳。
- 写法合同、章节表达控制和场景表达轨道只向模型发送具体写作要求，不再发送数值、档位或内部级别；生成与独立核对复用同一份背景快照。
- 文风合同在全量写作中保持完整，不会因上下文摘要只留下前半段规则；MiniMax 模型列表补充 MiniMax-M3。

#### 修复

- 章节人物只读取任务或场景明确关联的参与者，并按章节边界选择关系阶段，避免未参与人物、未来关系或本章旧稿状态提前进入新正文。
- 配置核对现在逐项验证必须动作、禁止项、正文引文和篇幅范围；漏场、相反事实、无法定位的完成声明或超出配置篇幅会保留为待处理，不再误报通过。
- 全书编排的表达轨道和人工调整的表达档位不再直接显示 `L1—L5`、百分数等内部参数，作者看到的是中文表达方式和具体执行要求。

开发说明见 [可选人工调整](./docs/wiki/workflows/optional-writing-adjustments.md)、[全书编排](./docs/wiki/product/book-arrangement.md)、[场景表达轨道合同](./docs/wiki/prompts/scene-expression-track-contract.md) 与 [叙事控制内核目标架构](./docs/wiki/architecture/narrative-control-kernel.md)，验收边界见 [人工调整检查点](./docs/checkpoints/2026-09-13-optional-writing-adjustments.md)、[编排检查点](./docs/checkpoints/2026-09-13-book-arrangement.md) 与 [场景表达轨道检查点](./docs/checkpoints/2026-09-14-scene-expression-tracks.md)。
