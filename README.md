...
- 全书编排新增“线索伏笔”入口：沿同一章节轴管理建立、强化、误导、揭示、回收和余波，区分计划与正文记录，支持未来节点拖动、事件／场景关联、风险诊断和正文证据定位。
- 独立“全书编排”菜单支持按章节对齐查看事件、场景、人物参与和表达目标，可保存编排草稿、预览应用所选章节要求，并生成可比较的大纲候选。
- 全书编排沿用底座导航、主题与表单组件，保留彩色轨道、计划虚线和历史实线；章节窗口可像滚动条一样拖动，轨道可用一个状态按钮全部折叠或展开，右键章节的事件／场景格可直接进入章节设置、场景编排或新增事件、场景、关系阶段与伏笔。
- 全书编排的章节表头会在纵向浏览时吸顶，并随矩阵横向滚动保持对齐；场景表达轨道不再重复显示章节标题。全书、当前章节、批量章节和单个场景操作使用明确范围标识，批量范围只在对应操作面板内选择。
- 章节导航窗口可直接拖动两端在 4—16 章间缩放，上方增加全书卷段缩略；顶部集中整书资料、人物关系和表达轨道，关系面板以图谱区分初始关系与章节快照并支持快速修改人物信息，卷段与章节从矩阵对象进入。
- 人物关系图支持自动重新排布，按相对方向把多条关系分散到人物卡四周的独立连接点，减少角色卡、连线与关系说明遮挡；点击人物卡可直接修改人物资料，点击关系标签可在同一面板修改初始关系并保存到本书。
- 章节表头整格可点击切换，顶部重复的场景编排与场景表达轨道快捷按钮已收口到章节右键菜单和全书入口；右侧面板的叉号与“关闭”按钮会直接关闭当前层并保留编排草稿。
- 卷段支持拖动移动、拉伸起止范围和编辑卷目标，保存草稿后可比较影响再应用；章节表头显示数字与名称。
- 事件、场景、关系与伏笔可在右侧查看来源、编辑和预览影响；人物参与支持移动、拆分与合并。五条通用场景表达轨道按章节对齐，同章多个圆形场景点放在一列内，填充量表示 L1—L5，支持上下拖动和右键精调；书级开关默认关闭，明确启用后才在后续章节生成中影响表达方式。
- 章节场景提供彩色篇幅分配条和场景卡工作区：可拖动相邻预算、排序、拆分、合并及编辑节奏与边界，预览后整章应用，已有正文保持不变。
- 核对结果按 AI 味、逻辑性、人物一致性和节奏偏差生成章节热力表，颜色与问题数用于快速定位，点击风险格继续查看原问题依据与修复入口。
- 从具体章节或核对问题进入复查与修复，复用原候选比较、手改、核对和采纳流程；关闭面板后可恢复未完成的编辑。
- 原作品页和章节编辑器增加折叠的“人工调整”：可配置本次要求、书章默认和五档表达参数，查看不会改变作品。
- 支持独立保存候选与手改稿、按实际稿件核对、定位历史证据，再明确采纳正文。
- 支持指定章节接管与导演交接；旧运行失去的写入许可不会随交接结束恢复，采纳后的资料同步可重试。
