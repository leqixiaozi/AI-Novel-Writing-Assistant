...
当前开发库已在用户确认后原地安装 `133_card_kernel_tables_only_upgrade`，达到上述 79+4、零业务视图和零兼容 schema 结构；迁移账本从 128 项增加到 129 项，不伪造 `132` 登记。作品、正文、研究原文、已有版本和模型配置均通过保留核验；源镜像与旧默认目录保留为内部归档证据。开发与联调统一使用这个库，不自动另建测试库或重建数据。检查与尚未验收的边界见[开发交付记录](../../new-design/docs/development-delivery.md#当前数据库更新方式)。

纯表空库基线 `132` 的历史隔离验证仍有效，但与当前开发库升级是不同证据。启动门禁接受真实的 `132` 或 `133` 账本，并核对 `card_kernel_v2` 的 installed／operational、`details.storage=tables_only` 和实际结构；原有停用功能不会仅因结构完成而自动开放。
...
当前 [`migrations.ts`](../../new-design/src/server/database/migrations.ts) 注册空库基线 `132`；存量 `131` 库使用显式 [`upgrade-card-kernel.cjs`](../../new-design/scripts/upgrade-card-kernel.cjs) 执行 `133`。后者核对准确目标、停写、已验证保护文件及未变化的数据基线，在一个事务内完成结构、映射、旧历史哈希核验和最终登记。普通启动仅核对状态，不自动升级。后续继续追加增量，不重跑基线或重复还原。重大变更另核授权，详见[数据库增量门禁](../../new-design/AGENTS.md#数据库增量门禁)。
